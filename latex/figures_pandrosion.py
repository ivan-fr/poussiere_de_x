...
def fig1_multistart_basins():
    ...
    # LEFT: Plain σ basins (fractal Julia)
    ax = axes[0]
    img_plain = np.zeros((res, res, 3))
    cmap = plt.cm.tab10
    for i, re in enumerate(np.linspace(-rng, rng, res)):
        for j, im in enumerate(np.linspace(-rng, rng, res)):
            z0 = re + 1j*im
            idx, _ = pandrosion_color(x, p, z0, max_iter=30)
            if idx == -1:
                img_plain[res-1-j, i] = [0, 0, 0]  # diverge = black
            else:
                img_plain[res-1-j, i] = cmap(idx % 10)[:3]
    ax.imshow(img_plain, extent=[-rng, rng, -rng, rng], interpolation='bilinear')

    # Mark anchors
    alpha = (1/x)**(1/p)
    for k in range(p):
        a = cyc_anchor(alpha, p, k)
        ax.plot(a.real, a.imag, 'w*', markersize=18, markeredgecolor='black', markeredgewidth=1.5)
    ax.set_title(rf'Plain $\sigma$ basins $(x={int(x)},\,p={p})$ — fractal Julia structure', fontweight='bold')
    ax.set_xlabel(r'$\mathrm{Re}(z_0)$'); ax.set_ylabel(r'$\mathrm{Im}(z_0)$')

    # RIGHT: Multi-start basin (universal — converges to chosen α₀)
    ax = axes[1]
    img_ms = np.zeros((res, res, 3))
    R = 0.5
    # Multi-start with seed = α₀ + ε(z - α₀), all converge to α₀
    for i, re in enumerate(np.linspace(-rng, rng, res)):
        for j, im in enumerate(np.linspace(-rng, rng, res)):
            z0 = re + 1j*im
            ...
            for it in range(20):
                ...
                if abs(z - alpha) < 1e-6:
                    converged = True
                    break
            color_intensity = max(0, 1 - it/20.0)
            img_ms[res-1-j, i] = [0.2 + 0.7*color_intensity, 0.4 + 0.5*color_intensity, 0.9]
    ax.imshow(img_ms, extent=[-rng, rng, -rng, rng], interpolation='bilinear')
    ax.plot(alpha.real, alpha.imag, 'r*', markersize=22, markeredgecolor='white', markeredgewidth=2,
             label=r'$\alpha_0$ (target)')
    ax.legend(loc='upper right', framealpha=0.9)
    ax.set_title(r'Multi-start basin: universal convergence to $\alpha_0$', fontweight='bold')
    ax.set_xlabel(r'$\mathrm{Re}(z_0)$'); ax.set_ylabel(r'$\mathrm{Im}(z_0)$')

    plt.suptitle(r'\bf Fig 1. Plain $\sigma$ vs Pandrosion-multi-start: '
                 r'§95 universal convergence circumvents §120 McMullen barrier',
                 fontsize=12, y=1.02)
    plt.tight_layout()
    plt.savefig(f'{OUT}/fig01_multistart_basins.png', dpi=200)
    plt.close()
    print("Fig 1 ✓")
...
def fig2_cyclotomic_anchors():
    ...
    plt.suptitle(r'\bf Fig 2. Cyclotomic anchor families $\gamma_k = \alpha\cdot e^{2\pi ik/p}$ '
...
def fig3_loglog_convergence():
    ...
    plt.suptitle(r'\bf Fig 3. Effective Kung-Traub loglog complexity (§98, §101) '
...
def fig4_voronoi_julia_null():
    ...
    plt.suptitle(r'\bf Fig 4. Pandrosion Julia-null theorem (§103): '
...
def fig5_pandrosion_riemann_analogy():
    ...
    plt.title(r'\bf Fig 5. Pandrosion-Riemann zeta analogy (§109): '
...
def fig6_spectral_determinant():
    ...
    plt.suptitle(r'\bf Fig 6. Pandrosion spectral determinant (§107): '
...
def fig7_aitken_acceleration():
    ...
    plt.suptitle(r'\bf Fig 7. Aitken-Steffensen acceleration (1926/1933, §113): '
...
def fig8_fatou_julia_decomposition():
    ...
    # LEFT: number of iterations to converge (Fatou structure)
    ax = axes[0]
    img = np.zeros((res, res))
    for i, re in enumerate(np.linspace(-rng, rng, res)):
        for j, im in enumerate(np.linspace(-rng, rng, res)):
            z0 = re + 1j*im
            _, n_iter = pandrosion_color(x, p, z0, max_iter=20, tol=1e-4)
            img[res-1-j, i] = n_iter

    im = ax.imshow(img, extent=[-rng, rng, -rng, rng],
                    cmap='magma', origin='upper', interpolation='bilinear')
    plt.colorbar(im, ax=ax, label='Iterations')

    alpha = (1/x)**(1/p)
    for k in range(p):
        a = cyc_anchor(alpha, p, k)
        ax.plot(a.real, a.imag, '*', color='cyan', markersize=18,
                 markeredgecolor='white', markeredgewidth=1.5)
    ax.set_title(r'Iteration count to converge — Fatou structure',
                 fontweight='bold')
    ax.set_xlabel(r'$\mathrm{Re}(z_0)$'); ax.set_ylabel(r'$\mathrm{Im}(z_0)$')

    # RIGHT: which anchor (basin labeling)
    ax = axes[1]
    ...
    for i, re in enumerate(np.linspace(-rng, rng, res)):
        for j, im in enumerate(np.linspace(-rng, rng, res)):
            z0 = re + 1j*im
            idx, _ = pandrosion_color(x, p, z0, max_iter=20)
            if idx == -1:
                img[res-1-j, i] = [0, 0, 0]
            else:
                img[res-1-j, i] = cmap(idx % 10)[:3]
    ax.imshow(img, extent=[-rng, rng, -rng, rng], interpolation='bilinear')

    for k in range(p):
        a = cyc_anchor(alpha, p, k)
        ax.plot(a.real, a.imag, '*', color='white', markersize=20,
                 markeredgecolor='black', markeredgewidth=1.5)
        ax.annotate(rf'$V_{{{k}}}$', (a.real + 0.06, a.imag + 0.06),
                     fontsize=12, fontweight='bold', color='white',
                     bbox=dict(facecolor='black', alpha=0.5, edgecolor='none'))
    ax.set_title(r'Voronoï basins $V_k$ — Pandrosion Fatou decomposition (§103)',
                 fontweight='bold')
    ax.set_xlabel(r'$\mathrm{Re}(z_0)$'); ax.set_ylabel(r'$\mathrm{Im}(z_0)$')

    plt.suptitle(r'\bf Fig 8. Fatou/Julia decomposition for $\sigma_{p,x}$ '
                 r'$(x=2, p=4)$ — §102, §103, §105',
                 fontsize=12, y=1.02)
    plt.tight_layout()
    plt.savefig(f'{OUT}/fig08_fatou_julia.png', dpi=200)
    plt.close()
    print("Fig 8 ✓")
...
def fig9_spectral_balance():
    ...
    # LEFT: weights ν_k as vectors in ℂ
    ax = axes[0]
    x = 2.0
    colors = plt.cm.tab10
    for ax_p_idx, p in enumerate([5]):
        alpha = (1/x)**(1/p)
        weights = [x * cyc_anchor(alpha, p, k) / p for k in range(p)]
        running = 0+0j
        for k, w in enumerate(weights):
            color = colors(k % 10)
            ax.annotate('', xy=(running.real + w.real, running.imag + w.imag),
                         xytext=(running.real, running.imag),
                         arrowprops=dict(arrowstyle='->', color=color, lw=2.5))
            running += w
            ax.annotate(rf'$\nu_{{{k}}}$',
                         ((running.real - w.real/2)*1.05, (running.imag - w.imag/2)*1.05),
                         fontsize=11, color=color, fontweight='bold')

    ax.plot(0, 0, 'k+', markersize=20, markeredgewidth=2.5)
    ax.annotate(r'$\sum \nu_k = 0$', (0, 0),
    ...
    ax.set_title(rf'Vectorial sum of spectral weights $\nu_k = x\alpha_k/p$ '
                 rf'$(p=5,\,x={int(x)})$', fontweight='bold')
    ax.set_xlabel(r'$\mathrm{Re}$'); ax.set_ylabel(r'$\mathrm{Im}$')

    # RIGHT: |Σ ν_k| for various (x, p) — should always be 0
    ax = axes[1]
    ps = list(range(2, 13))
    xs_to_test = [1, 2, 3, 5, 10]
    matrix = np.zeros((len(xs_to_test), len(ps)))
    for i, x in enumerate(xs_to_test):
        for j, p in enumerate(ps):
            alpha = (1/x)**(1/p)
            sum_nu = sum(x * cyc_anchor(alpha, p, k) / p for k in range(p))
            matrix[i, j] = abs(sum_nu)

    log_mat = np.log10(np.maximum(matrix, 1e-17))
    im = ax.imshow(log_mat, aspect='auto', cmap='RdYlGn_r', vmin=-17, vmax=0,
                    extent=[ps[0]-0.5, ps[-1]+0.5, len(xs_to_test)-0.5, -0.5])
    plt.colorbar(im, ax=ax, label=r'$\log_{10}|\sum \nu_k|$')
    ax.set_xticks(ps)
    ax.set_yticks(range(len(xs_to_test)))
    ax.set_yticklabels([f'$x={x}$' for x in xs_to_test])
    ax.set_xlabel('$p$', fontsize=11)
    ax.set_title(r'Vanishing $|\sum_k \nu_k| \approx 0$ for all $(x, p)$ (machine precision)',
                 fontweight='bold')

    plt.suptitle(r'\bf Fig 9. Multi-start spectral balance $\sum_k \nu_k = 0$ '
                 r'(§106, §122 Vieta $e_1 = 0$)',
                 fontsize=12, y=1.02)
    plt.tight_layout()
    plt.savefig(f'{OUT}/fig09_spectral_balance.png', dpi=200)
    plt.close()
    print("Fig 9 ✓")
...
def fig10_module_graph():
    ...
    # Legend / counters
    legend_text = (
        r'\bf Lean 4 / Mathlib v4.7.0 corpus}: 121 modules, axiom-clean'
        '\n'
    ...
    ax.axis('off')
    ax.set_title(r'\bf Fig 10. Pandrosion-Steffensen Lean 4 corpus structure '
