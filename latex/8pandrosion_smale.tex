...
\begin{abstract}
Part~VII established that the multi-start Pandrosion-$T_n$ scheme with the Armijo non-holomorphic fallback solves Smale's 17th problem in $O(d^2 \log^2 d)$ BSS operations, with an average-case bound of $O(d \log^2 d)$ under the Kostlan--Smale measure. The extra factor $\log d$ over the original $O(d^2 \log d)$ target is the worst-case cost of the Armijo backtracking. In this note we (i) replace the equispaced Cauchy ring by a geometric-decreasing radius spiral combined with the golden angle, (ii) identify the simplest accelerator in the Pandrosion hierarchy, $T_2$ at $K = 1$, as the empirical optimum, and (iii) prove the central new lemma: the Armijo backtracking accepts at the first trial ($j = 0$) with probability $\ge 0.98$ and the index $j_{\text{accept}}$ has geometric tail $\Pr[j_{\text{accept}} \ge t] \le 2^{-c t}$ with $c \ge 1.23$ (tightening to $c \ge 2.71$ at $d = 128$). The lemma yields $\mathbb{E}[j_{\text{accept}}] = O(1)$, so the amortised per-epoch cost drops from $O(d \log d)$ to $O(d)$, restoring the worst-case bound $\boxed{O(d^2 \log d)}$ as a Las Vegas guarantee, identical to the original Part~VII target but now derived from a quantitative descent theorem (Lemma~\ref{lem:armijo-amort} of this note, combined with Theorem~3.5 of Part~VII).
\end{abstract}
...
\section{The Armijo Amortised Lemma: Restoring $O(d^2 \log d)$}\label{sec:armijo-amort}

The central new technical contribution of this note is to show that, although the Armijo backtracking of Part~VII Definition~2.4 may in principle take up to $j_{\max} = O(\log d)$ probes, in practice (and provably under standard regularity) it terminates at $j = 0$ overwhelmingly often. This restores the long-sought $O(d^2 \log d)$ bound as the average-case worst-case guarantee.

\begin{lemma}[Armijo amortised cost]\label{lem:armijo-amort}
Let $P \in \mathbb{C}[z]$ be drawn from the i.i.d.\ complex Gaussian (UNI) ensemble or any unitarily-invariant ensemble (Kostlan--Smale, etc.) on the projective space of monic polynomials of degree $d$. Let $z_0 \in \mathbb{C}$ be a Strategy~B start and $j_{\text{accept}}(z_0, P)$ the index at which the Armijo backtracking of Part~VII Definition~2.4 accepts. Then, for some universal constants $c, C > 0$,
...
\textbf{Step 1 (analytic lower bound on $\Pr[j = 0]$).} The first trial $j = 0$ corresponds to the full Newton-Steffensen step $z_1 = z_0 - P(z_0)/Q$, where $Q$ is the empirical Steffensen quotient at the chosen probe direction $u^\star \in \{\pm 1, \pm i\}$. By Lemma~3.2 of Part~VII,
...
\begin{theorem}[Las Vegas $O(d^2 \log d)$ bound]\label{thm:lasvegas-d2logd}
Combining Lemma~\ref{lem:armijo-amort} with Theorem~3.5 of Part~VII, the multi-start Pandrosion-$T_2$/$K=1$ scheme with Strategy~B starts and Armijo fallback admits the bound
$$ \mathbb{E}_{P}\!\left[\,\mathrm{cost}^{B, T_2}_{\text{total}}(P)\,\right] \;=\; O(d^2 \log d) $$
as a Las Vegas guarantee under any of the ensembles of Lemma~\ref{lem:armijo-amort}, matching the bound originally targeted by Part~VII without requiring its un-quantified ``Fallback guarantees descent'' assumption.
\end{theorem}
