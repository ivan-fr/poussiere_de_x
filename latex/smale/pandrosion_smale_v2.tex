...
\label{thm:main}
Given a monic polynomial $P$ of degree~$d$ with simple roots, the Pure Pandrosion-$T_3$ algorithm with $d$ equispaced starts on the Cauchy circle and iterated scaling finds an approximate zero in at most $O(d^3)$ arithmetic operations in the BSS model.
\end{theorem}

\begin{proof}
By Theorem~\ref{thm:winding}, there exists $s^*$ such that $\Lambda_{s^*} < 1$ at every epoch.  Theorem~\ref{thm:universal_descent} gives $\Lambda \leq e^{-\pi/2}$ per epoch, requiring $\lceil 2d\log R/\pi\rceil = O(d)$ epochs to reach the basin of attraction.  By Theorem~\ref{thm:contraction_ratio}, once $|a-\zeta| = O(1)$, convergence accelerates to quadratic in $O(\log\log\varepsilon^{-1})$ additional epochs.  Each epoch consists of~$3$ Pandrosion steps at $O(d)$ cost each.  Total per orbit: $O(d) \times O(d) = O(d^2)$.  With $d$ starting orbits: $O(d^3)$.
\end{proof}
