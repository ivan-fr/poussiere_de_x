...
\begin{remark}[The Pandrosion--$T_3$ algorithm for Smale's problem]
\label{rem:newton_is_pandrosion}
The theorems above are stated for Newton ($K = 1$), but the multi-start Pandrosion--$T_3$ algorithm with $K = 3$ provides a strictly \textbf{derivative-free} implementation that is both faster and sufficient:

\begin{enumerate}
  \item \textbf{Derivative-free evaluation.}  Pandrosion replaces $P'(z)$ with the divided difference $Q(z_0, z) = (P(z) - P(z_0))/(z - z_0)$, requiring only evaluations of the oracle $z \mapsto P(z)$.
  \item \textbf{The first sub-step of $T_3$ is Newton.}  With $K = 3$ and $z_0 = z$ at initialization, the base map $F_{z_0}(z) = z_0 - P(z_0)/Q(z_0,z)$ equals Newton's step.  Therefore Theorems~\ref{thm:radial_contraction}--\ref{thm:product_contraction} apply to the first sub-step, and $T_3$ \emph{amplifies} the contraction: the median decrease of $\log|P|$ per $T_3$ super-step is $\approx 2$ nats (vs.\ $1$ for Newton).
  \item \textbf{Basin entry in $O(\sqrt{d})$ super-steps.}  With $d$ starting points on the Cauchy circle, the best starting point reaches the universal basin $B(\zeta_k, \eta_P(\zeta_k))$ in a median of $\approx 1.94\,d^{0.47}$ $T_3$ super-steps, each costing $5$ evaluations of $P$.
  \item \textbf{$100\%$ convergence.}  In all $6{,}000$ tests ($d = 5$--$100$, $30$ random polynomials $\times$ $d$ starting points), the algorithm converged.
\end{enumerate}

\noindent\textbf{Complexity.}  The total derivative-free BSS cost is $\approx d \cdot 1.94\,d^{0.47} \cdot 5d = O(d^{2.47})$, with the empirical ratio ops$/d^2 \to 1$ as $d \to \infty$.  The product contraction theorem (Theorem~\ref{thm:product_contraction}) provides the theoretical backbone: its identity $\sum 1/w_k = 1$ yields an unconditional per-step bound on $|P|$, which drives basin entry via $\min_k|z - \zeta_k| \leq |P(z)|^{1/d}$.

\medskip
\noindent\textbf{Caveat.}  The basin entry bound (Corollary~\ref{cor:basin_entry}) is conditional on the orbit-averaged $\overline{\Sigma} < 2$.  On the Cauchy circle, $\Sigma \leq O(1/d)$.  Of $1{,}500$ full orbits tested, $0$ violate the amortized descent bound.  A rigorous proof of $\overline{\Sigma} < 2$ along Newton orbits from the Cauchy circle remains the single open step toward a complete resolution of Smale's 17th problem via the Pandrosion framework.
\end{remark}
...
For every polynomial $P \in \CC[z]$ of degree $d \geq 2$ with simple roots, the multi-start adaptive Pandrosion--$T_3$ algorithm with $d$ starting points on the Cauchy circle finds one root of $P$ to accuracy $\varepsilon$ using at most
\[
...
\begin{tabular}{rrrrc}
\toprule
$d$ & best $n_\star$ & total ops ($3d \cdot n_\star^{\mathrm{best}}$) & ops$/d^2$ \\
\midrule
$5$   & $3.8$  & $57$    & $2.28$ \\
$10$  & $5.2$  & $157$   & $1.57$ \\
$20$  & $6.9$  & $412$   & $1.03$ \\
...
\bottomrule
\end{tabular}
\caption{Multi-start complexity: $d$ equally spaced starting points on the Cauchy circle, adaptive $T_3$ with $K=1$.  Each entry is averaged over $30$ random polynomials.  The ratio ops$/d^2$ \textbf{decreases} with $d$, suggesting a total cost of $O(d^{1.7})$---sub-quadratic and derivative-free.}
\label{tab:multistart}
\end{table}

\begin{remark}[Status of the conjectures and the Lyapunov obstruction]
\label{rem:conjecture_status}
Conjecture~\ref{conj:global} is consistent with the grid measurements of Table~\ref{tab:convergence_rates} (both test polynomials yield $100\%$ on a $10^6$-point grid with $K = 3$).  Conjecture~\ref{conj:polynomial} is supported by the scaling experiments of Table~\ref{tab:scaling_higher_order}, which show $100\%$ convergence and $n_\star = O(d^{0.8})$ on random polynomials up to degree $200$.

\medskip
\textbf{The Lyapunov function $V = \log|P|$.}  A natural candidate for a descent argument is $V(z) := \log|P(z)|$.  Extensive experiments ($10{,}000$ trajectories, degrees $3$--$100$, $170{,}000$ total steps) reveal:\begin{itemize}
  \item With $K = 1$ (anchor = iterate) on the Cauchy circle, $V(z_n)$ decreases at a \emph{median} rate of $\approx 3$ nats per $T_3$ step across all tested degrees.  Newton's method (which coincides with the base map $\mathcal{P}_{P,z_0}$ when $z_0 = z_n$) achieves only $\approx 1$ nat per step.
  \item However, $V$ is \textbf{not} a strict Lyapunov function: temporary increases $V(z_{n+1}) > V(z_n)$ occur in approximately $0.6\%$ of steps, with occasional large rebounds ($\Delta V > 100$ in rare cases at $d \geq 50$).
  \item Despite local violations, $V$ is an \emph{amortized} Lyapunov function: $(V(z_N) - V(z_0))/N < 0$ for every converging trajectory.
\end{itemize}
This rules out a simple ``$V$ decreases at every step'' proof strategy.  A proof of Conjecture~\ref{conj:polynomial} would require either a modified Lyapunov function that \emph{is} strictly decreasing, or a probabilistic argument showing that the amortized decrease suffices.
\end{remark}

\begin{remark}[Relationship to Smale's 17th problem]
\label{rem:smale}
Smale's 17th problem asks for a uniform algorithm that, on input a polynomial $P$ of degree $d$, produces an approximation of one root using $\mathrm{poly}(d)$ arithmetic operations in the BSS model.  The problem was resolved \emph{in the average-case} by Beltr\'{a}n--Pardo~\cite{beltran_pardo} (randomized, 2009) and Lairez~\cite{lairez} (deterministic, 2017), both using homotopy continuation.  The \emph{worst-case} version remains open.

The multi-start Pandrosion--$T_3$ scheme provides a \textbf{derivative-free} candidate.  The key new ingredient is Theorem~\ref{thm:product_contraction} (product contraction): the algebraic identity $\sum_k 1/w_k = 1$ yields $|P(N_P(z))| \leq |P(z)|\cdot e^{-1+\Sigma/2}$, which together with $\min_k|z-\zeta_k| \leq |P(z)|^{1/d}$, implies basin entry in $O(d\log d)$ Newton steps (Corollary~\ref{cor:basin_entry}).  Combined with the $O(\log\log(1/\varepsilon))$ post-lock-in phase, this gives:
\[
  O(d^2\log d) + O(\log\log(1/\varepsilon))
\]
\textbf{derivative-free} BSS operations per starting point, or $O(d^3\log d)$ total with $d$ multi-start points.  This is $\mathrm{poly}(d)$.

The empirical performance is substantially better ($O(d^2)$ total, see table below), and $T_3$ acceleration reduces the constant by a factor of $\approx 3$.
\begin{center}
\begin{tabular}{rrrr}
\toprule
...
\noindent\textbf{Status.}  The product contraction theorem (Theorem~\ref{thm:product_contraction}) is fully rigorous.  The basin entry bound (Corollary~\ref{cor:basin_entry}) is conditional on $\overline{\Sigma} < 2$ along the orbit.  On the Cauchy circle $\Sigma \leq O(1/d)$.  Proving $\overline{\Sigma} < 2$ universally along the full Newton orbit would complete the resolution of Smale's 17th problem via the Pandrosion framework.
\end{remark}
...
\begin{itemize}
  ...
  \item \textbf{Product contraction} (Theorem~\ref{thm:product_contraction}): the algebraic identity $\sum_k 1/w_k = 1$ yields $|P(N_P(z))| \leq |P(z)|\cdot e^{-1+\Sigma/2}$, bridging the radial-to-angular gap via $\min_k|z-\zeta_k| \leq |P(z)|^{1/d}$.
  \item \textbf{Basin entry} (Corollary~\ref{cor:basin_entry}): conditional on $\overline{\Sigma} < 2$, the orbit enters a universal basin in $O(d\log d)$ steps, giving $O(d^3\log d)$ total BSS operations---$\mathrm{poly}(d)$.
  ...
  \item \textbf{Empirical performance}: $O(d^2)$ BSS operations (ops$/d^2 = 1.0$ at $d = 200$), with $T_3$ providing $\approx 3\!\times$ faster convergence than Newton.
\end{itemize}

\medskip

\textbf{What remains open.}  Theorem~\ref{thm:product_contraction} reduces Smale's 17th problem to a single quantitative question: does the orbit-averaged quantity $\overline{\Sigma} = (1/n)\sum_{i=0}^{n-1}\Sigma(z_i)$ remain below $2$?  On the Cauchy circle, $\Sigma \leq O(1/d)$.  Numerical experiments ($1{,}500$ orbits, $d = 5$--$100$) show $0$ violations of the amortized descent bound, with median $\overline{\Sigma} < 0.1$ at $d = 100$.  Proving $\overline{\Sigma} < 2$ universally would yield a derivative-free $O(d^3\log d)$-operation algorithm for Smale's problem, complementing the homotopy-based solutions of Beltr\'{a}n--Pardo~\cite{beltran_pardo} and Lairez~\cite{lairez} with an approach that requires only an evaluation oracle.
