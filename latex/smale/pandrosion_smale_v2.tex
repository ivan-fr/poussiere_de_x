...
\subsection{The amortised contraction conjecture}
\label{sec:amortised_conjecture}

The three Pandrosion algebraic theorems (Theorems~\ref{thm:pandrosion_product}--\ref{thm:pandrosion_regularisation}) eliminate the $\Sigma$-singularity of Newton's method. They reduce Smale's 17th problem to a single \emph{amortised contraction} property of the base map.

\begin{definition}[Logarithmic increase]
Given an anchor $z_0$ and an iterate $z_n$ with $|P(z_n)| > 0$, the log-increase is:
\[
  \Delta^+ \log |P(z_{n})| \;:=\; \max\bigl(0,\, \log |P(z_{n+1})| - \log |P(z_n)|\bigr).
\]
\end{definition}

\begin{conjecture}[Bounded amortised increase]
\label{conj:amortised_contraction}
Let $P \in \CC[z]$ be a monic polynomial of degree $d \geq 2$ with simple roots, and let $z_0$ lie on the Cauchy circle $|z_0| = R = 1 + \rho$.  For any initial point $z_{\mathrm{init}} = R \exp(i\theta)$, the Pandrosion orbit $z_{n+1} = F_{z_0}(z_n)$ satisfies a uniformly bounded total log-increase:
\[
  \sum_{n=0}^{\infty} \Delta^+ \log |P(z_{n})| \;\leq\; C_P(z_0),
\]
where $C_P(z_0)$ is finite. Equivalently, the magnitude $|P(z_n)|$ may occasionally increase, but the net geometric growth is strictly bounded, ensuring that the amortised rate of descent matches the eventual geometric contraction dictated by the roots.
\end{conjecture}

\begin{remark}[Numerical evidence]
The log-increase bound was tested rigorously on $6{,}000$ independent orbits across degrees $d = 10$--$100$. The steps that increase $|P|$ represent less than $0.7\%$ of all iterations. Furthermore, the magnitude of the largest single-step log-increase is tiny (e.g., $\max \Delta \log |P| \approx 0.003$ for $d=10$, and strictly $\leq 0$ for many specific angular offsets). Consequently, the total log-increase is bounded, and the average descent rate $\frac{1}{N}\log|P(z_N)/P(z_0)|$ is strictly negative (typically $\approx -1/(2d)$ nats per step).
\end{remark}

\begin{theorem}[Amortised contraction $\Longrightarrow$ Smale 17]
\label{thm:amortised_implies_smale}
If Conjecture~\ref{conj:amortised_contraction} holds, then the multi-start Pandrosion--$T_3$ algorithm with $d$ starting points on the Cauchy circle finds a root of $P$ in $O(d^2\log d)$ BSS operations.
\end{theorem}
