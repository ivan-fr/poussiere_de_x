...
With anchor-update period $K = 1$, the base Pandrosion map coincides with Newton: $\mathcal{P}_{P,z_n}(z_n) = z_n - P(z_n)/Q(z_n,z_n) = z_n - P(z_n)/P'(z_n)$, since $Q(z,z) = P'(z)$ by the definition of the divided difference.  The $T_3$ acceleration then provides a \emph{derivative-free refinement} of Newton, using only evaluations of $P$ (via $Q(z_0, w) = (P(w)-P(z_0))/(w-z_0)$, which requires no explicit derivative).  Theorem~\ref{thm:radial_contraction} therefore gives the first half of a pre-lock-in bound for the Pandrosion framework: the orbit reaches the root cluster in $O(d)$ steps; what remains is to prove entry into a universal basin.
\end{remark}
