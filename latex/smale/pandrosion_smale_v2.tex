...
\begin{remark}[Newton, Pandrosion, and Smale's 17th problem]
\label{rem:newton_is_pandrosion}
Theorems~\ref{thm:radial_contraction}--\ref{thm:product_contraction} and Corollary~\ref{cor:basin_entry} together give:
\begin{enumerate}
  \item \textbf{Radial contraction} (Theorem~\ref{thm:radial_contraction}): the orbit reaches the root cluster in $O(d)$ steps.
  \item \textbf{Per-root contraction} (Theorem~\ref{thm:per_root}): on the Cauchy circle, every root is simultaneously approached.
  \item \textbf{Product contraction} (Theorem~\ref{thm:product_contraction}): $|P|$ decreases by factor $\approx e^{-1}$ per step from the Cauchy circle, proved via the algebraic identity $\sum 1/w_k = 1$.
  \item \textbf{Basin entry} (Corollary~\ref{cor:basin_entry}): $\min_k|z_n - \zeta_k| \leq \eta_P$ after $O(d\log d)$ steps, closing the angular gap.
  \item \textbf{Post-lock-in} (Theorem~\ref{thm:iteration_count}): $O(\log\log(1/\varepsilon))$ from the basin.
\end{enumerate}
The Pandrosion framework implements these Newton steps \textbf{derivative-free}: $Q(z_0, z) = (P(z)-P(z_0))/(z-z_0)$ replaces $P'(z)$ using only evaluations of $P$.  The $T_3$ acceleration reduces the step count by a factor of $\approx 3$.

\medskip
\noindent\textbf{Caveat.}  The bound in Corollary~\ref{cor:basin_entry} is conditional on $\overline{\Sigma} < 2$ along the orbit.  On the Cauchy circle, $\Sigma \leq O(1/d)$, and the median orbit-averaged $\overline{\Sigma}$ is $< 0.1$ at $d = 100$.  However, rare orbits exhibit large instantaneous spikes in $\Sigma$ (when $z$ passes very close to a root).  Numerical experiments on $1{,}500$ orbits show $0$ violations of the amortized bound, but a rigorous proof that $\overline{\Sigma} < 2$ holds universally remains open.
\end{remark}
