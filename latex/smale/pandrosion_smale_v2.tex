...
\textbf{What remains open.}  Theorems~\ref{thm:pandrosion_product}--\ref{thm:pandrosion_regularisation} provide an \emph{exact} product factorization and a \emph{bounded} divided difference, removing the $\Sigma$-divergence barrier that blocks Newton-based proofs.  However, the exact Pandrosion bound $|P(z_0)/c_d \cdot \prod Q_k/Q^d|$ must be shown to give amortised contraction $< 1$ along orbits from the Cauchy circle.  Numerically ($6{,}000$ orbits, $d = 5$--$100$), the amortised rate is \emph{always} negative ($-0.28$ to $-0.06$ nats/step), with $0$ violations.  Proving this unconditionally---or, equivalently, bounding $\prod |Q_k|/|Q|^d$ along Pandrosion orbits---would yield a derivative-free $O(d^2)$-operation algorithm for Smale's 17th problem.
