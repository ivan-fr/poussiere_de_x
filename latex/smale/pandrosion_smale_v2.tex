...
\begin{remark}[From Cauchy circle to Smale's problem]
\label{rem:newton_is_pandrosion}
With $K = 1$, the Pandrosion base map is Newton: $\mathcal{P}_{P,z_n}(z_n) = z_n - P(z_n)/P'(z_n)$.  Theorems~\ref{thm:radial_contraction} and~\ref{thm:per_root} together establish:
\begin{enumerate}
  \item The orbit from the Cauchy circle contracts \emph{radially} at rate $(1 - 1/d)$ per step (Theorem~\ref{thm:radial_contraction}).
  \item On the Cauchy circle, every root is \emph{simultaneously approached} (Theorem~\ref{thm:per_root}).
\end{enumerate}
The remaining gap to Smale's 17th problem is that Theorem~\ref{thm:per_root} is proved only for the \emph{first} Newton step (from $|z| = R$).  Along subsequent iterates---which are no longer on a fixed circle---per-root violations do occur ($\approx 3\%$ of per-root ratios exceed $1$ inside the root cluster).  Closing this gap would give an unconditional $O(d^2 \log(1/\eta_P) + \log\log(1/\varepsilon))$ bound.
\end{remark}
...
\label{rem:smale}
Smale's 17th problem asks for a uniform algorithm that, on input a polynomial $P$ of degree $d$, produces an approximation of one root using $\mathrm{poly}(d)$ arithmetic operations.  The problem was resolved \emph{in the average-case} by Beltr\'{a}n--Pardo~\cite{beltran_pardo} (randomized, 2009) and Lairez~\cite{lairez} (deterministic, 2017), both using homotopy continuation.  The \emph{worst-case} version remains open.

The adaptive Pandrosion--Steffensen scheme, \emph{as stated and as proved here}, does \textbf{not} resolve Smale's problem.  What we establish is strictly weaker but fully rigorous:
\begin{itemize}
  \item the \emph{post-lock-in} phase runs in $O(\log\log \varepsilon^{-1})$ Steffensen steps (Theorem~\ref{thm:iteration_count});
  \item pole collisions happen only on a Lebesgue-null set (Theorem~\ref{thm:pole_avoidance});
  \item every simple root has an explicit universal basin of radius $\eta_P(\zeta)$ with contraction ratio $L \leq 4/9$ (Theorem~\ref{thm:universal_local}).
\end{itemize}
The gap separating these results from Smale's problem is exactly the pre-lock-in phase: we have no uniform bound on $n_\star$.  The empirical scaling $n_\star \approx 1.42\, d^{0.74}$ (power-law fit, $3000$ single-start tests on random polynomials of degree $3$--$100$) suggests $n_\star = O(d)$, but proving this---or even the weaker $n_\star = O(\mathrm{poly}(d))$---remains the central open question.  Our contribution is to locate this difficulty precisely and to provide the surrounding rigorous infrastructure.
\end{remark}
...
We have extended Pandrosion's geometric iteration from $\RR^+$ to $\CC$ and built a rigorous convergence framework for the universal polynomial generalization:

\begin{itemize}
  \item The \textbf{generalized Pandrosion operator} $\mathcal{P}_{P,z_0}$ applies to \emph{any} polynomial $P \in \CC[z]$, recovering Newton's method as a limiting case when $z_0 = z_n$ (Sections~\ref{sec:complex_recap}--\ref{sec:universal_poly}).
  \item \textbf{Universal local convergence} (Theorem~\ref{thm:universal_local}): every simple root $\zeta$ of $P$ admits an explicit basin of radius $\eta_P(\zeta) = \min(\delta/2,\, |P'(\zeta)|/(8M_2),\, \sqrt{|P'(\zeta)|/(4M_3)})$ on which $\mathcal{P}_{P,z_0}$ is a strict contraction with universal ratio $L \leq 4/9$.
  \item \textbf{Pole avoidance in full measure} (Theorem~\ref{thm:pole_avoidance}): on any compact set, the initial data producing a pole collision form a Lebesgue-null set.
  ...
  \item \textbf{Lyapunov structure}: $V(z) = \log|P(z)|$ acts as an \emph{amortized} Lyapunov function for the adaptive $T_3$ scheme, decreasing at a median rate of $\approx 3$ nats per step (vs.\ $\approx 1$ for Newton), though strict monotonicity fails in $\approx 0.6\%$ of steps.
\end{itemize}

\medskip

\textbf{What remains open.}  The central gap is the pre-lock-in phase $n_\star$: we have no uniform bound.  The empirical scaling $n_\star \approx 1.42\, d^{0.74}$ (Table~\ref{tab:scaling_higher_order}) suggests $n_\star = O(d)$, which would yield a $\mathrm{poly}(d)$ root-finding algorithm in the BSS model---complementing the homotopy-based solutions of Beltr\'{a}n--Pardo~\cite{beltran_pardo} and Lairez~\cite{lairez} with a \emph{derivative-free} approach.  We do \textbf{not} claim to resolve Smale's 17th problem; rather, we locate the remaining difficulty precisely and provide strong evidence that a proof may be within reach.
