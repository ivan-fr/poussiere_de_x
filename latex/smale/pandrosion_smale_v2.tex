...
\begin{conjecture}[Half-plane containment]
\label{conj:halfplane}
For every monic polynomial $P$ of degree~$d \geq 3$ with simple roots $|\zeta_k| \leq 1$ and $R \geq 2\max_k|\zeta_k|$, the ratio $r_s = P(z_s)/P(a_s)$ satisfies $\operatorname{Re}(r_s) < 0$ for all $s \in \{0,\ldots,d-1\}$.
\end{conjecture}

\begin{remark}
Conjecture~\ref{conj:halfplane} is verified exhaustively for $30$ adversarial polynomial families (including Wilkinson, Mignotte, Chebyshev, clustered, and random polynomials) and all degrees $3 \leq d \leq 500$.  Theorem~\ref{thm:winding}(ii) establishes the structural factorisation $r_s = -1 \cdot (\text{correction})$ where the correction is a product of $d$ factors each in the disk $|z-1| \leq 1/2$.  The remaining analytical step is to prove that cancellation among these factors keeps the total correction phase below~$\pi/2$.
\end{remark}

\begin{theorem}[Polynomial root-finding complexity]
\label{thm:main}
Assume Conjecture~\ref{conj:halfplane}.  Given a monic polynomial $P$ of degree~$d$ with simple roots, the Pure Pandrosion-$T_3$ algorithm with $d$ equispaced starts on the Cauchy circle and iterated scaling finds an approximate zero in at most $O(d^3)$ arithmetic operations in the BSS model.
\end{theorem}

\begin{proof}
By Conjecture~\ref{conj:halfplane} and Theorem~\ref{thm:winding}(iii), there exists $s^*$ such that $\Lambda_{s^*} < 1$ at the first epoch.  Theorem~\ref{thm:universal_descent} gives $\Lambda \leq e^{-\pi/2}$ per epoch on the Cauchy circle, requiring $\lceil 2d\log R/\pi\rceil = O(d)$ epochs to reduce $|P|$ to $O(1)$.  By Theorem~\ref{thm:contraction_ratio}, once $|a-\zeta| = O(1)$, $\lambda < 1$ and convergence accelerates; convergence to an approximate zero follows in $O(\log\log\varepsilon^{-1})$ additional epochs.  Each epoch consists of~$3$ Pandrosion steps at $O(d)$ cost each.  Total per orbit: $O(d) \times O(d) = O(d^2)$.  With $d$ starting orbits: $O(d^3)$.
\end{proof}
...
\label{rem:roadmap}
The strategy for a full proof decomposes into three layers:
\begin{enumerate}
  ...
  \item \textbf{Sum-of-logs bound}: show that $\Sigma_{\log}(z_0,z) \leq -c'$ per epoch by exploiting the algebraic constraint $\sum 1/\omega_k = d - P'/Q$ (Theorem~\ref{thm:pandrosion_sum}) and the regularisation $|Q| \gg 0$ (Theorem~\ref{thm:pandrosion_regularisation}).  \emph{Status: verified numerically ($\Sigma_{\log} \approx -5$ with $T_3$) on all tested families up to $d = 500$; proof open.}
\end{enumerate}
\end{remark}
...
\textbf{What remains open.}  Theorems~\ref{thm:pandrosion_product}--\ref{thm:pandrosion_regularisation} provide an \emph{exact} product factorization and a \emph{bounded} divided difference, eliminating the conditional $\bar{\Sigma} < 2$ barrier that blocks Newton-based proofs.  The remaining gap is isolated by the \emph{Amortised Block Descent Conjecture}~\ref{conj:block_descent}: for the adaptive scheme with reanchoring period $K \leq K_0$, each $K$-epoch must reduce $|P|$ by a factor $e^{-c}$ with $c > 0$ universal.  The roadmap (Remark~\ref{rem:roadmap}) decomposes this into three layers:
\begin{enumerate}
...
\begin{itemize}
  \item \emph{Proving the sum-of-logs bound}: exploit the Jensen-type structure of $\Sigma_{\log}$ (Remark~\ref{rem:sum_of_logs}) and the algebraic constraint $\sum 1/\omega_k = d - P'/Q$ to establish that the geometric mean of $|Q_k/Q|$ is strictly below~$1$ in the adaptive regime.
  \item \emph{Non-autonomous Fatou--Julia theory}: develop a framework analogous to~\cite{milnor} for families of rational maps indexed by a slowly varying anchor, as a path toward formalising the scaling principle of Conjecture~\ref{conj:block_descent}.
