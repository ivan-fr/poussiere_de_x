...
\begin{remark}[Three-phase proof structure for the pure Pandrosion-$T_3$]
\label{rem:three_phase}
The iterated scaling cascade creates a three-phase convergence mechanism:
\begin{enumerate}
  \item \textbf{Phase 1: Radial approach.}  From the Cauchy circle, the radial contraction (Theorem~\ref{thm:radial_contraction}) brings the iterate within the root cluster in $O(d\log d)$ base-map steps.  The contraction ratio $\lambda(a,\zeta) \approx 1$ during this phase (Theorem~\ref{thm:contraction_ratio}(i)), giving slow but positive descent.
  \item \textbf{Phase 2: Basin entry (majority vote).}  Once $|a - \zeta| = O(\rho)$, the majority vote (Proposition~\ref{prop:majority_vote}) ensures $\Sigma_{\log} < 0$: the $d-1$ far roots contribute bounded negative terms, while the single near root contributes at most $O(1)$ positive.  The orbit enters the basin of attraction in $O(d)$ additional steps.
  \item \textbf{Phase 3: Scaling acceleration.}  Once $|a - \zeta| \ll 1$, the contraction ratio $\lambda(a,\zeta) \to 0$ (Theorem~\ref{thm:contraction_ratio}(ii)).  With Steffensen acceleration, $T_3$ yields cubic convergence: $O(\log\log\varepsilon^{-1})$ epochs to reach precision $\varepsilon$.
\end{enumerate}
\textbf{Total BSS cost:}  $O(d\log d)$ steps per starting point $\times$ $O(d)$ cost per polynomial evaluation $= O(d^2\log d)$ per orbit.  With $d$ starting points on the Cauchy circle, the total cost is $O(d^3\log d)$ --- \textbf{polynomial in $d$}.

The only remaining gap for a complete proof of Smale's 17th problem is the rigorous passage from Phase~1 to Phase~2: showing that the radial contraction theorem extends to anchors that have moved off the Cauchy circle (the current proof requires the anchor to be on $|z| = R$).  Numerically, this progression is universal and robust.
\end{remark}
