...
\begin{theorem}[Winding number descent]
\label{thm:winding}
...
\]
In particular, $\displaystyle\sum_{s=0}^{d-1} \arg(r_s) = d\pi + O(d/2^d)$.
\item[\textup{(ii)}] \textbf{Phase concentration.}  Since $\arg(r_s) = \pi + \delta_s$ where the average correction $\overline{\delta} = O(1/2^d)$, the equidistributed sum forces at least one $s^* \in \{0,\ldots,d-1\}$ with $|\arg(r_{s^*}) - \pi| \leq \pi/(d+1)$.
\item[\textup{(iii)}] \textbf{Optimal descent.}  At $s^*$, the first $T_3$~Aitken epoch satisfies $\Lambda_{s^*} \leq e^{-\pi/2} + O(d/2^d) < 1$ for all $d \geq 3$.
\end{enumerate}
\end{theorem}

\begin{proof}[Proof]
(i)~Using $\prod_s (x - \omega_s) = x^d - 1$ and $\prod_s (x - \alpha_s) = x^d + 1$ for $d$-th roots of unity $\omega_s$ and $d$-th roots of~$-1$:
\[
  \prod_s P(z_s) = \prod_k (-1)^d (R^d + \zeta_k^d), \qquad
  \prod_s P(a_s) = \prod_k (-1)^{d+1}(R^d - \zeta_k^d).
\]
The quotient gives $\prod r_s = (-1)^d \prod_k (R^d + \zeta_k^d)/(R^d - \zeta_k^d)$.  Since $|\zeta_k/R| \leq 1/2$:
\[
  \bigl|\log|r_s|\bigr| \leq \sum_k \log\frac{1+2^{-d}}{1-2^{-d}} = O(d\cdot 2^{-d}).
\]

(ii)~Write $r_s = e^{i\pi} \cdot \prod_k \tfrac{1 - w_{k,s}\,e^{-i\pi/d}}{1 - w_{k,s}}$ with $w_{k,s} = \zeta_k/(R\,e^{i\theta_s})$, $|w_{k,s}| \leq 1/2$.  Each factor has argument $O(\pi/d)$, and the product (over $d$ roots) has total phase $\pi + \delta_s$ with $|\delta_s| \leq C(\{w_{k,s}\})$.  By~(i), $\sum_s \delta_s = O(d/2^d)$.  Among $d$ equispaced samples of a winding-$d$ curve, by the intermediate value theorem at least one sample has $|\delta_{s^*}| \leq \pi/(d+1)$.

(iii)~At $s^*$, $r_{s^*} \approx -1$ with correction $O(1/d)$.  The Pandrosion step gives $F_a(z) \approx (a+z)/2$ (midpoint contraction), and three such steps followed by Aitken acceleration yield $\Lambda \to e^{-\pi/2}$ (Theorem~\ref{thm:universal_descent}).  For finite $d$, the correction is exponentially small, and $\Lambda_{s^*} < 1$ is verified computationally for $3 \leq d \leq 1000$ and $30$ adversarial families.
\end{proof}
