...
\begin{conjecture}[Amortised Contraction and Smale 17]
\label{conj:amortised_smale}
The Pandrosion operator $\mathcal{P}_{P,z_0}$ admits an algorithm achieving basin entry uniformly in $O(d \log d)$ steps, calculating a root with a total arithmetic complexity of $O(d^2 \log d)$, providing a derivative-free resolution to Smale's 17th problem.
\end{conjecture}

\begin{remark}[The missing link for the worst-case bound]
...
The total accumulated product $\prod \left| 1 - P(z_n)/P(z_0) \right|$ is therefore geometrically constrained by $\frac{|z_{\mathrm{init}} - z_0|}{|\zeta_k - z_0|}$, a finite, non-zero constant $C$. This conservation law forces the sum of the algebraic ratio $\sum |P(z_n)/P(z_0)|$ to be uniformly bounded, absorbing any transient local increases in $|P(z)|$. However, translating this bounded continuous log-increase into a strict worst-case bound of $O(d\log d)$ iterations prior to lock-in requires preventing adversarial sequences of geometrically small steps that never escape the locus $|P(z_n)| \approx |P(z_0)|$. While empirically we observe rapid exponential descent governed by the Pandrosion product identity (Theorem~\ref{thm:pandrosion_product}), the formal proof of this bounded step count remains open.
\end{remark}
...
Conjecture~\ref{conj:amortised_smale} is consistent with the empirical grid measurements and scaling experiments, which show $100\%$ convergence and $O(d^{0.8})$ active starts on random polynomials up to degree $200$.

\medskip
\textbf{The limits of the Lyapunov argument.}  A natural candidate for a descent strategy is the continuous formulation of Theorem~\ref{thm:global_conservation}. While the macroscopic distance ratio $|w_0|/|w_N|$ perfectly bounds the total relative product $\prod |1 - P(z_n)/P(z_0)|$, translating this exact limit into a strict worst-case bound on the \emph{number of steps} requires ruling out adversarial sequences of microscopic steps that cycle near the locus $|P(z)| \approx |P(z_0)|$. Extensive experiments ($170{,}000$ total steps) confirm that the descent is heavily amortised: local rebounds occur (in $\approx 0.6\%$ of steps), but are fully absorbed by the global geometric budget. A complete worst-case polynomial proof of Conjecture~\ref{conj:amortised_smale} requires explicitly lower-bounding the per-step progression using the kinematic identity, bridging the gap between total product conservation and discrete time.
\end{remark}
...
The multi-start Pandrosion--$T_3$ scheme framework introduces exact local algebraic theorems and macroscopic geometric bounds that strongly support a \textbf{derivative-free} polynomial algorithm, but the complete global worst-case proof remains to be established. If confirmed, ensuring continuous basin entry in $O(d\log d)$ base steps and combining with the $O(\log\log(1/\varepsilon))$ post-lock-in phase yields $O(d^2\log d)$ derivative-free BSS operations per starting point, or $O(d^3\log d)$ total.

The empirical average-case performance is substantially better ($O(d^2)$ total, see table below), and $T_3$ acceleration reduces the constant by a factor of $\approx 3$.
\begin{center}
\begin{tabular}{rrrr}
\toprule
