...
From Paper~I~\cite{besevic2026}, the iteration is dramatically accelerated by exploiting monotonicity through the \textbf{Geometric interpretation: Thales preconditioning}. This guarantees the \emph{optimal starting point} for the Pandrosion iteration:
\begin{equation}
  \label{eq:s0opt}
  s_0^{\mathrm{opt}} = h(1) = 1 - \frac{x-1}{x \cdot p}.
\end{equation}

This represents one evaluation of $h$ at the continuous reference voltage $V_{\text{ref}} = 1$~V. The residual is further bounded by applying \textbf{The scaling principle} ($x \to x/A$) as an analog pre-amplification stage: if the physical input $x$ is large, a simple analog attenuator $1/A$ scales the input into the optimal dynamic range $[1, 2]$~V before processing. 

For $p = 3$, an unscaled $x = 2$ gives $s_0^{\mathrm{opt}} = 5/6 \approx 0.833$, while $s^* = 1/\cbrt{2} \approx 0.794$.  The initial residual is therefore tightly constrained:
\[
...
\begin{remark}[Continuous input vs.\ discrete topology]
In this pipeline, the value $x \geq 1$ is simply the continuous \textbf{input voltage}. The chip requires no clock or parameter tuning to react to changing inputs: if $x(t)$ varies, the output $v(t) \approx \prt{p}{x(t)}$ tracks it continuously, delayed only by the propagation time ($\sim 120$~ns). Conversely, the root order $p$ is permanently encoded in the \textbf{hardware topology} (the physical wiring of the $S_p$ summation block). A single universal layout handles any $x$; changing $p$ requires activating different physical traces.
\end{remark}
