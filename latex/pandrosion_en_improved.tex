...
\begin{remark}
The contraction ratio $\lambda_{p,x}$ is entirely determined by $\alpha = x^{1/p}$: the target itself dictates the rate at which Pandrosion's method approaches it. We will occasionally refer to $\lambda_{p,x}$, viewed as a rational function of $\alpha$, as the \emph{convergence signature} of the method. This is purely terminological --- $\lambda_{p,x}$ is of course nothing more than $h'(s^*)$, the standard asymptotic error constant of a linearly convergent fixed-point iteration --- and we introduce the name only to make the dependence on the target (rather than on the starting point) verbally prominent.
\end{remark}
...
\begin{proof}
Write $\lambda_{p,\alpha^p} = (\alpha-1) N_p(\alpha)/D_p(\alpha)$, where $N_p(\alpha) = \sum_{k=1}^{p-1} k\,\alpha^{p-1-k}$ and $D_p(\alpha) = \sum_{k=0}^{p-1}\alpha^k$.

\emph{Part (1) --- monotonicity in $\alpha$ for fixed $p$.} We prove the case $p = 3$ explicitly and sketch the general case.

For $p = 3$: $\lambda = (\alpha-1)(\alpha+2)/(\alpha^2+\alpha+1)$, so
\[
  \frac{d}{d\alpha}\log\lambda
  = \frac{1}{\alpha - 1} + \frac{1}{\alpha + 2} - \frac{2\alpha + 1}{\alpha^2 + \alpha + 1}.
\]
Common-denominator algebra gives
\[
  \frac{d}{d\alpha}\log\lambda
  = \frac{2\alpha^3 + 3\alpha^2 - 2}{(\alpha-1)(\alpha+2)(\alpha^2+\alpha+1)},
\]
...
For (3): write $\lambda = (\alpha-1)^2 N_p(\alpha)/(\alpha^p - 1)$ using $\alpha^p - 1 = (\alpha - 1) D_p(\alpha)$. With $\alpha = x^{1/p}$, set $k = \lfloor tp \rfloor$ for $t \in [0,1]$ and note $\alpha^k = e^{(k/p)\ln x} \to x^t$; hence the Riemann-sum approximations
\[
  \tfrac{1}{p} D_p(\alpha) \longrightarrow \int_0^1 x^t \,dt
  = \frac{x-1}{\ln x},
  \qquad
  \tfrac{1}{p^2} N_p(\alpha) \longrightarrow \int_0^1 (1-t)\, x^t \,dt
  = \frac{(x-1) - \ln x}{(\ln x)^2}.
\]
Together with $\alpha - 1 \sim (\ln x)/p$, this gives
\[
  \lambda_{p,x}
  \;=\;
  (\alpha-1) \cdot \frac{N_p(\alpha)}{D_p(\alpha)}
  \;\longrightarrow\;
  \frac{\ln x}{p} \cdot p \cdot \frac{(x-1) - \ln x}{(\ln x)^2} \cdot \frac{\ln x}{x-1}
  \;=\;
  1 - \frac{\ln x}{x-1}.
\]
Numerically: for $x = 2$, $1 - \ln 2 \approx 0.3069$ and $\lambda_{200,2} \approx 0.3057$, $\lambda_{1000,2} \approx 0.3066$, $\lambda_{10000,2} \approx 0.3068$, confirming the limit.
\end{proof}
...
\begin{equation}\label{eq:lambda-alt}
  |\lambda_{p,x}| \;=\; \frac{(1-\alpha)^2 \, N_p(\alpha)}{1 - \alpha^p},
  \qquad N_p(\alpha) := \sum_{k=1}^{p-1} k\, \alpha^{p-1-k}.
\end{equation}
The right-hand side is a continuous function of $\alpha \in (0,1)$. As $\alpha \to 1^-$, it tends to $0$; as $\alpha \to 0^+$, $N_p(\alpha) \to p-1$ and $(1-\alpha)^2 / (1-\alpha^p) \to 1$, so $|\lambda_{p,x}| \to p - 1 \geq 1$. By continuity, there exists at least one $\alpha^\ast_p \in (0,1)$ with $|\lambda_{p,\alpha^{\ast p}_p}| = 1$; a direct inspection (done case by case below) shows monotonicity, so the threshold is unique.
\end{proof}
...
The central tension of this paper is between Pandrosion's ancient geometric construction — an order-one iteration requiring only a ruler and parallels — and modern acceleration theory. Once the universal iteration $s_{n+1} = 1 - (x-1)/(x\,S_p(s_n))$ is written down, three ingredients suffice to make it competitive: the closed-form contraction ratio $\lambda_{p,x}$ (Theorem~\ref{thm:lambda}), the preconditionings of Propositions~\ref{prop:optimal_start} and~\ref{prop:scaling}, and the Aitken $\Delta^2$ upgrade (Theorem~\ref{thm:steffensen}). Together they turn Pandrosion into a derivative-free, quadratically convergent scheme with a quadratic constant $K_S \ll K_{\text{Newton}}$ and an iteration count that, for large $x$, collapses from hundreds to single digits.

The closed-form signature $\lambda_{p,x}$ is the hinge: it is monotone in both $p$ and $x$ (Theorem~\ref{thm:monotonicity}), has an explicit large-$p$ limit $1 - \ln(x)/(x-1)$ (Proposition~\ref{prop:limits}), and admits a clean threshold analysis for $x < 1$ (Proposition~\ref{prop:x_small}). The rest of the paper is built on this object.

\medskip

\textbf{Perspectives.} Natural extensions are generalized nonlinear Pandrosion constructions (with other polynomials replacing $S_p$); higher-order acceleration variants; a deeper topological study of the divergence regime $x < x^\ast_p$ in~$\mathbb{C}$. None of these is attempted here.
