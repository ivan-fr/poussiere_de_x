...
\begin{enumerate}
  ...
  \[
    \lim_{p \to \infty} \lambda_{p,x} = L(x) < 1.
  \]
  For example, $L(2) \approx 0.3069$ (computed numerically).
\end{enumerate}
\end{proposition}
...
For (3): as $p \to \infty$ with $x$ fixed, $\alpha = x^{1/p} \to 1^+$ with $\alpha - 1 \sim (\ln x)/p$.  Monotonicity in $p$ (part~(2) of Theorem~\ref{thm:monotonicity}) and the bound $\lambda_{p,x} < 1$ guarantee the existence of $L(x) = \lim_{p \to \infty} \lambda_{p,x}$.  Numerically, for $x = 2$, computing $\lambda_{p,2}$ in $50$-digit precision shows convergence: $\lambda_{100,2} = 0.3044$, $\lambda_{1000,2} = 0.3066$, $\lambda_{10000,2} = 0.3068$, consistent with $L(2) \approx 0.3069$.  A naive expansion $\alpha^k \approx 1 + k(\ln x)/p$ suggests $L(x) = (\ln x)/(1 + \ln x)$, but this overestimates the limit (e.g., $\ln 2/(1+\ln 2) = 0.409$ vs.\ $L(2) \approx 0.307$).  The discrepancy arises because the higher-order terms in the expansion $\alpha^k = e^{k \ln x / p}$ contribute non-negligibly to the sums.  A closed-form expression for $L(x)$ remains an open question.
\end{proof}
