...
Rational iterations for $p$-th roots are a classical topic with a large literature: Newton's method applied to $f(u) = u^p - x$, Halley's method, and more generally Schröder's (1870) and König's (1884) families produce iterations of arbitrary order with explicit asymptotic constants, and Traub's \emph{Iterative Methods for the Solution of Equations} (1964) systematizes the analysis of fixed-point iterations and their acceleration. The derivative-free counterpart via Aitken--Steffensen (1926/1933) is also standard and appears in every textbook on numerical analysis~\cite{stoer,numrecipes}.
