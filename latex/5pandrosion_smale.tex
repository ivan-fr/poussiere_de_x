...
\title{Smale's Conjecture for Quartic Polynomials: \\ The Pandrosion Reduction to a Resultant Inequality}
\author{Ivan Besevic}
\date{April 2026}
...
\begin{proof}
From $3ac^2 = -4c^3 - 2bc - 1$: $ac^2 = (-4c^3 - 2bc - 1)/3$. Then:
\begin{align*}
\tilde{q}(c) &= ac^2 + 2bc + 3 = \frac{-4c^3 - 2bc - 1}{3} + 2bc + 3 = \frac{-4c^3 + 4bc + 8}{3}.
\end{align*}
Using $2bc = -4c^3 - 3ac^2 - 1$: $4bc = -8c^3 - 6ac^2 - 2$. So:
$\tilde{q}(c) = (-4c^3 + (-8c^3 - 6ac^2 - 2)/2 + 8)/3 = (-8c^3 - 3ac^2 + 7)/3$.

Alternatively, directly: $a + 2c = a + 2c$, $c^2(a + 2c) = ac^2 + 2c^3$,
$1 - c^2(a + 2c) = 1 - ac^2 - 2c^3$.
...
\end{align*}
From $4c^3 + 3ac^2 + 2bc + 1 = 0$: $bc = (-4c^3 - 3ac^2 - 1)/2$, giving $2 + bc - c^3 = (3(1 - 2c^3 - ac^2))/2$. Hence $1 - c^2(a + 2c) = (2/3) \cdot (3(1-2c^3-ac^2)/2) = 1 - 2c^3 - ac^2 = 1 - c^2(a+2c)$. The identity $\tilde{q}(c) = 2(1 - c^2(a + 2c))$ follows directly from the first computation.
\end{proof}
...
So $\sum w_k$ is stationary at the extremal. Since $\sum (1 - w_k) = 3 \cdot (3/2) = 9/2$ is maximal, perturbation cannot push all $|1 - w_k|$ above $3/2$ simultaneously.
\end{proof}
...
\bibitem{besevic49} I. Besevic, \textit{Smale's conjecture and the fiber vanishing identity}. Pandrosion Dynamics \textbf{49} (2026).
\end{thebibliography}
