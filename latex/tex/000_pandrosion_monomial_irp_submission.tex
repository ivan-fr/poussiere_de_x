...
\newcommand{\Sp}{S_p}
\newcommand{\hp}{h_{p,x}}
\newcommand{\irp}{\mathcal I_{p,q,K}}
...
Finally, iterated renormalized Pandrosion (IRP) turns the scaling rule into
a dynamic acceleration principle.  Each layer renormalizes the current
multiplicative residual, applies one raw Pandrosion readout in the normalized
cell, and returns to the root coordinate.  Locally, one such layer is
quadratic in the logarithmic root error, and repeated layers amplify the
order dyadically.  The complex monomial extension is kept at the level needed for an
...
charts, iterated renormalization, and complex roots.
\end{abstract}
...
construct the map and compute its residual multiplier.  Section~\ref{sec:irp}
turns the scaling rule into iterated renormalized Pandrosion.
Sections~\ref{sec:scaling}--\ref{subsec:chart-contraction-lemma}
...
convergent when it is applied to one fixed normalized target.  The preceding
multiplier formula suggests a different acceleration principle: do not try to
cancel derivatives of the fixed map.  Instead, renormalize the current
multiplicative residual so that every raw Pandrosion layer sees a target close
to $1$.

This gives \emph{iterated renormalized Pandrosion} (IRP).  In the normalized
root equation
\[
  u^p=y,
\]
let the current positive approximation be $u$ and define the multiplicative
residual
\[
  R_y(u)=\frac{y}{u^p}.
\]
A residual $R_y(u)>1$ says that $u$ is too small, while $R_y(u)<1$ says that
$u$ is too large.  For a local positive residual target $Y\ge1$, define the
one-step Pandrosion readout from the neutral point by
\begin{equation}
  A_p(Y)=Y\,h_{p,Y}(1)^{p-1}.
  \label{eq:Ap-one-layer}
\end{equation}
This is the visible root-coordinate factor obtained by one raw Pandrosion
construction applied to $v^p=Y$.

To make the layer global, choose a scale palette
\[
  \mathcal B_{q,K}=\{q^{m+j/K}:m\in\mathbb Z,\ j=0,\ldots,K-1\}.
\]
Given any $T\ge1$, let $B(T)\in\mathcal B_{q,K}$ be a closest admissible scale,
namely one for which
\[
  Y(T)=\frac{T}{B(T)^p}\ge1
\]
and $\log Y(T)$ is minimal among the admissible choices.  A single
renormalized Pandrosion layer is then
\begin{equation}
  \mathcal I_{p,q,K}(u;y)=
  \begin{cases}
    u\,B(R_y(u))\,A_p(Y(R_y(u))), & R_y(u)\ge1,\\[0.4em]
    \displaystyle \frac{u}{B(R_y(u)^{-1})\,A_p(Y(R_y(u)^{-1}))}, & 0<R_y(u)<1.
  \end{cases}
  \label{eq:irp-layer}
\end{equation}
Thus IRP is a repeated composition of the form
\[
  \underbrace{(\hbox{renormalize}\to\hbox{raw Pandrosion})\circ\cdots\circ
  (\hbox{renormalize}\to\hbox{raw Pandrosion})}_{m\ \hbox{layers}}.
\]
The scale choice in each layer is the same logarithmic chart rule as before,
but applied dynamically to the current residual rather than only once at the
beginning.

\begin{theorem}[Local order of one IRP layer]
\label{thm:irp-one-layer}
Let $u_*=y^{1/p}$ and write the logarithmic root error as
\[
  \ell=\log\frac{u}{u_*}.
\]
In a local cell where the palette scale is inactive, equivalently where the
renormalized residual is already close to $1$, one IRP layer satisfies
\begin{equation}
  \ell^+
  =-\operatorname{sgn}(\ell)\,\frac{(p-1)^2}{2}\,|\ell|^2
   +O(|\ell|^3).
  \label{eq:irp-one-layer-expansion}
\end{equation}
Consequently one layer is quadratically convergent in logarithmic root error.
\end{theorem}

\begin{proof}
Assume first that $\ell<0$, so $u<u_*$ and the residual is
$R_y(u)=e^{pt}$ with $t=-\ell>0$.  Put $\delta=pt$.  A single local layer uses
$Y=e^\delta$ and
\[
  h_{p,e^\delta}(1)=1-\frac{1-e^{-\delta}}{p}.
\]
The logarithmic error after multiplying by the raw readout is
\[
  \ell^+
  =-t+\log\Big(e^\delta h_{p,e^\delta}(1)^{p-1}\Big).
\]
Expanding at $\delta=0$ gives
\[
  \ell^+=\frac{(p-1)^2}{2}t^2+O(t^3).
\]
If $\ell>0$, the reciprocal branch is used and the same computation changes
only the sign.  This proves \eqref{eq:irp-one-layer-expansion}.
\end{proof}

\begin{corollary}[Dyadic order amplification]
\label{cor:dyadic-irp}
Fix $m\ge1$.  In a sufficiently small local cell, $m$ successive IRP layers
satisfy
\[
  |\ell^{(m)}|=O(|\ell|^{2^m}).
\]
Thus the order is amplified dyadically by repeated geometric renormalization:
one layer gives order $2$, two layers give order $4$, three layers give order
$8$, and so on.
\end{corollary}

\begin{proof}
Theorem~\ref{thm:irp-one-layer} gives $|\ell^+|\le C|\ell|^2$ in a small enough
cell.  Repeating this estimate gives
$|\ell^{(m)}|\le C_m |\ell|^{2^m}$.
\end{proof}

The role of the discrete palette is to make this local mechanism usable away
from the root.  If the palette width is small enough that
\[
  0\le \log Y(T)\le \frac{p\log q}{2K}
\]
lies inside a chosen local cell, then each raw layer is formed only after a
geometric rescaling has made the residual target close to $1$.  The higher
order is therefore not introduced by a Newton, Halley, or fixed-point extrapolation
correction.  It emerges from repeated geometric renormalization of the original
first-order Pandrosion construction.

\begin{takeaway}
IRP replaces derivative cancellation by dynamic chart normalization.  A raw
Pandrosion layer is still first-order as a fixed map, but after the residual is
renormalized its multiplier is proportional to the current logarithmic error.
Iterating this renormalized layer amplifies the order dyadically.
\end{takeaway}
...
start $s_0=h_{p,y}(1)$.  The column ``IRP$_2$'' counts two-layer IRP
macro-updates from the same visible canonical start
\[
  u_0=y\,h_{p,y}(1)^{p-1}.
\]
For this table IRP uses the fixed dynamic scale palette $q=1.25$, $K=4$; each
IRP$_2$ macro-update contains two renormalized raw Pandrosion layers.  The
column ``Newton'' counts Newton updates for $u^p=y$, started from the same
visible canonical value $u_0$.  Thus the table reports the same normalized
problems under the raw geometric iteration, the renormalized Pandrosion
cascade, and the standard numerical baseline.
...
\toprule
$p$ & $x$ & chart & $B$ & normalized $y$ & $\lambda_{p,y}$ & raw & IRP$_2$ & Newton & raw/IRP$_2$ \\
\midrule
$3$ & $2\cdot10^{-3}$ & recip. & $1$ & $500$ & $0.958294558$ & $659$ & $2$ & $13$ & $329.5$ \\
...
$5$ & $1/(3\cdot4^5)$ & recip. & $1$ & $3072$ & $0.993515266$ & $4295$ & $2$ & $29$ & $2147.5$ \\
$5$ & $1/(3\cdot4^5)$ & recip.+scale & $4$ & $3$ & $0.385672651$ & $28$ & $3$ & $6$ & $9.3$ \\
$5$ & $5\cdot10^{-6}$ & recip. & $1$ & $200000$ & $0.999737824$ & $106226$ & $2$ & $44$ & $53113$ \\
$5$ & $5\cdot10^{-6}$ & recip.+scale & $10$ & $2$ & $0.256508225$ & $19$ & $2$ & $5$ & $9.5$ \\
\bottomrule
\end{tabular}%
}
\end{center}
...
\begin{figure}[H]
  ...
  \caption{Iteration histories on the normalized problem $u^3=4$.  Raw Pandrosion is linear, Newton is quadratic, and two-layer IRP reaches the plotted precision floor in a few renormalized macro-updates.}
...
geometric map changes from nearly stalled to short and renormalization-friendly
once the reciprocal homothety makes $|\log y|$ small.  IRP pushes this principle
one step further: the residual is re-normalized again before each raw layer.
...
applied to $u^p-x$.  IRP does not cancel this derivative by a rational
correction.  It changes the normalized residual before each raw layer is
formed, so that the multiplier of the next layer is proportional to the current
logarithmic error.  This is the precise, limited novelty asserted here: a
...
convergent.  A two-layer IRP update has local order four in the logarithmic
root error, but it uses two raw Pandrosion readouts together with two scale
selections from the chosen palette.  Each raw readout requires the finite
geometric sum $S_p(s)$.  Evaluated by Horner's rule, this sum costs $O(p)$
scalar operations and one division in the formula for $h$.
...
IRP$_2$ & order $4$ locally & $2$ evaluations of $S_p$ + $2$ scale selections & renormalized acceleration \\
...
after one such block.  A two-layer IRP update has a higher local order in a
small normalized cell, but it pays for this by two evaluations of $S_p$ and by
palette selection.  The resulting efficiency depends on how expensive scale
selection is in the chosen implementation and how often the problem is already
in a local cell.  Halley is stronger still when its rational formula is
...
ground for geometric renormalization because all constants are explicit.
...
substantially under chart scaling and iterated renormalization.
...
\begin{figure}[H]
  ...
  \caption{Cyclotomic anchors for the complex monomial problem.  The poles are the non-trivial roots of unity; small disks around anchors provide local seeding regions for the branch-local IRP map.}
  \label{fig:cyclotomic-anchors}
\end{figure}

The complex dynamics has non-trivial basin structure, as is typical for rational
iteration in one complex variable~\cite{milnor}.  The real IRP rule above uses
an order relation on positive residuals, so it is not promoted here as a global
complex-plane algorithm.  The complex statement needed for the monomial account
is more modest: near a chosen cyclotomic anchor one may choose a local logarithm
branch and apply the same renormalized residual idea inside that branch.
...
the target anchor.  For $\eps$ small enough, the seeded point lies in a disk
where the poles of $h_{p,x}$ are avoided and a branch of the logarithmic error
coordinate can be fixed.

\begin{proposition}[Branch-local IRP near an anchor]
\label{prop:complex-branch-irp}
Let $x\in\C\setminus\{0,1\}$ and let $\gamma$ be one of the anchors
\eqref{eq:anchors}.  There is a disk $D(\gamma,r_\gamma)$ avoiding the poles of
$h_{p,x}$ and carrying a branch of $\eta=\log(s/\gamma)$ such that the local
one-layer IRP map is analytic in $\eta$ and satisfies
\[
  \eta^+=O(\eta^2).
\]
Consequently any fixed number $m$ of branch-local IRP layers satisfies
$\eta^{(m)}=O(\eta^{2^m})$ in that local branch.
\end{proposition}

\begin{proof}
The poles of $h_{p,x}$ are the zeros of $S_p$, namely the non-trivial
$p$-th roots of unity.  Since $x\ne1$, the chosen anchor $\gamma$ is not one of
these poles.  Choose a disk around $\gamma$ whose closure avoids the poles and
on which $s/\gamma$ has a single-valued logarithm.  In that coordinate the
residual target is an analytic function of $\eta$ and equals $1$ at
$\eta=0$.  The same Taylor expansion used in
Theorem~\ref{thm:irp-one-layer} gives a vanishing linear term for the
renormalized layer, hence $\eta^+=O(\eta^2)$.  Iteration gives the dyadic
estimate exactly as in Corollary~\ref{cor:dyadic-irp}.
\end{proof}
...
simplest selector is the nearest-anchor rule: assign $z$ to the anchor
...
branch-local IRP or raw Pandrosion updates depends continuously on the input
inside the cell.
\end{proposition}
...
\begin{enumerate}
  ...
  $T'(x^{1/p})=\lambda_{p,x}\ne0$ for $x>1$;
  \item IRP preserves the raw derivative-free Pandrosion layer while making
  higher-order convergence emerge from repeated geometric renormalization, and
  the complex monomial picture is handled locally by cyclotomic anchors and
  finite anchor selection.
\end{enumerate}
