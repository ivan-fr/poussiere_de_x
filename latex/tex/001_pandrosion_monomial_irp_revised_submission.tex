...
\hypersetup{
  ...
  pdftitle={A Monomial Pandrosion Scheme: Contraction, Chart Scaling, and Iterated Renormalization},
...
\large Contraction, Chart Scaling, and Iterated Renormalization for $z^p=x$}
...
The point is different: a modern algebraic reading of the proportional-mean
geometry discussed in the sources surrounding Pappus and Pandrosion leads to a
...
For $x>1$, the positive fixed point is also globally attractive on
$(0,\infty)$: every positive orbit converges to it monotonically.
...
worst-case supremum $1-p/S_p(q)$, whereas allowing the reciprocal chart
...
Finally, the same logarithmic principle leads to an intrinsic acceleration mechanism called Iterated Renormalized Pandrosion (IRP).  Instead of applying an external Aitken--Steffensen or Halley correction, IRP repeatedly recomputes the residual chart, snaps it to a small logarithmic scale cell, applies one raw Pandrosion layer, and returns to the root coordinate.  In the local cell containing the exact residual, one renormalized raw layer is quadratic in the logarithmic root error, and iterating the layer gives exact dyadic order amplification: $k$ renormalized layers have local order $2^k$.  The result is a deliberately narrow but coherent real-positive account of the Pandrosion monomial scheme: fixed point, global convergence, contraction, scaling, reciprocal charts, and iterated renormalization.
\end{abstract}
...
modern algebraic normalization of a proportional-means construction gives a fixed-point map whose
...
The historical claim is intentionally limited.  Book III of Pappus' collection
addresses Pandrosion and discusses a flawed planar treatment of two mean
proportionals; the attribution of a specific recursive algorithm is indirect.
Pendlebury already recast the construction criticized by Pappus as an iterative
approximation in 1873, and modern historical studies discuss both iterative
readings and rescaling~\cite{pendlebury1873,knorr1989,cuomo2000,steihaug2019}.
The contribution claimed here is therefore not the discovery of iteration or
scaling in this historical material.  It is the closed monomial multiplier,
the sharp logarithmic palette bounds, and the residual-renormalization mechanism
developed from them.
...
preconditioning principle, and the scale-lattice results turn that principle
into a sharp finite-palette bound.  Section~\ref{sec:irp} then promotes the
same logarithmic chart rule from a one-time preconditioner into iterated
renormalized Pandrosion.  Finally, Section~\ref{sec:comparison} compares the
method with standard iterations and states the intended numerical scope.
...
The motivating geometry begins with the ancient problem of doubling the cube:
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.68\linewidth]{../figures/001/fig1_pandrosion_construction.pdf}
...
through Pappus; modern reconstructions differ in their interpretation of the
passage~\cite{pappus,knorr1989,cuomo2000}.  Recursive algebraic readings also
predate the present work~\cite{pendlebury1873,steihaug2019}.  Accordingly, no
claim below depends on assigning the displayed recurrence verbatim to
Pandrosion.  The mathematical results begin with the explicit map
\eqref{eq:cube-s} and its monomial generalization.
\end{remark}

The proportional-means diagram suggests repeated improvement rather than an
exact planar construction in finitely many steps.  The modern viewpoint is to
write that repeated improvement as an iteration and to study its residual
profile.

For the cube-root normalization used in this paper, write the recursive model with an internal
...
that is read from the diagram.  Equation~\eqref{eq:classical-u} is the chosen
algebraic model of the repeated proportional step; the historical sources do
not present it in this notation.
...
For $p>3$ this is a modern monomial extension of the proportional-means pattern,
not a historical attribution.  A chain
$1:m_1=m_1:m_2=\cdots=m_{p-1}:x$ records successive powers of one common ratio;
after normalization, the finite sum of those powers is $S_p$.
...
\begin{theorem}[Global monotone convergence on the positive line]
\label{thm:global-positive-convergence}
Let $x>1$, $p\ge2$, and $s_0>0$.  Then the orbit
$s_{n+1}=h_{p,x}(s_n)$ remains positive and converges monotonically to the
unique positive fixed point $s_*=x^{-1/p}$.  It increases when $s_0<s_*$ and
decreases when $s_0>s_*$.
\end{theorem}

\begin{proof}
Because $S_p(s)\ge1$ for $s>0$,
\[
  h_{p,x}(s)
  =1-\frac{x-1}{xS_p(s)}
  \ge \frac1x>0.
\]
Moreover $h_{p,x}$ is strictly increasing on $(0,\infty)$, and the fixed-point
calculation in Theorem~\ref{thm:fixed-point} shows that $s_*$ is its only
positive fixed point.  The identity
\begin{equation}
  h_{p,x}(s)-s=\frac{1-xs^p}{xS_p(s)}
  \label{eq:global-sign}
\end{equation}
determines the direction of motion.  If $0<s<s_*$, then
$s<h_{p,x}(s)<h_{p,x}(s_*)=s_*$; if $s>s_*$, then
$s_*=h_{p,x}(s_*)<h_{p,x}(s)<s$.  Thus the orbit is monotone and bounded in
either case.  Its limit is positive and, by continuity, must be the unique
positive fixed point $s_*$.
\end{proof}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.66\linewidth]{../figures/001/fig5_global_convergence.pdf}
  \caption{The sign identity~\eqref{eq:global-sign} yields global monotone
  convergence.  The cobwebs approach $s_*$ from both sides for the example
  $p=3$, $x=8$.}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.86\linewidth]{../figures/001/fig2_lambda_profile.pdf}
...
\begin{equation}
  s_0^{\mathrm{can}}=\hp(1)=1-\frac{x-1}{xp}.
  \label{eq:startcan}
\end{equation}
...
The start $s_0^{\mathrm{can}}=\hp(1)$ is the canonical one-step start produced
by the same Pandrosion geometry.  Near $x=1$ it satisfies
\[
  s_0^{\mathrm{can}}=x^{-1/p}+O((x-1)^2).
\]
\end{proposition}
...
\begin{takeaway}
Canonical initialization is not an extra algorithmic layer.  It is the first
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.82\linewidth]{../figures/001/fig6_riemann_inversion.pdf}
...
of $x$ for fixed $p$ and $q$.  For every
$\widetilde\Lambda>\Lambda_{p,q}^{(2)}$, continuity of the derivative gives a
sufficiently small neighborhood of the fixed point on which the contraction
factor is at most $\widetilde\Lambda$.  Once an orbit is in that neighborhood,
reducing an internal error by a factor $R>1$ requires at most
\[
  \left\lceil \frac{\log R}{-\log \widetilde\Lambda}\right\rceil
\]
raw updates.  The asymptotic multiplier bound itself is tight for the chosen
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.92\linewidth]{../figures/001/fig3_scale_lattice.pdf}
...
%=====================================================================
\section{Iterated Renormalized Pandrosion}
\label{sec:irp}
%=====================================================================

The preceding chart-scaling results show that the raw Pandrosion multiplier
is small when the normalized target is close to $1$.  IRP turns this
one-time preconditioning rule into a dynamic acceleration principle: after each
raw layer, recompute the residual chart before applying the next one.  Near a
normalized target $y=1$, Proposition~\ref{prop:lambda-monotone} and
Lemma~\ref{lem:local-chart-contraction} give
\[
  \lambda_{p,e^\delta}=\frac{p-1}{2p}\delta+O_p(\delta^2).
\]
Thus, if a chart change keeps the residual target in a small logarithmic cell,
one raw Pandrosion layer is almost nilpotent.  Iterated Renormalized Pandrosion
(IRP) makes this observation into a method: recompute the residual chart before
each raw layer, then apply the same Pandrosion map again.

Consider the normalized root problem
\[
  u^p=y_0,
  \qquad y_0>0,
\]
and let $u>0$ be the current root-coordinate approximation.  Define the
multiplicative residual
\[
  R(u)=\frac{y_0}{u^p}.
\]
If $R(u)\ge1$, the residual equation is $w^p=R(u)$ and the return is
$u\mapsto u w$.  If $R(u)<1$, the reciprocal residual equation is
$w^p=1/R(u)$ and the return is $u\mapsto u/w$.

Let a root-scale palette be
\[
  \mathcal B_{q,K}=\{q^{m+j/K}:m\in\mathbb Z,
  \ j=0,\ldots,K-1\},\qquad q>1.
\]
For $A\ge1$, choose $B=B(A)\in\mathcal B_{q,K}$ so that
\[
  Y(A)=\frac{A}{B(A)^p}\ge1
\]
and $\log Y(A)$ is as small as the admissibility constraint allows.  One
renormalized raw layer is then
\[
  W(A)=B(A)\,Y(A)\,h_{p,Y(A)}(1)^{p-1}.
\]
The IRP layer is
\begin{equation}
  \IRP_{p,y_0}(u)=
  \begin{cases}
    u\,W(R(u)), & R(u)\ge1,\\[0.35em]
    u/W(1/R(u)), & R(u)<1.
  \end{cases}
  \label{eq:irp-layer}
\end{equation}
A $k$-layer IRP macro-update is the $k$-fold composition of
\eqref{eq:irp-layer}, recomputing the residual and the scale after every raw
layer.

\begin{lemma}[One renormalized raw layer is locally quadratic]
\label{lem:irp-one-layer}
In the local palette cell where the selected scale is $B=1$, let
$r=y_0^{1/p}$ and write the logarithmic root error as
\[
  \ell=\log\frac{u}{r}.
\]
Then one IRP layer satisfies
\begin{equation}
  \ell_+
  =-\operatorname{sgn}(\ell)\,\frac{(p-1)^2}{2}\,\ell^2
  +O_p(|\ell|^3)
  \qquad (\ell\to0).
  \label{eq:irp-quadratic-layer}
\end{equation}
\end{lemma}

\begin{proof}
It is enough to treat the case $\ell<0$; the reciprocal case changes the sign
by symmetry.  Put $a=-\ell>0$.  Then the residual target is
$R=e^{pa}$.  A single raw Pandrosion layer from the neutral point $s=1$ gives
\[
  s_1=h_{p,e^{pa}}(1)=1-\frac{1-e^{-pa}}{p},
  \qquad
  w_1=e^{pa}s_1^{p-1}.
\]
The exact residual root is $e^a$.  Hence
\[
  \log\frac{w_1}{e^a}
  =pa+(p-1)\log\left(1-\frac{1-e^{-pa}}{p}\right)-a.
\]
Expanding at $a=0$ gives
\[
  \log\frac{w_1}{e^a}
  =\frac{(p-1)^2}{2}a^2+O_p(a^3).
\]
Since the new error is $\ell_+=\ell+\log w_1=a(-1)+a+
\log(w_1/e^a)$, the displayed formula follows for $\ell<0$.  The case
$\ell>0$ uses the reciprocal return $u\mapsto u/w_1$ and gives the opposite
sign.
\end{proof}

\begin{theorem}[Dyadic order amplification]
\label{thm:irp-dyadic}
Let $\Phi_p$ denote one local IRP layer in the setting of
Lemma~\ref{lem:irp-one-layer}.  For every fixed $k\ge1$, the $k$-fold local
composition satisfies
\begin{equation}
  \Phi_p^{\circ k}(\ell)
  =(-1)^k\operatorname{sgn}(\ell)\,
    c_p^{\,2^k-1}|\ell|^{2^k}
    +O_p\bigl(|\ell|^{2^k+1}\bigr),
  \qquad c_p=\frac{(p-1)^2}{2}.
  \label{eq:irp-exact-order}
\end{equation}
Thus one renormalized raw layer is quadratic, two layers are quartic, three
layers have order eight, and so on.
\end{theorem}

\begin{proof}
Lemma~\ref{lem:irp-one-layer} is the case $k=1$.  Suppose
\eqref{eq:irp-exact-order} holds for $k$, and put $m=2^k$.  Its leading
coefficient is non-zero, so the sign of $\Phi_p^{\circ k}(\ell)$ is
$(-1)^k\operatorname{sgn}(\ell)$ for sufficiently small non-zero $\ell$.
Applying the one-layer expansion once more squares the magnitude and reverses
the sign.  The leading coefficient becomes
\[
  c_p\bigl(c_p^{2^k-1}\bigr)^2=c_p^{2^{k+1}-1},
\]
while squaring an $O(|\ell|^{m+1})$ remainder produces an
$O(|\ell|^{2m+1})$ remainder; the cubic remainder of the outer layer is
$O(|\ell|^{3m})=O(|\ell|^{2m+1})$.  This proves the formula for $k+1$.
Since $c_p>0$, the first non-zero term has degree exactly $2^k$.
\end{proof}

\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.72\linewidth]{../figures/001/fig7_irp_order.pdf}
  \caption{The exact local IRP layer for $p=3$, evaluated without an
  asymptotic surrogate.  One, two, and three layers exhibit slopes $2$, $4$,
  and $8$ on logarithmic axes, in agreement with
  Theorem~\ref{thm:irp-dyadic}.}
  \label{fig:irp-order}
\end{figure}

The finite-palette version is the global normalization layer.  It does not
assert that a coarse palette magically gives a global high-order method for all
parameters.  It says that, for a chosen palette, every raw layer is evaluated on
an admissible target $Y(A)$ whose logarithmic distance from $1$ is bounded by
the palette width.  The local dyadic order appears once the orbit has entered a
cell where the selected scale is stable.  This is the same logarithmic geometry
as in Theorem~\ref{thm:minimax-palette}, but used dynamically after each raw
layer rather than only once at initialization.

\begin{takeaway}
IRP is not an Aitken--Steffensen or Halley correction.  It uses no derivative
cancellation and no extrapolation quotient.  Higher local order comes from the
cascade
\[
  \text{renormalize}\ \longrightarrow\ \text{raw Pandrosion}
  \longrightarrow\ \text{renormalize}\ \longrightarrow\ \text{raw Pandrosion}
  \longrightarrow\cdots,
\]
which repeatedly moves the residual geometry back near the target $1$ before
asking the raw map to act.
\end{takeaway}
...
start $s_0=h_{p,y}(1)$.  The column ``IRP-2'' counts macro-updates of two
renormalized raw layers, starting from $u_0=1$ and using the dynamic residual
palette $\mathcal B_{1.25,4}$.  Each IRP-2 macro-update recomputes the residual
scale between the two raw layers.  The column ``Newton'' counts Newton updates
for $u^p=y$, started from the same visible canonical value
\[
  u_0=y\,h_{p,y}(1)^{p-1}.
\]
Thus the table reports the raw Pandrosion phenomenon, the IRP renormalization
variant, and the standard Newton baseline on the same list of targets.
The figure-generation and iteration code used for this revision is included in
the companion repository as
\texttt{scripts/001\_generate\_pandrosion\_figures.py}; the tabulated stopping
counts were independently recomputed at 100 decimal digits.
...
\toprule
$p$ & $x$ & chart & $B$ & normalized $y$ & $\lambda_{p,y}$ & raw & IRP-2 & Newton & raw/IRP \\
\midrule
$3$ & $2\cdot10^{-3}$ & recip. & $1$ & $500$ & $0.958294558$ & $659$ & $2$ & $13$ & $330$ \\
$3$ & $2\cdot10^{-3}$ & recip.+scale & $5$ & $4$ & $0.412598948$ & $30$ & $2$ & $5$ & $15$ \\
$3$ & $2\cdot10^{-6}$ & recip. & $1$ & $500000$ & $0.999529779$ & $59506$ & $2$ & $24$ & $29753$ \\
$3$ & $2\cdot10^{-6}$ & recip.+scale & $50$ & $4$ & $0.412598948$ & $30$ & $2$ & $5$ & $15$ \\
$5$ & $1/(3\cdot4^5)$ & recip. & $1$ & $3072$ & $0.993515266$ & $4295$ & $3$ & $29$ & $1432$ \\
$5$ & $1/(3\cdot4^5)$ & recip.+scale & $4$ & $3$ & $0.385672651$ & $28$ & $3$ & $6$ & $9.3$ \\
$5$ & $5\cdot10^{-6}$ & recip. & $1$ & $200000$ & $0.999737824$ & $106226$ & $3$ & $44$ & $35409$ \\
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.86\linewidth]{../figures/001/fig4_convergence_comparison.pdf}
  \caption{Iteration histories on the normalized problem $u^3=4$.  Raw Pandrosion is linear, IRP-2 collapses the residual by repeated renormalized raw layers, and Newton provides the standard quadratic baseline; the accelerated curves are truncated once they reach the plotted precision floor.}
...
geometric map changes from nearly stalled to short once reciprocal homothety and IRP residual renormalization make $|\log y|$ small before each raw layer.
...
return map transports the result back to the original variable.  IRP repeats
this idea dynamically: each layer renormalizes the current residual before the
next raw Pandrosion readout is formed.
\end{takeaway}
...
applied to $u^p-x$.  IRP does not cancel this linear term by an Aitken quotient or by a derivative-based
König formula.  Instead it changes the target seen by the raw map before each
layer, so that the multiplier itself becomes proportional to the current
logarithmic residual.  This is the precise, limited novelty asserted here: a
proportional-geometry map with its own residual multiplier, plus dynamic chart
scaling that controls that multiplier.

Pandrosion is therefore not advertised here as a faster replacement for Newton
on ordinary monomial floating-point computation.  Its raw form is linearly convergent.  Its IRP-2 form uses two raw Pandrosion
layers per macro-update, but recomputes the residual chart and scale between
the two layers.  Each layer requires one evaluation of the same fixed-point map
$h$, hence one finite geometric sum $S_p(s)$.  Evaluated by
...
IRP-2 & quartic local model & $2$ raw layers + residual scale selection & renormalized geometric acceleration \\
...
after one such block, whereas an IRP-2 macro-update uses two raw layers and,
in the local continuous model, has quartic order because the two renormalized
layers square the logarithmic error twice.  This does not make IRP-2
automatically faster than Newton in ordinary floating-point arithmetic: scale
selection, logarithms, and finite-palette bookkeeping also have a cost.  The
crude scalar model prevents the iteration counts from being read as a wall-clock
speed claim.
...
on the Riemann sphere can normalize small targets.  It is also a clean testing ground for derivative-free renormalization because all
...
substantially under chart scaling and iterated residual renormalization.
...
geometry admits a complete residual account organized by seven facts:
\begin{enumerate}
  ...
  $1-p/S_p(\alpha)$;
  \item on the positive real line, every orbit for $x>1$ converges monotonically
  to the unique positive fixed point;
  ...
  $T'(x^{1/p})=\lambda_{p,x}\ne0$ for $x>1$;
  \item IRP preserves the derivative-free raw map but amplifies local order
  exactly through repeated residual renormalization.
\end{enumerate}
...
The present scope is deliberately restricted to positive real targets.  A
complex extension would require a branch and scale selector that can be computed
without already knowing $x^{1/p}$; exact cyclotomic fixed points are useful for
local dynamics but are circular as seeds for a root-finding algorithm.  That
problem is left open here.  The elementary algebraic identities used in the
real-positive account are suitable for a small formal-audit companion, but no
independent proof-assistant artifact is claimed in this manuscript.

\section*{Acknowledgments}

\textbf{Generative AI disclosure.}
OpenAI Codex (GPT-5, accessed July 2026) was used during manuscript preparation
for LaTeX editing, figure regeneration, consistency checks, and editorial
revision.  The author reviewed the AI-assisted output and remains responsible
for the accuracy, originality, integrity, and references of the manuscript.
...
\bibitem{pendlebury1873}
R. Pendlebury,
``On a method of finding two mean proportionals,''
\textit{Messenger of Mathematics}, series 2, \textbf{2} (1873), 166--169.

\bibitem{knorr1989}
W. R. Knorr,
``Pappus' texts on cube duplication,'' in
\textit{Textual Studies in Ancient and Medieval Geometry},
Birkh\"auser, Boston, 1989, pp.~63--76.

\bibitem{cuomo2000}
S. Cuomo,
\textit{Pappus of Alexandria and the Mathematics of Late Antiquity},
Cambridge University Press, Cambridge, 2000, Sec.~4.1.

\bibitem{steihaug2019}
T. Steihaug and D. G. Rogers,
``Approximating cube roots of integers, after Heron's \textit{Metrica} III.20,''
\textit{CoRR} abs/1905.03547 (2019), arXiv:1905.03547.
