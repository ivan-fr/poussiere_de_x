...
Finally, the same logarithmic principle leads to an intrinsic acceleration mechanism called Iterated Renormalized Pandrosion (IRP).  Instead of applying an external Aitken--Steffensen or Halley correction, IRP repeatedly recomputes the residual chart, snaps it to a small logarithmic scale cell, applies one raw Pandrosion layer, and returns to the root coordinate.  In the local continuous model one renormalized raw layer is quadratic in the logarithmic root error, and iterating the layer gives dyadic order amplification: $k$ renormalized layers have local order $2^k$.  The complex monomial extension is kept at the level needed for an expository account: cyclotomic anchors, local seeding regions, and a finite selector.  The result is a deliberately narrow but coherent account of the Pandrosion monomial scheme: fixed point, contraction, scaling, reciprocal charts, iterated renormalization, and complex roots.
\end{abstract}
...
renormalized Pandrosion.  Finally, Sections~\ref{sec:comparison}--\ref{sec:selector-regularity}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.86\linewidth]{figures/fig1_pandrosion_construction.png}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.86\linewidth]{figures/fig2_lambda_profile.png}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.78\linewidth]{figures/fig6_riemann_inversion.png}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/fig3_scale_lattice.png}
...
In the local continuous cell where the selected scale is $B=1$, let
...
\[
  \Phi_p^{\circ k}(\ell)=O_p\bigl(|\ell|^{2^k}\bigr)
  \qquad (\ell\to0).
\]
...
\begin{proof}
Lemma~\ref{lem:irp-one-layer} gives $|\Phi_p(\ell)|\le C|\ell|^2$ for
$\ell$ in a sufficiently small neighborhood of $0$.  Iterating this estimate
gives
\[
  |\Phi_p^{\circ k}(\ell)|
  \le C_k |\ell|^{2^k}
\]
for some constant $C_k$, which is the asserted local order.
\end{proof}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.86\linewidth]{figures/fig4_convergence_comparison.png}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.58\linewidth]{figures/fig5_cyclotomic_anchors.png}
...
the target anchor.  For $\eps$ small enough, the seeded point lies in a local branch where the rational map and any fixed finite IRP layer are analytic away from their finite pole set.


\begin{proposition}[Effective seeded local branch]
\label{prop:effective-seeded-complex}
Let $x\in\C\setminus\{0,1\}$ and let $\gamma$ be one of the anchors~\eqref{eq:anchors}.
Then there exists an effectively computable radius $r_\gamma>0$ such that
$h_{p,x}$ is analytic on $D(\gamma,r_\gamma)$ and the disk avoids the poles of
$h_{p,x}$.  If, in addition, $|h'_{p,x}(\gamma)|<1$, then $r_\gamma$ may be
chosen so that raw Pandrosion is a contraction on that disk.  Every seeded
point $\operatorname{seed}_{\gamma,\eps}(z)$ lying in the disk then converges
linearly to $\gamma$ under raw Pandrosion.
\end{proposition}

\begin{proof}
The poles of $h_{p,x}$ are the zeros of $S_p$, namely the non-trivial $p$-th
roots of unity.  Since $x\ne1$, the anchor $\gamma$ is not one of these poles.
Put
\[
  P_p=\{\omega:\omega^p=1,\ \omega\ne1\},
  \qquad d_0=\dist(\gamma,P_p)>0.
\]
Taking $r_\gamma<d_0/2$ gives a pole-free disk.  If
$|h'_{p,x}(\gamma)|<1$, continuity of $h'_{p,x}$ gives a smaller disk on which
$|h'_{p,x}(z)|\le\Lambda<1$.  The mean-value estimate on the convex disk gives
$|h_{p,x}(z)-\gamma|\le\Lambda|z-
\gamma|$, and the contraction claim follows.  The radius and the bound are
effective because $h_{p,x}$ is an explicit rational function.
\end{proof}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.58\linewidth]{figures/fig7_anchor_voronoi.png}
...
\]
is locally $\eps$-Lipschitz on each such cell, and every fixed finite number of raw or IRP layers depends continuously on the input inside the cell, as long as the chosen branch avoids poles and selector boundaries.
\end{proposition}
...
\begin{enumerate}
  ...
  $T'(x^{1/p})=\lambda_{p,x}\ne0$ for $x>1$;
  \item IRP preserves the derivative-free raw map but amplifies local order through repeated residual renormalization, and the complex monomial picture is handled by cyclotomic anchors and finite anchor selection.
\end{enumerate}
