...
\begin{conjecture}[Amortised block descent]\label{conj:abd}
There exist universal constants $K_0 = O(1)$ and $c > 0$ such that the following holds. For any normalised polynomial $P \in \mathbb{C}[z]$ of degree $d$ with simple roots and any reanchoring period $K \le K_0$, the adaptive Pandrosion-$T_3$ scheme satisfies: for any block of $K$ consecutive steps within a single epoch,
\begin{equation}
|P(z_K)| \le e^{-c} |P(z_0)|,
\end{equation}
...
\begin{theorem}[Conditional resolution of Smale's 17th problem]\label{thm:smale17-cond}
If Conjecture~\ref{conj:abd} holds, the multi-start Pandrosion-$T_3$ scheme finds a root of any degree-$d$ polynomial $P$ with a deterministic worst-case BSS cost of
\begin{equation}
