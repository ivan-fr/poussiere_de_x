...
\title{A Pandrosion Operator for Derivative-Free Polynomial Root-Finding \\ \large Universal polynomial evaluation, non-local geometry, and an algorithmic companion to Smale's 17th problem}
\author{Ivan Besevic}
\date{April 2026}
...
We identify a \emph{far-anchor obstruction} for fixed-anchor Cauchy-circle starts and resolve it via the \textbf{scaling principle}: adaptive anchor reanchoring every $K = O(1)$ steps keeps the effective argument $\|\mathbf{Q}_F^{-1} F\| \approx 1$.

The resulting adaptive Pandrosion-$T_3$ scheme is verified numerically on four adversarial univariate families up to degree $d = 500$, and on multivariate Smale-17-style polynomial systems with B\'ezout degree $d^n$ up to $(n, d) \in \{(2, 3), (3, 3), (4, 2)\}$ (cf.\ \verb|latex/scripts/benchmark_smale17_v4.py|).

Conditionally on Conjecture~\ref{conj:abd}, this yields a deterministic, derivative-free $O(d^2 \log d)$-operation BSS algorithm for univariate root-finding; the multivariate Smale-17 average-case $O(\mathrm{poly}(d, n))$ bound is established unconditionally only by Beltr\'an--Pardo~\cite{beltranpardo} and Lairez~\cite{lairez}, and our scheme provides a derivative-free \emph{algorithmic alternative} with comparable empirical complexity, not a competing complexity-theoretic resolution. We close with a formal Pandrosion-language rewriting of the Euler product for $\zeta(s)$.
\end{abstract}

\paragraph{Reader's guide.}
This is the first companion paper after the verified monomial paper. It starts
the \emph{algorithmic} branch of the corpus: the difference-quotient operator,
adaptive anchors, and multivariate lift introduced here are the objects that
Parts~VII--VIII later modify with fallbacks and start strategies. Parts~II--VI
are a separate MVC branch; they use the same quotient notation but address a
different Smale problem and should not be read as prerequisites for the
algorithmic complexity statements below.
...
\begin{itemize}
\item \textbf{Uniform anchor-based evaluation}: Pandrosion's iteration is a special case of an anchor-based difference-quotient scheme applicable to any polynomial in $\mathbb{C}[z]$, and lifts formally to polynomial systems via a matrix-valued quotient $\mathbf{Q}_F$.

This provides a derivative-free algorithmic framework of independent interest, and an algorithmic companion to --- \emph{not} a competing complexity-theoretic resolution of --- Smale's 17th problem (already resolved by Beltr\'an--Pardo~\cite{beltranpardo} and Lairez~\cite{lairez}).
\item \textbf{Fractal boundary structure}: the induced complex dynamics exhibit self-similar basin boundaries distinct from those of Newton's method. We describe this structure qualitatively and do not claim new results on complex dynamics itself.
\item \textbf{A structural rewriting of the Euler product}: the infinite geometric sum $S_\infty(s) = (1 - s)^{-1}$ recasts the Euler factors of $\zeta(s)$ as ``infinite Pandrosion sums''. This is a notational observation and implies \emph{no} consequence for the Riemann Hypothesis.
\end{itemize}
...
\paragraph{Acknowledgment of classical prior art.} The matrix-valued divided difference $\mathbf{Q}_F$ that we use is \emph{not new}. It is the \emph{Schmidt slope matrix} (also called the \emph{Steffensen divided difference operator for systems}), introduced in \cite{schmidt1963} and developed systematically in \cite[Chapter 11]{ortegarheinboldt1970}. The corresponding fixed-point iteration $\mathbf{z}_{n+1} = \mathbf{z}_0 - \mathbf{Q}_F^{-1} F(\mathbf{z}_0)$ is the classical \emph{multivariate secant method} when the anchor is fixed, and reduces to multivariate Newton when the anchor tracks the iterate. Recent derivative-free methods exploiting variants of this slope matrix include Grau-S\'anchez--Noguera--Amat~\cite{grausanchez2011} (Ostrowski-type), Amat--Busquier~\cite{amat-busquier} (Steffensen-Newton hybrids), and the textbook treatment of Potra--Pt\'ak~\cite{potraptak1984}. Our \emph{specific} contribution here is not the slope matrix itself, but its combination with: (i) the \emph{periodic adaptive reanchoring} of Definition~\ref{def:adaptive-pandrosion-recall} (the scaling principle borrowed from \cite{besevic-real}), (ii) the higher-order Aitken-Steffensen acceleration $T_3$, and (iii) the non-holomorphic fallback of \cite{besevic7}. We restate Schmidt's slope matrix below for self-containment.

\begin{definition}[Schmidt slope matrix; multivariate Pandrosion difference quotient]\label{def:multivariate-Q}
...
\begin{proposition}[Empirical observation, multivariate adaptive Pandrosion-$T_3$]\label{thm:multivariate-empirical}
...
\end{equation*}
covering B\'ezout degrees $D = d^n$ from $9$ up to $1024$. The scheme achieves $5/5$ convergence at \emph{every} tested configuration ($45/45 = 100\%$ overall) with mean iteration counts:
\begin{center}
...
$(5, 2)$ & $32$ & $5/5$ & $13.4$ & \multicolumn{4}{c}{Mean across all configs: $16.2$} \\
\bottomrule
\end{tabular}
\end{center}
The largest tested configuration is $(n, d) = (5, 4)$ with B\'ezout degree $D = 1024$ ($1024$ projective roots, $126$ monomials per equation). \emph{This is an empirical observation, not a theorem; the convergence guarantee at the scale relevant to Smale 17 ($D \to \infty$) is open.} Detailed benchmarks are in \verb|latex/scripts/benchmark_multivariate_extended.py| and \verb|latex/scripts/benchmark_smale17_v4.py|.
\end{proposition}

\begin{remark}[Honest accounting of the multivariate per-orbit cost]
For a polynomial system of B\'ezout degree $D = d^n$, evaluating $F$ at one point costs $O(D \cdot n)$ BSS operations. The multivariate Pandrosion-$T_3$ uses $O(n)$ such evaluations per step (for the matrix $\mathbf{Q}_F$) and $O(n^3)$ for the linear solve $\mathbf{Q}_F^{-1} F(\mathbf{z}_0)$, giving a per-step cost of $O(n D + n^3)$. \emph{If} a multivariate analogue of Theorem~\ref{thm:product-contr} could be established giving $O(\log D)$ epochs to basin entry, the per-orbit cost would be $O((nD + n^3) \log D)$. We emphasise:
\begin{itemize}
\item This $O(\log D)$ epoch bound is \textbf{essentially the difficulty of Smale 17 itself} and is \emph{not} established by any of our arguments. Beltr\'an--Pardo~\cite{beltranpardo} and Lairez~\cite{lairez} achieve the analogous bound via deep $\mu$-theory and homotopy continuation respectively, methods orthogonal to ours.
\item The per-orbit cost above must be multiplied by the number of starts (heuristically $D$, by analogy with the univariate case) for a system-level total.
\item Our empirical validation extends to $(n, d) = (5, 4)$ with B\'ezout degree $D = 1024$, the largest setting in which the algorithm achieves $100\%$ convergence rate; this is still well below the asymptotic regime relevant to Smale 17 ($D \to \infty$).
\end{itemize}
\textbf{Honest summary.} The multivariate Pandrosion-$T_3$ is a derivative-free algorithmic alternative whose \emph{practical} performance matches Newton with analytic Jacobian on small-scale Smale-17-style benchmarks. Whether it admits an unconditional polynomial average-case complexity bound \emph{of the same order} as Lairez~\cite{lairez} is an \textbf{open question that we do not address}.
\end{remark}
...
\begin{proposition}[Pandrosion regularisation away from level collisions]\label{thm:reg}
For $z_0$ on the Cauchy circle $|z_0| = R = 1 + \rho$ and any $z \ne z_0$:
$$ |Q(z_0, z)| = \frac{|P(z) - P(z_0)|}{|z - z_0|} \ge \frac{||P(z_0)| - |P(z)||}{|z - z_0|}. $$
Consequently, on any sublevel region $|P(z)| \le \theta |P(z_0)|$ with
$0\le\theta<1$, the divided difference is bounded below by
\[
|Q(z_0,z)|\ge \frac{(1-\theta)|P(z_0)|}{|z-z_0|}.
\]
Thus the Pandrosion denominator is regular on strict residual sublevels of the
anchor. It may still vanish at level collisions $P(z)=P(z_0)$ with $z\ne z_0$;
the statement is a conditional regularisation, not a global non-vanishing
theorem.
\end{proposition}
...
The \emph{strict universal} form of Conjecture~\ref{conj:abd} (asserting the bound for \emph{every} normalised $P$) was refuted by computational search reported in Part~VII~\cite{besevic7}: there exist root constellations and internal anchors $z_0$ acting as exact stagnation traps, where the geometric sum of logarithms $\Sigma_{\log} > 0$ and $|P(z_K)| \approx |P(z_0)|$. This refutation is consistent with McMullen's topological impossibility theorem~\cite{mcmullen} for purely rational iterative schemes of degree $d \ge 4$. Part~VII addresses the obstruction by introducing a derivative-free \emph{non-holomorphic fallback} (Steffensen-Pandrosion-Armijo) and an analytic $\gamma$-damped variant, giving conditional BSS bounds under explicit non-degeneracy and basin-entry hypotheses. The conjecture above is retained in this Part~I as the natural open question for the \emph{purely rational} adaptive scheme; Theorem~\ref{thm:smale17-cond} below should be read as a conditional result on Conjecture~\ref{conj:abd}.
\end{remark}
...
\paragraph{What remains open.} Theorems~\ref{thm:pp-id}--\ref{thm:reg} provide an exact product factorisation and a divided-difference regularisation on strict residual sublevels. They do not remove all global obstructions: level collisions $P(z)=P(z_0)$ and stagnation traps remain possible.

The remaining gap is isolated by the \emph{Amortised Block Descent Conjecture}~\ref{conj:abd}: each $K$-epoch should reduce $|P|$ by a factor $e^{-c}$ with $c > 0$ universal on an appropriate generic class. Its strict universal form is known to be false (Remark~\ref{rem:conj-status} and \cite{besevic7}); what remains plausible is a relaxed form holding off a lower-dimensional exceptional set of anchors.
...
Under this conjectural form, Theorem~\ref{thm:smale17-cond} records a \emph{conditional} derivative-free $O(d^2 \log d)$-operation BSS bound for univariate root-finding. The multivariate extension to systems (Section~\ref{sec:multivariate}) is an algorithmic companion to the unconditional average-case resolutions of Beltr\'an--Pardo~\cite{beltranpardo} and Lairez~\cite{lairez}, not a replacement for them.
...
\bibitem{schmidt1963} J. W. Schmidt, ``Eine \"Ubertragung der Regula Falsi auf Gleichungen in Banachr\"aumen,'' Z. Angew. Math. Mech. \textbf{43} (1963), 1--10. \emph{(Original definition of the divided-difference / slope matrix for nonlinear systems.)}
\bibitem{ortegarheinboldt1970} J. M. Ortega and W. C. Rheinboldt, \textit{Iterative Solution of Nonlinear Equations in Several Variables}, Academic Press, New York, 1970. (Reprinted SIAM Classics in Applied Mathematics 30, 2000.) \emph{(Standard reference for the multivariate secant method and the slope matrix; cf.\ Chapter 11.)}
\bibitem{potraptak1984} F. A. Potra and V. Pt\'ak, \textit{Nondiscrete Induction and Iterative Processes}, Pitman, London, 1984.
\bibitem{grausanchez2011} M. Grau-S\'anchez, M. Noguera, and S. Amat, ``On the approximation of derivatives using divided difference operators preserving the local convergence order of iterative methods,'' arXiv:1110.2281 (2011).
\bibitem{amat-busquier} S. Amat and S. Busquier, ``Convergence and numerical analysis of a family of two-step Steffensen's methods,'' Comput. Math. Appl. \textbf{49} (2005), 13--22.
\end{thebibliography}
