...
\end{equation*}
The scheme achieves $5/5$ convergence at all tested configurations with mean iteration counts within a factor $2$--$3$ of pure multivariate Newton (with the analytic Jacobian) and within a factor $5$--$10$ of \verb|scipy.optimize.fsolve|. The largest tested configuration $(n, d) = (4, 3)$ corresponds to B\'ezout degree $D = 81$. \emph{This is an empirical observation, not a theorem; the convergence guarantee at the scale relevant to Smale 17 ($D \to \infty$) is open.} Detailed benchmarks are in \verb|benchmark_smale17_v4.py|.
\end{proposition}
...
\item Our empirical validation extends only to $(n, d) = (4, 3)$, far below the asymptotic regime relevant to Smale 17.
\end{itemize}
