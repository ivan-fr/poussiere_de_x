...
\begin{proposition}[Sendov on $\alpha$-regular polynomials, corrected]\label{prop:alpha-reg}
Let $P$ have a root $\zeta_j$ such that the Smale invariant of $P'$ at $\zeta_j$ is small,
\begin{equation}\label{eq:alpha-sendov}
\alpha_{\mathrm{Sendov}}(P, \zeta_j) \;:=\; \alpha(P', \zeta_j) \;=\; \frac{|P'(\zeta_j)|}{|P''(\zeta_j)|}\, \gamma(P', \zeta_j) \;<\; \alpha^\star.
\end{equation}
Then there is a critical point $\xi_k$ of $P$ at distance
$$
|\zeta_j - \xi_k| \;\le\; \frac{|P'(\zeta_j)|}{|P''(\zeta_j)|} \cdot \frac{1}{1 - 2\alpha_{\mathrm{Sendov}}(P, \zeta_j)} \;<\; 1
$$
provided $|P'(\zeta_j)|/|P''(\zeta_j)| \le (1 - 2\alpha^\star)$, i.e., Sendov holds at $\zeta_j$.
\end{proposition}

\begin{proof}[Sketch]
By Smale's $\alpha$-theorem, if $\alpha_{\mathrm{Sendov}}(P, \zeta_j) < \alpha^\star$, then Newton's iteration on $P'$ from $\zeta_j$ converges to a unique zero $\xi_k$ of $P'$ (a critical point of $P$) with explicit basin radius $\beta(P', \zeta_j)/(1 - 2\alpha)$. The Sendov inequality $|\zeta_j - \xi_k| \le 1$ follows when this radius is $\le 1$.
\end{proof}

\begin{remark}[Honest scope of Proposition~\ref{prop:alpha-reg}]
This corrects an earlier draft where the invariant was incorrectly evaluated at critical points $\xi$ rather than at roots $\zeta_j$. At a critical point, $P'(\xi) = 0$ by definition, hence $\alpha(P, \xi) = +\infty$ trivially: the "$\alpha$-regular at critical points" condition is empty.

The corrected invariant $\alpha_{\mathrm{Sendov}}(P, \zeta_j) := \alpha(P', \zeta_j)$ evaluates Smale's invariant of the \emph{derivative} polynomial $P'$ at the \emph{root} $\zeta_j$ of $P$. This measures whether Newton's iteration on $P'$, started at the root $\zeta_j$, converges to a critical point. The fraction of polynomials satisfying this condition is non-trivial and depends on the ensemble; we do not claim a $1 - O(1/d)$ measure here, since this requires a separate Bombieri--Weyl analysis of $P'$.
\end{remark}
