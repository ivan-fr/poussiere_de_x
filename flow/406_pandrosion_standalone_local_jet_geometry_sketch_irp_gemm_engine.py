#!/usr/bin/env python3
"""
406_pandrosion_standalone_local_jet_geometry_sketch_irp_gemm_engine.py
...
Mathematical idea:
  ...
  2. Do not call a multivariate algebraic solver, Groebner/resultant machinery, or a
     ...
     The 406 default is adaptive-sketch: for small systems it keeps the fastest
     stable GEMM/Newton-Schulz path; for larger local jets it can solve the
     correction in a randomized low-dimensional subspace.  The goal is not to
     beat a dense matmul kernel on its own terms, but to avoid doing a dense
     n-by-n inverse/matmul when Pandrosion geometry exposes a smaller direction
     space.

  3. 406 prepares waves of starts with a fused batched direct prepass.  The prepass
...
resolution.  The 406 difference from 405 is a randomized subspace pseudo-inverse:

    delta = P u,       min_u ||J P u + F||^2 + mu ||u||^2,

where P is a deterministic randomized sketch/basis with k << n when n is large.
This can beat dense matmul only by changing the problem size.  The complex GEMM,
real-packed GEMM, and SVD paths remain available as diagnostics/fallbacks.  The
full matrix kernel remains a GEMM-first pseudo-inverse:
...
MATRIX_ALGORITHM = "adaptive-sketch"
BATCH_KERNEL = "auto"
TORCH_DEVICE = "auto"
...
def configure_matrix_backend(args: argparse.Namespace) -> None:
    global MATRIX_BACKEND, MATRIX_ALGORITHM, BATCH_KERNEL, TORCH_DEVICE, TORCH_COMPLEX_DTYPE, TORCH_REAL_DTYPE, TORCH_RESOLVED_DEVICE, TORCH_RESOLVED_DTYPE, TORCH_RESOLVED_REAL_DTYPE, NS_ITERS, NS_DAMPING_FLOOR, SKETCH_DIM, SKETCH_FACTOR, SKETCH_MIN_N, SKETCH_SEED, SKETCH_MODE, SKETCH_SOLVER
    ...
    algorithm = str(getattr(args, "matrix_algorithm", "adaptive-sketch") or "adaptive-sketch").strip().lower().replace("_", "-")
    MATRIX_ALGORITHM = algorithm if algorithm in {"adaptive-sketch", "sketch-ns", "adaptive-ns", "realpack-ns", "gemm-ns", "svd", "auto"} else "adaptive-sketch"
    ...
    SKETCH_SEED = int(getattr(args, "sketch_seed", 406))
...
def selected_matrix_algorithm_406(dim: int = 0) -> str:
    if MATRIX_ALGORITHM not in {"adaptive-sketch", "adaptive-ns"}:
        return MATRIX_ALGORITHM
    if MATRIX_ALGORITHM == "adaptive-sketch" and int(dim) >= int(SKETCH_MIN_N):
        return "sketch-ns"
...
def matrix_svd_step(A: Any, rhs: Any, rcond: float = 1e-12, condition_cap: float = 1e12, damping: float = 0.0) -> tuple[Any, dict[str, Any]]:
    ...
    In 406 the default path is real-packed GEMM/Newton-Schulz on regularised
    ...
    if MATRIX_BACKEND in {"torch", "auto"}:
        try:
            selected_algorithm = selected_matrix_algorithm_406(int(np.asarray(A).shape[-1]))
...
def matrix_least_squares_fit(X: Any, Y: Any, rcond: float = 1e-12, condition_cap: float = 1e12) -> tuple[Any, dict[str, Any]]:
    ...
    if MATRIX_BACKEND in {"torch", "auto"}:
        try:
            selected_algorithm = selected_matrix_algorithm_406(int(np.asarray(X).shape[-1]))
            if selected_algorithm == "sketch-ns":
...
def make_system_406(args: argparse.Namespace, n: int, d: int) -> tuple[LocalJetGeometry, Any, str, str]:
...
def universal_atlas_start(target: TargetTrack, n: int, trial: int, seed: int, powers: Sequence[float], angles: Sequence[float], radii: Sequence[float], cap: float, roots_found: int, duplicates: int, failures: int, target_count: int, universal_cells: int = 16, universal_shells: int = 5, atlas_selection: str = "diverse-shell", **_: Any) -> tuple[Any, dict[str, Any]]:
    ...
    if mode in {"diverse", "diverse-shell", "shell", "raw-shell"}:
        ...
        meta = {
            "chart": "406-standalone-universal-diverse-shell-atlas",
            "atlas_mode": "406-diverse-shell",
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        if not trigger:
            if improved:
                ...
                status = "local-collapsed" if collapsed else "direct-406-pandrosion-irp-gemm-running"
                continue
...
def torch_batched_gemm_fit_406(X: Any, Y: Any) -> Any:
    XX_np = np.asarray(X, dtype=np.complex128)
    YY_np = np.asarray(Y, dtype=np.complex128)
    device, dtype = torch_device(), torch_complex_dtype()
    XX = torch.as_tensor(XX_np, dtype=dtype, device=device)
...
def torch_batched_gemm_step_406(A: Any, rhs: Any, damping: float = 0.0) -> Any:
    AA_np = np.asarray(A, dtype=np.complex128)
    bb_np = np.asarray(rhs, dtype=np.complex128)
    device, dtype = torch_device(), torch_complex_dtype()
    AA = torch.as_tensor(AA_np, dtype=dtype, device=device)
...
def torch_batched_gemm_candidates_406(dY: Any, dF: Any, F: Any, Y: Any, lambdas: Any, damping: float = 0.0, trust_radius: float = 0.0, yn: Optional[Any] = None) -> Any:
    """Fused 406 batch prepass kernel: fit local jets, step, and candidates in Torch."""
...
def torch_batched_realpack_candidates_406(dY: Any, dF: Any, F: Any, Y: Any, lambdas: Any, damping: float = 0.0, trust_radius: float = 0.0, yn: Optional[Any] = None) -> Any:
    """Fused 406 real-packed prepass kernel using real GEMM/Newton-Schulz."""
    dY_np = np.asarray(dY, dtype=np.complex128)
    dF_np = np.asarray(dF, dtype=np.complex128)
    F_np = np.asarray(F, dtype=np.complex128)
    Y_np = np.asarray(Y, dtype=np.complex128)
...
def torch_batched_sketch_candidates_406(dY: Any, dF: Any, F: Any, Y: Any, lambdas: Any, damping: float = 0.0, trust_radius: float = 0.0, yn: Optional[Any] = None) -> Any:
    """Fused 406 subspace-sketch prepass kernel.
    ...
    k = sketch_dim_for(n)
    if k >= n:
        return torch_batched_gemm_candidates_406(dY_np, dF_np, F_np, Y_np, L_np, damping, trust_radius, yn)
    device, dtype = torch_device(), torch_complex_dtype()
    X = torch.as_tensor(dY_np, dtype=dtype, device=device)
    Z = torch.as_tensor(dF_np, dtype=dtype, device=device)
    Fv = torch.as_tensor(F_np, dtype=dtype, device=device)
...
def selected_batch_kernel_406() -> str:
    if BATCH_KERNEL in {"sketch", "realpack", "complex"}:
        return BATCH_KERNEL
    selected_algorithm = selected_matrix_algorithm_406(128)
    if selected_algorithm == "sketch-ns":
        return "sketch"
    return "realpack" if selected_algorithm in {"realpack-ns", "auto"} else "complex"


def batch_direct_gemm_prepass_406(target: TargetTrack, Y0: Any, max_epochs: int, tol: float, accept: float, line_search: int, line_grid: Sequence[float], direction_seed: int, cloud_nodes: int, lm_damping: float, trust_radius: float) -> list[dict[str, Any]]:
    ...
    configured_kernel = selected_batch_kernel_406()
    ...
    for ep in range(max(0, int(max_epochs))):
        ...
        try:
            if configured_kernel == "sketch":
                C = torch_batched_sketch_candidates_406(dY, dF, Fa, Ya, lambdas, float(lm_damping), float(trust_radius), yn)
            elif configured_kernel == "realpack":
                C = torch_batched_realpack_candidates_406(dY, dF, Fa, Ya, lambdas, float(lm_damping), float(trust_radius), yn)
            else:
                C = torch_batched_gemm_candidates_406(dY, dF, Fa, Ya, lambdas, float(lm_damping), float(trust_radius), yn)
        except Exception as exc:
    ...
    for i in range(B):
        out.append({
            ...
            "batch_prepass_algorithm": f"406-{configured_kernel}-fused-gemm-batched-direct-before-irp",
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    geometry, base, source, backend = make_system_406(args, n, d)
    ...
    def build_trial_start(trial: int) -> dict[str, Any]:
        ...
        if explicit:
            ...
            geom = {"chart": "406-explicit-start", "atlas_mode": "explicit-start", "homothety": 1.0, "theta_deg": None}
        else:
            y_raw = None
            geom: dict[str, Any] = {}
            if bool(args.origin_seed) and (trial == 0 if int(args.origin_seed_period) <= 0 else trial % max(1, int(args.origin_seed_period)) == 0):
                ...
                if y_origin is not None:
                    y_raw = y_origin
                    geom = {"chart": "406-origin-affine-gemm-matrix-seed", "atlas_mode": "origin-affine-gemm-matrix-seed", "homothety": 1.0, "theta_deg": None, **ometa}
            if y_raw is None:
    ...
    while trial < int(args.pool) and len(roots) < int(args.count):
        ...
        if not wave:
            break
        use_prepass = bool(args.batch_prepass) and len(wave) > 1 and int(args.batch_prepass_epochs) > 0 and MATRIX_BACKEND in {"torch", "auto"} and MATRIX_ALGORITHM in {"adaptive-sketch", "sketch-ns", "adaptive-ns", "realpack-ns", "gemm-ns", "auto"}
        if use_prepass:
            pre = batch_direct_gemm_prepass_406(target, np.asarray([w["y0"] for w in wave], dtype=np.complex128), int(args.batch_prepass_epochs), float(args.tol), float(args.accept), int(args.line_search), parse_float_list(args.line_grid, []), geometry.seed + 0x406000 + 8191 * trial, int(args.hypercube_nodes), float(args.lm_damping), float(args.trust_radius))
    ...
    result = {
        ...
        "mode": "406-standalone-pandrosion-irp-batched-local-jet-gemm-matrix",
        "flow_formula": "any system as black-box geometric oracle -> lazily-sampled local-jet field (O(n)/step, no global precompute) -> universal Mobius atlas starts -> StartOpt -> sketch/realpack/full GEMM direct prepass -> Pandrosion lazy IRP charts -> hypercube Torch sketch/GEMM/Newton-Schulz matrix-jet correction + matrix polish, ALL on the jet geometry -> faithful residual acceptance",
        ...
        "parameters": {"system_source": str(args.system_source), "polys": str(args.polys or ""), "variables": str(args.variables or ""), "jet_quadratic": bool(args.jet_quadratic), "jet_radius": float(args.jet_radius), "jet_cache": bool(args.jet_cache), "geometric_polish_steps": int(args.geometric_polish_steps), "batch_wave": int(args.batch_wave), "batch_prepass": bool(args.batch_prepass), "batch_prepass_epochs": int(args.batch_prepass_epochs), "batch_kernel": BATCH_KERNEL, "resolved_batch_kernel": selected_batch_kernel_406(), "matrix_backend": MATRIX_BACKEND, "matrix_algorithm": MATRIX_ALGORITHM, "resolved_matrix_algorithm": selected_matrix_algorithm_406(n), "ns_iters": int(NS_ITERS), "ns_damping_floor": float(NS_DAMPING_FLOOR), "sketch_dim": int(SKETCH_DIM), "resolved_sketch_dim": int(sketch_dim_for(n)), "sketch_factor": float(SKETCH_FACTOR), "sketch_min_n": int(SKETCH_MIN_N), "sketch_mode": str(SKETCH_MODE), "sketch_solver": str(SKETCH_SOLVER), "sketch_seed": int(SKETCH_SEED), "torch_device": TORCH_DEVICE, "torch_resolved_device": TORCH_RESOLVED_DEVICE, "torch_complex_dtype": TORCH_COMPLEX_DTYPE, "torch_resolved_complex_dtype": TORCH_RESOLVED_DTYPE, "torch_real_dtype": TORCH_REAL_DTYPE, "torch_resolved_real_dtype": TORCH_RESOLVED_REAL_DTYPE, "torch_version": str(torch.__version__), "matrix_rcond": float(args.matrix_rcond), "matrix_condition_cap": float(args.matrix_condition_cap), "starts": str(args.starts or ""), "system_mode": str(args.system_mode), "count": int(args.count), "pool": int(args.pool), "accept": float(args.accept), "tol": float(args.tol), "epochs": int(args.epochs), "cluster_sep": float(args.cluster_sep), "line_search": int(args.line_search), "hypercube_nodes": int(args.hypercube_nodes), "atlas_selection": str(args.atlas_selection), "atlas_bypass_startopt": bool(args.atlas_bypass_startopt), "atlas_calls": int(atlas_calls), "startopt_steps": int(args.startopt_steps), "startopt_candidates": int(args.startopt_candidates), "startopt_gains_source": startopt_gains_source, "startopt_gains_count": len(gains), "scale_ladder": bool(args.scale_ladder), "ladder_subdiv": int(args.ladder_subdiv), "ladder_octaves": int(args.ladder_octaves), "ladder_base": float(args.ladder_base), "powers_count": len(powers), "irp_gains_source": irp_gains_source, "irp_gains_count": len(irp_gain_values), "irp_scales_count": len(irp_scales)},
        "roots": encoded_roots,
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="406 standalone batched Pandrosion IRP local-jet sketch/GEMM matrix engine.")
    ...
    p.add_argument("--batch-kernel", choices=["auto", "sketch", "realpack", "complex"], default="auto", help="Batched prepass kernel. auto follows the selected 406 matrix algorithm.")
    ...
    p.add_argument("--matrix-backend", choices=["torch", "numpy", "auto"], default="torch", help="Matrix kernel backend. torch+adaptive-sketch is the 406 default; numpy preserves the SVD fallback behavior.")
    p.add_argument("--matrix-algorithm", choices=["adaptive-sketch", "sketch-ns", "adaptive-ns", "realpack-ns", "gemm-ns", "svd", "auto"], default="adaptive-sketch", help="Matrix correction algorithm. adaptive-sketch uses a low-dimensional Pandrosion subspace for large local jets and a full GEMM path for small systems.")
    ...
    p.add_argument("--torch-real-dtype", choices=["auto", "float16", "bfloat16", "float32", "float64"], default="auto", help="Torch real dtype for 406 real-packed kernels. auto uses float32.")
    ...
    p.add_argument("--sketch-seed", type=int, default=406, help="Deterministic seed for the randomized sketch basis.")
    ...
    p.add_argument("--outdir", default="/private/tmp/406_standalone_batched_pandrosion_irp_gemm_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(args.self_test):
        ...
        args.out = args.out or "/private/tmp/406_standalone_batched_pandrosion_irp_gemm_out/self_test_406.json"
    ...
    out = Path(args.out) if args.out else Path(args.outdir) / f"406_batched_pandrosion_irp_gemm_{cases[0].replace(',', 'x') if cases else 'case'}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")
    print("=" * 120, flush=True)
    print("406 STANDALONE BATCHED PANDROSION IRP -- SKETCH/GEMM MATRIX-JET CALCULUS", flush=True)
    print("PyTorch + NumPy + stdlib only; no local flow imports. The geometry is 317's lazily-sampled local-jet field;", flush=True)
    print(f"matrix_backend={MATRIX_BACKEND}, algorithm={MATRIX_ALGORITHM}->{selected_matrix_algorithm_406(parse_case(cases[0])[0] if cases else 0)}, torch={torch.__version__}, device={TORCH_RESOLVED_DEVICE}, complex_dtype={TORCH_RESOLVED_DTYPE}, real_dtype={TORCH_RESOLVED_REAL_DTYPE}, batch_kernel={selected_batch_kernel_406()}, sketch_dim={sketch_dim_for(parse_case(cases[0])[0]) if cases else 0}, batch_wave={int(args.batch_wave)}, irp=on", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
        g = r.get("geometry", {})
