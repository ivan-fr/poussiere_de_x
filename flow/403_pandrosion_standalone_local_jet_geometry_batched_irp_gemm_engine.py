#!/usr/bin/env python3
"""
403_pandrosion_standalone_local_jet_geometry_batched_irp_gemm_engine.py
...
Mathematical idea:
  ...
  2. Do not call a multivariate algebraic solver, Groebner/resultant machinery, or a
     ...
     The 403 default forms regularised normal equations and applies an approximate
     inverse by Newton-Schulz matrix products, so the heavy kernel is GEMM-like.

  3. 403 prepares waves of starts with a batched GEMM/Newton-Schulz direct prepass,
     then hands every candidate to the normal Pandrosion lazy IRP chart corrector.
...
resolution.  The 403 difference from 402 is real wave batching before IRP. The
matrix kernel remains a GEMM-first pseudo-inverse:
...
def matrix_svd_step(A: Any, rhs: Any, rcond: float = 1e-12, condition_cap: float = 1e12, damping: float = 0.0) -> tuple[Any, dict[str, Any]]:
    ...
    In 403 the default path is GEMM-first Newton-Schulz on regularised normal
...
def make_system_403(args: argparse.Namespace, n: int, d: int) -> tuple[LocalJetGeometry, Any, str, str]:
...
def universal_atlas_start(target: TargetTrack, n: int, trial: int, seed: int, powers: Sequence[float], angles: Sequence[float], radii: Sequence[float], cap: float, roots_found: int, duplicates: int, failures: int, target_count: int, universal_cells: int = 16, universal_shells: int = 5, atlas_selection: str = "diverse-shell", **_: Any) -> tuple[Any, dict[str, Any]]:
    ...
    if mode in {"diverse", "diverse-shell", "shell", "raw-shell"}:
        ...
        meta = {
            "chart": "403-standalone-universal-diverse-shell-atlas",
            "atlas_mode": "403-diverse-shell",
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        if not trigger:
            if improved:
                ...
                status = "local-collapsed" if collapsed else "direct-403-pandrosion-irp-gemm-running"
                continue
...
def torch_batched_gemm_fit_403(X: Any, Y: Any) -> Any:
    XX_np = np.asarray(X, dtype=np.complex128)
    YY_np = np.asarray(Y, dtype=np.complex128)
...
def torch_batched_gemm_step_403(A: Any, rhs: Any, damping: float = 0.0) -> Any:
    AA_np = np.asarray(A, dtype=np.complex128)
    bb_np = np.asarray(rhs, dtype=np.complex128)
    device, dtype = torch_device(), torch_complex_dtype()
    AA = torch.as_tensor(AA_np, dtype=dtype, device=device)
...
def batch_direct_gemm_prepass_403(target: TargetTrack, Y0: Any, max_epochs: int, tol: float, accept: float, line_search: int, line_grid: Sequence[float], direction_seed: int, cloud_nodes: int, lm_damping: float, trust_radius: float) -> list[dict[str, Any]]:
    ...
    for ep in range(max(0, int(max_epochs))):
        ...
        coef = torch_batched_gemm_fit_403(dY, dF)
        A = np.swapaxes(coef, 1, 2)
        delta = torch_batched_gemm_step_403(A, -Fa, float(lm_damping))
        if float(trust_radius) > 0.0:
            lim = float(trust_radius) * yn
            dn = np.linalg.norm(delta, axis=1).astype(float)
            scale = np.where((dn > lim) & (dn > 0), lim / np.maximum(dn, 1e-300), 1.0)
            delta = delta * scale[:, None]
        C = Ya[:, None, :] + lambdas[None, :, None] * delta[:, None, :]
    ...
    for i in range(B):
        out.append({
            ...
            "batch_prepass_algorithm": "403-gemm-batched-direct-before-irp",
            **last_meta[i],
        })
    return out
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    geometry, base, source, backend = make_system_403(args, n, d)
    ...
    def build_trial_start(trial: int) -> dict[str, Any]:
        ...
        if explicit:
            ...
            geom = {"chart": "403-explicit-start", "atlas_mode": "explicit-start", "homothety": 1.0, "theta_deg": None}
        else:
            y_raw = None
            geom: dict[str, Any] = {}
            if bool(args.origin_seed) and (trial == 0 if int(args.origin_seed_period) <= 0 else trial % max(1, int(args.origin_seed_period)) == 0):
                ...
                if y_origin is not None:
                    y_raw = y_origin
                    geom = {"chart": "403-origin-affine-gemm-matrix-seed", "atlas_mode": "origin-affine-gemm-matrix-seed", "homothety": 1.0, "theta_deg": None, **ometa}
            if y_raw is None:
    ...
    result = {
        ...
        "mode": "403-standalone-pandrosion-irp-batched-local-jet-gemm-matrix",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="403 standalone batched Pandrosion IRP local-jet Torch GEMM matrix engine.")
    ...
    p.add_argument("--matrix-backend", choices=["torch", "numpy", "auto"], default="torch", help="Matrix kernel backend. torch+gemm-ns is the 403 default; numpy preserves the SVD fallback behavior.")
    ...
    p.add_argument("--outdir", default="/private/tmp/403_standalone_batched_pandrosion_irp_gemm_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(args.self_test):
        ...
        args.out = args.out or "/private/tmp/403_standalone_batched_pandrosion_irp_gemm_out/self_test_403.json"
    ...
    out = Path(args.out) if args.out else Path(args.outdir) / f"403_batched_pandrosion_irp_gemm_{cases[0].replace(',', 'x') if cases else 'case'}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")
    print("=" * 120, flush=True)
    print("403 STANDALONE BATCHED PANDROSION IRP -- PYTORCH GEMM MATRIX-JET CALCULUS", flush=True)
