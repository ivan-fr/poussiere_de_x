#!/usr/bin/env python3
"""
120_pandrosion_equal_value_tensor_aware_numpy_engine.py
...
def pandrosion_corrector(
    ...
    """120 equal-value/tensor-aware pure derivative-free Pandrosion corrector.

    Same mathematical step as 118:
        Q_G(a,b_*) delta = -G(a), a_next = a + lambda delta.
    Improvements are purely NumPy/geometric:
      * b_* scoring is batched;
      * a few top probes are penalized by exact finite-slope conditioning;
      * probes close to equal-value poles F(b)=F(a) are discouraged;
      * finite-slope curvature is estimated by slope variation along the probe segment;
      * the line-search evaluates all lambdas in one batch and takes the best.
    """
    ensure_numpy()
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        # Batch line-search: choose the best decrease, not merely the first.
        L = np.asarray(lambdas, dtype=float)
        Ytry = y[None, :] + L[:, None] * delta[None, :]
        Rtry = target.residuals_batch(Ytry)
        total_line_evals += int(len(L))
        finite = np.isfinite(Rtry)
        better = finite & ((Rtry < r) | (Rtry < best_r))
        if not np.any(better):
            status = "no-decrease"
            break
        # Prefer lowest residual; a tiny lambda penalty avoids unnecessary huge steps on ties.
    ...
    return {
        ...
        "corrector": "120-equal-value-tensor-aware-pure-pandrosion-exact-slope",
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    # Fully autonomous 120 keeps A simple and robust; the projective/homothetic flow provides the main geometry.
    ...
    for trial in range(int(args.pool)):
        ...
        loc = pandrosion_corrector(target, y0, max_epochs=int(args.epochs), tol=float(args.tol), accept=float(args.accept), trial_timeout=float(args.trial_timeout), line_search=int(args.line_search), probe_scale=float(getattr(args, "probe_scale", 0.035)), direction_seed=system.seed + 7919 * trial, probe_candidates=int(args.probe_candidates), probe_radii=probe_radii, include_self_probe=bool(args.probe_self), probe_condition_top=int(args.probe_condition_top), probe_condition_weight=float(args.probe_condition_weight), probe_axis=bool(args.probe_axis), probe_equal_value_weight=float(args.probe_equal_value_weight), probe_equal_value_eps=float(args.probe_equal_value_eps), probe_curvature_top=int(args.probe_curvature_top), probe_curvature_weight=float(args.probe_curvature_weight), probe_curvature_mid=float(args.probe_curvature_mid), trust_cond_weight=float(args.trust_cond_weight), trust_cond_min=float(args.trust_cond_min), line_grid=parse_float_list(args.line_grid, []))
        ...
        roots.append({
            "id": rid,
            "source": "120-autonomous-batch-probe-aware-pure-thales-pandrosion",
            "trial": int(trial),
    ...
    result = {
        "script": "120_pandrosion_equal_value_tensor_aware_numpy_engine.py",
        "autonomous": True,
        "dependencies": {"python_scripts": [], "numpy": bool(np is not None)},
        "mode": "120-autonomous-equal-value-tensor-aware-pure-pandrosion/thales-power-riemann-startopt-exact-slope-pole-avoidance-curvature-conditioning",
        "flow_formula": "u -> Mobius_Riemann(theta,pole)(u) -> y=Lambda*Mobius(u) -> batched StartOpt(y) -> z=A*y -> PURE Pandrosion Q_G(a,b) with batched b-probes and batched line search; no Newton/Jacobian fallback",
        "case": f"{n},{d}",
        "family": "ks",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="120 standalone NumPy pure-Thales Pandrosion engine: 120 successor with equal-value pole avoidance and finite-slope tensor-curvature scoring.")
    ...
    p.add_argument("--outdir", default="/mnt/data/120_equal_value_tensor_aware_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(getattr(args, "self_test", False)):
        ...
        args.out = args.out or "/mnt/data/120_equal_value_tensor_aware_out/self_test_120.json"
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "120_pandrosion_equal_value_tensor_aware_numpy_engine.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
    ...
    print("120 autonomous EQUAL-VALUE/TENSOR-AWARE NumPy PURE Thales Pandrosion - direct successor of 119", flush=True)
    print("No dependency on previous Python scripts; batched b-probes + equal-value pole avoidance + finite-slope curvature/conditioning + batched line search; no Newton/Jacobian fallback path.", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
