...
Autonomous NumPy Pandrosion root extractor for exact KS systems or exact
user-supplied polynomial systems, using the 304 universal atlas, the 305/306
hypercube tensor inverse-jet corrector, and an optional 315 fitted geometry
kernel guide.
...
No imports from previous Pandrosion scripts.  This file contains its own:
  ...
  - exact expression-polynomial system source for small custom systems
  ...
  - fitted geometry-kernel guide around any exact system source
...
The 315 geometry backend is the intended large-KS mode.  It samples projective
...
For user-supplied systems, 315 uses the geometry kernel only as a fitted guide
for starts.  eval/eval_batch and final residual validation remain exact.
...
def auto_lazy_feature_count(n: int, d: int, cap: int) -> int:
    """Conservative default for the 315 random-feature backend."""
...
def auto_geometry_anchor_count(n: int, d: int, cap: int) -> int:
    """Default anchor budget for the 315 projective geometry backend."""
...
class LazyFeatureKostlanSystem:
    """315 large-case Kostlan evaluator.
    ...
    def make(
        ...
        m = max(n + 1, int(m))
        seed = stable_seed(n, d, seed_index, salt=0x315315)
        rng = np.random.default_rng(seed)
...
class GeometryKernelKostlanSystem:
    """315 projective geometry oracle for large Kostlan systems.
    ...
    def make(
        ...
        m = max(n + 2, int(m))
        seed = stable_seed(n, d, seed_index, salt=0x315C0DE)
        rng = np.random.default_rng(seed)
...
class SafeExpression:
    """Small AST evaluator for polynomial-like NumPy expressions."""

    def __init__(self, raw: str) -> None:
        self.raw = str(raw).strip()
        expr = self.raw.replace("^", "**")
    ...
    def _eval_node(self, node: ast.AST, env: dict[str, Any]) -> Any:
        ...
        if isinstance(node, ast.Name):
            if node.id not in env:
                raise ValueError(f"unknown variable {node.id!r} in polynomial expression")
            return env[node.id]
...
class ExpressionPolynomialSystem:
    """Exact user-supplied polynomial system evaluated by a safe AST subset."""
    ...
    def make(cls, n: int, d: int, raw: str, variable_names: Optional[Sequence[str]] = None, seed_index: int = 0) -> "ExpressionPolynomialSystem":
        ...
        n = int(n)
        d = int(d)
        names = [p.strip() for p in (variable_names or []) if str(p).strip()]
        if not names:
            names = ["x"] if n == 1 else [f"x{i+1}" for i in range(n)]
        if len(names) != n:
        ...
        seed = stable_seed(n, d, seed_index, salt=0x315EAC7)
        return cls(n=n, d=d, seed=seed, expressions_raw=parts, expressions=exprs, variable_names=names)
    ...
    def _env(self, ZZ: "np.ndarray") -> dict[str, Any]:
...
class FittedGeometryKernelSystem:
    """Kernel geometry fitted to exact F-values at projective anchors."""
    ...
    def make(cls, base: Any, n: int, d: int, args: argparse.Namespace) -> "FittedGeometryKernelSystem":
        ensure_numpy()
        t0 = now()
        kernel = GeometryKernelKostlanSystem.make(
            n,
            d,
        ...
        M = int(A.shape[0])
        try:
            W = np.linalg.solve(A + ridge * np.eye(M, dtype=np.complex128), F)
            method = "solve"
        except Exception:
            W, _, _, _ = np.linalg.lstsq(A + ridge * np.eye(M, dtype=np.complex128), F, rcond=None)
            method = "lstsq"
        obj = cls(base=base, kernel=kernel, weights=np.asarray(W, dtype=np.complex128), ridge=ridge, fit_seconds=now() - t0)
...
class GeometryWrappedSystem:
    """Exact system plus a fitted geometry guide.

    eval/eval_batch/slope_matrix always delegate to the exact system.  The
    geometry system is exposed separately and used only for starts/metadata.
    """
    ...
    @property
    def generation_seconds(self) -> float:
        return float(getattr(self.exact, "generation_seconds", 0.0)) + float(self.geometry_system.fit_seconds)
...
def make_system_315(args: argparse.Namespace, n: int, d: int) -> tuple[Any, str, str]:
    ...
    if source in {"poly", "polynomial", "expr", "expression"}:
        ...
        exact = ExpressionPolynomialSystem.make(n, d, str(getattr(args, "polys", "") or ""), variables, seed_index=int(args.seed_index))
        # Backward-compatible spelling: for explicit systems, --system-mode
        # geometry-kernel means geometry around the exact system, not replacement.
        mode = str(getattr(args, "system_mode", "auto")).strip().lower().replace("_", "-")
        ...
        if geometry_mode in {"kernel", "geometry", "geometry-kernel", "projective-kernel"}:
            ...
            return GeometryWrappedSystem(exact=exact, geometry_system=guide, geometry_mode="kernel"), "polynomial", "exact+geometry-kernel"
...
def parse_start_points(raw: Optional[str], n: int) -> list[Any]:
    ...
    if int(n) == 1 and ";" not in text and "|" not in text:
        ...
        return [np.asarray([parse_complex_token(p)], dtype=np.complex128) for p in parts]
    points = []
...
def origin_affine_start_315(target: TargetTrack, n: int, h: float, max_norm: float) -> tuple[Optional[Any], dict[str, Any]]:
    """Finite-difference affine seed for the 315 lazy large-KS backend.
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    system, system_source, system_backend = make_system_315(args, n, d)
    ...
    explicit_starts = parse_start_points(getattr(args, "starts", None), n)
    ...
    for trial in range(int(args.pool)):
        ...
        if trial < len(explicit_starts):
            y_raw = np.asarray(explicit_starts[trial], dtype=np.complex128).copy()
            ...
            geom = {
                ...
                "atlas_geometry": "user-supplied-start",
                "atlas_selected_geometry_cell": "explicit-start",
                "atlas_cells_tested": 0,
                "atlas_admissible_cells": 1,
                "atlas_selected_index": int(trial),
                "atlas_selection_rule": "explicit-start-order",
            }
        origin_period = int(args.origin_seed_period)
        origin_due = y_raw is None and bool(args.origin_seed) and system_backend in {"lazy-feature", "geometry-kernel", "exact+geometry-kernel"} and (trial == 0 if origin_period <= 0 else trial % max(1, origin_period) == 0)
        if origin_due:
            cap = float(args.origin_seed_max_norm)
            if not (math.isfinite(cap) and cap > 0):
                cap = 2.0 * math.sqrt(max(1, n))
            y_origin, ometa = origin_affine_start_315(guide_target, n, float(args.origin_seed_h), cap)
            if y_origin is not None:
                ...
                geom = {
                    ...
                    "fail_pressure": float((failures + 1.0) / (trial + 1.0)),
                    "progress": float(min(1.0, max(0.0, len(roots) / max(1.0, float(args.count))))),
                    "chart": "315-origin-affine-finite-difference-seed",
                    "atlas_mode": "315-origin-affine-seed",
        ...
        if y_raw is None:
            y_raw, geom = universal_atlas_start(
                guide_target, n, trial, system.seed + 0x113000, powers, angles, radii, float(args.power_cap),
        ...
        start_target = target if start_is_explicit else guide_target
        ...
        roots.append({
            "id": rid,
            "source": "315-geometry-kostlan-plus-312-lazy-irp-hypercube-inversejet",
            "trial": int(trial),
    ...
    result = {
        "script": "315_pandrosion_exact_system_geometry_irp_hypercube_inversejet_numpy_engine.py",
        "autonomous": True,
        ...
        "flow_formula": "315 exact system source (KS or user polynomial) plus optional fitted geometry-kernel guide for starts -> exact-system StartOpt seed -> 312 lazy policy on exact F: direct 306 hypercube inverse-jet first; activate IRP chart cascade only on stagnation/weak contraction/bad condition; final residual is always exact F, no Halley, no anchored Q, no Newton fallback",
        "case": f"{n},{d}",
        "family": system_source,
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="315 standalone NumPy Pandrosion engine: exact KS or exact user polynomials, optional fitted geometry-kernel guide for starts, plus 312 lazy IRP and 306 hypercube inverse-jet core.")
    ...
    p.add_argument("--system-source", choices=["ks", "kostlan", "polynomial", "poly", "expr", "expression"], default="ks", help="315 exact system source; polynomial keeps eval/validation exact")
    p.add_argument("--polys", "--poly", default=None, help="315 polynomial expression(s), separated by ';' for systems; use x or x1,x2,... variables")
    p.add_argument("--variables", default=None, help="315 optional comma-separated variable names for --system-source polynomial")
    p.add_argument("--geometry-mode", choices=["none", "kernel", "geometry", "geometry-kernel", "projective-kernel"], default="none", help="315 optional fitted geometry guide around the exact system; eval/validation still use exact F")
    p.add_argument("--geometry-starts", action="store_true", default=True, help="315 use the fitted geometry guide for atlas/start optimization only")
    p.add_argument("--no-geometry-starts", dest="geometry_starts", action="store_false")
    p.add_argument("--geometry-fit-ridge", type=float, default=1e-8, help="315 ridge for fitting the geometry kernel to exact F(anchor) values")
    p.add_argument("--starts", default=None, help="315 explicit start points; n=1 accepts comma list or semicolon list, n>1 uses 'x1,x2;...'")
    p.add_argument("--system-mode", choices=["auto", "dense", "geometry-kernel", "geometry", "kernel", "projective-kernel", "lazy-feature", "lazy", "feature", "stream"], default="auto", help="315 system backend; auto uses dense below --dense-max-terms and geometry-kernel above it")
    p.add_argument("--dense-max-terms", type=int, default=250000, help="315 auto backend threshold for exact dense monomial materialization")
    p.add_argument("--geometry-anchors", type=int, default=0, help="315 geometry-kernel anchor count; 0 chooses a dimension/degree-based default")
    p.add_argument("--geometry-anchor-cap", type=int, default=4096, help="315 cap for automatic geometry-kernel anchor count")
    p.add_argument("--geometry-anchor-scales", default="0.25,0.5,1,2,4", help="315 affine projective anchor shell scales")
    p.add_argument("--geometry-dynamic-normalize", action="store_true", default=True, help="315 pointwise kernel scaling for overflow/underflow-safe geometry evaluation")
    p.add_argument("--no-geometry-dynamic-normalize", dest="geometry_dynamic_normalize", action="store_false")
    p.add_argument("--geometry-self-normalize", action="store_true", default=True, help="315 normalize each kernel row to unit RMS before residual evaluation")
    p.add_argument("--no-geometry-self-normalize", dest="geometry_self_normalize", action="store_false")
    p.add_argument("--geometry-eval-block", type=int, default=128, help="315 max eval rows per geometry-kernel NumPy block")
    p.add_argument("--lazy-features", type=int, default=0, help="315 lazy-feature count; 0 chooses a dimension/degree-based default")
    p.add_argument("--lazy-feature-cap", type=int, default=8192, help="315 cap for automatic lazy-feature count")
    p.add_argument("--lazy-projective-normalize", action="store_true", default=True, help="315 scale lazy evaluations by the Kostlan projective norm; zeros are unchanged")
    p.add_argument("--no-lazy-projective-normalize", dest="lazy_projective_normalize", action="store_false")
    p.add_argument("--lazy-dynamic-normalize", action="store_true", default=True, help="315 pointwise dynamic feature scaling for overflow-safe lazy evaluation; zeros are unchanged")
    p.add_argument("--no-lazy-dynamic-normalize", dest="lazy_dynamic_normalize", action="store_false")
    p.add_argument("--lazy-eval-block", type=int, default=128, help="315 max eval rows per lazy NumPy feature block")
    p.add_argument("--origin-seed", action="store_true", default=True, help="315 use a finite-difference affine seed at the origin for large geometry/lazy cases")
    p.add_argument("--no-origin-seed", dest="origin_seed", action="store_false")
    p.add_argument("--origin-seed-h", type=float, default=1e-5, help="315 finite coordinate step for the origin affine seed")
    p.add_argument("--origin-seed-max-norm", type=float, default=0.0, help="315 cap for origin affine seed norm; 0 uses 2*sqrt(n)")
    p.add_argument("--origin-seed-period", type=int, default=0, help="315 origin seed trial period for geometry/lazy cases; 0 means trial 0 only")
    ...
    p.add_argument("--outdir", default="/mnt/data/315_geometry_kostlan_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(getattr(args, "self_test", False)):
        ...
        args.out = args.out or "/mnt/data/315_geometry_kostlan_out/self_test_315.json"
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "315_pandrosion_exact_system_geometry_irp_hypercube_inversejet_numpy_engine.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
        first = cases[0].replace(",", "x") if cases else "case"
        out = Path(args.outdir) / f"315_geometry_kostlan_{first}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")

    print("=" * 120, flush=True)
    print("315 autonomous EXACT SYSTEM + GEOMETRY-GUIDED LAZY IRP + UNIVERSAL HYPERCUBE INVERSE-JET NumPy Pandrosion", flush=True)
    print("No dependency on previous Python scripts; exact KS or user polynomial eval/validation, optional fitted geometry-kernel guide for starts, 304/306 universal atlas + hypercube tensor inverse-jet core wrapped in 312 lazy/collapse-triggered IRP chart homothety/reciprocal renormalization; no Halley/anchored-Q/Newton fallback path.", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
        ...
        print(f"case={family}({r['n']},{r['degree']}), backend={backend}{backend_note}, seed={r['seed']}, terms={r['terms']}, Bezout={r['bezout']}", flush=True)
