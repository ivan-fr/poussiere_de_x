#!/usr/bin/env python3
"""Expanded multivariate benchmark: 201 vs 301.
...
FLOW = ROOT / "flow"
OUT = ROOT / "benchmarks" / "201_vs_301_complete"
...
def run_one(tag: str, n: int, d: int, seed: int) -> dict:
    ...
    wall = time.time() - t0
    rec = {"engine": tag, "case": case, "n": n, "degree": d, "seed_index": seed, "requested_count": cnt, "returncode": proc.returncode, "wall_seconds": wall, "json": str(out), "stderr_tail": proc.stderr[-1200:], "stdout_tail": proc.stdout[-1200:]}
...
def main() -> int:
    ...
    md=[]
    md += ["# Complete multivariate benchmark 201 vs 301", "", f"Cases: {CASES}", f"Seeds: {SEEDS}", f"Base args: `{' '.join(BASE)}`", "Requested roots: `min(Bezout, 12)`", ""]
    ...
    wins={"roots":{"201":0,"301":0,"tie":0},"speed":{"201":0,"301":0,"tie":0},"best_residual":{"201":0,"301":0,"tie":0},"median_residual":{"201":0,"301":0,"tie":0}}
    for g in groups.values():
        if '201' not in g or '301' not in g: continue
        a,b=g['201'],g['301']
        ar,br=int(a.get('unique_roots') or 0),int(b.get('unique_roots') or 0)
        wins['roots']['201' if ar>br else '301' if br>ar else 'tie']+=1
        at,bt=float(a.get('total_seconds') or 1e99),float(b.get('total_seconds') or 1e99)
        wins['speed']['201' if at<bt else '301' if bt<at else 'tie']+=1
        ...
        wins['best_residual']['201' if av<bv else '301' if bv<av else 'tie']+=1
        am,bm=a.get('median_residual'),b.get('median_residual')
        am=float(am) if am is not None else 1e99; bm=float(bm) if bm is not None else 1e99
        wins['median_residual']['201' if am<bm else '301' if bm<am else 'tie']+=1
    for k,v in wins.items(): md.append(f"- {k}: {v}")
