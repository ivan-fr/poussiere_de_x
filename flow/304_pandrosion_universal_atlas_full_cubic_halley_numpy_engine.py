#!/usr/bin/env python3
"""
304_pandrosion_universal_atlas_full_cubic_halley_numpy_engine.py
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    for trial in range(int(args.pool)):
        ...
        roots.append({
            "id": rid,
            "source": "304-universal-atlas-full-cubic-halley",
            "trial": int(trial),
    ...
    result = {
        "script": "304_pandrosion_universal_atlas_full_cubic_halley_numpy_engine.py",
        "autonomous": True,
        "dependencies": {"python_scripts": [], "numpy": bool(np is not None)},
        "mode": "304-universal-atlas-full-cubic-halley",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="304 standalone NumPy Pandrosion engine: fixed universal atlas plus anchored full cubic Halley.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "304_pandrosion_universal_atlas_full_cubic_halley_numpy_engine.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
    ...
    print("304 autonomous UNIVERSAL FIXED ATLAS + ANCHORED FULL-CUBIC HALLEY NumPy Pandrosion", flush=True)
