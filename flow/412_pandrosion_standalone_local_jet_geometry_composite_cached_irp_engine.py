#!/usr/bin/env python3
"""
412_pandrosion_standalone_local_jet_geometry_composite_cached_irp_engine.py
...
def configure_matrix_backend(args: argparse.Namespace) -> None:
    ...
    SKETCH_SEED = int(getattr(args, "sketch_seed", 412))
...
def directional_sketch_delta(target: TargetTrack, y: Any, f: Any, ep: int, seed: int, lm_damping: float = 0.0, trust_radius: float = 0.0, matrix_rcond: float = 1e-12, matrix_condition_cap: float = 1e12) -> Optional[tuple[Any, dict[str, Any]]]:
    ...
    return delta, {
        ...
        "directional_jet": True,
        "directional_jet_algorithm": "412-composite-cached-coded-probe-or-full-directional-oracle-sampled-jp-subspace",
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    result = {
        ...
        "mode": "412-standalone-pandrosion-irp-composite-cached-directional-sketch-local-jet-matrix",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="412 standalone Pandrosion IRP composite-cached directional-sketch local-jet matrix engine.")
    ...
    p.add_argument("--batch-kernel", choices=["auto", "sketch", "realpack", "complex"], default="auto", help="Batched prepass kernel. auto follows the selected 412 matrix algorithm.")
    ...
    p.add_argument("--matrix-backend", choices=["torch", "numpy", "auto"], default="torch", help="Matrix kernel backend. torch+adaptive-directional-sketch is the 412 default; numpy preserves the SVD fallback behavior.")
    ...
    p.add_argument("--torch-real-dtype", choices=["auto", "float16", "bfloat16", "float32", "float64"], default="auto", help="Torch real dtype for 412 real-packed kernels. auto uses float32.")
    ...
    p.add_argument("--sketch-seed", type=int, default=412, help="Deterministic seed for the randomized sketch basis.")
    ...
    p.add_argument("--directional-coded-probe", action="store_true", default=True, help="Try the 412 composite-cached coded probe D=P R with q<<k before the full directional sketch.")
    ...
    p.add_argument("--directional-fast-projector", action="store_true", default=True, help="Try the 412 diagonal projected reduced solver before SVD/Newton-Schulz.")
    ...
    p.add_argument("--outdir", default="/private/tmp/412_standalone_composite_cached_pandrosion_irp_gemm_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(args.self_test):
        ...
        args.out = args.out or "/private/tmp/412_standalone_composite_cached_pandrosion_irp_gemm_out/self_test_412.json"
    ...
    out = Path(args.out) if args.out else Path(args.outdir) / f"412_composite_cached_pandrosion_irp_gemm_{cases[0].replace(',', 'x') if cases else 'case'}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")
    print("=" * 120, flush=True)
    print("412 STANDALONE PANDROSION IRP -- COMPOSITE-CACHED CODED MATRIX-JET CALCULUS", flush=True)
