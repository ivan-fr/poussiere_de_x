#!/usr/bin/env python3
"""420 Pandrosion full-IRP compact engine, no external fallback.

This is the compact version of the 419 idea:
...
OUT = Path("/private/tmp/420_full_irp_no_fallback_out/self_test_420.json")
...
def corrector(target: Target, y0: np.ndarray, cfg: Config) -> dict[str, Any]:
    ...
    return {
        ...
        "epochs": len(history),
        "corrector": "420-full-pandrosion-irp-q-residual-cascade-no-fallback",
...
def run_self_test(cfg: Config) -> dict[str, Any]:
    ...
    return {
        "mode": "420-self-test-random-identity-full-irp",
...
def run_benchmark(cfg: Config, dims: Sequence[int], seeds: int) -> dict[str, Any]:
    ...
    return {"mode": "420-benchmark-full-irp-no-fallback", "success": True, "rows": rows}
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="420 compact standalone full Pandrosion IRP engine, no external fallback.")
...
def main() -> int:
    ...
    if bool(args.self_test):
        ...
        rec = corrector(target, y0, cfg)
        result = {
            "mode": "420-standalone-full-pandrosion-irp-no-fallback",
            "system": smeta,
