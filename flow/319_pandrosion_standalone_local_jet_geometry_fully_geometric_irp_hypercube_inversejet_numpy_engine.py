"""
319_pandrosion_standalone_local_jet_geometry_fully_geometric_irp_hypercube_inversejet_numpy_engine.py

Standalone NumPy engine that realises pandrosion as a FULLY (jet-)GEOMETRIC method.

NEW IN 319 (over 318) -- same architecture, seven structural upgrades:

  [319-A] BROYDEN-RECYCLED JETS.  Rebuilding the 2n-sample paired jet at EVERY
      correction epoch is the dominant oracle cost.  319 keeps the last Jacobian
  ...
  [319-B] DEFLATED LINE SEARCH (root repulsion).  318 could only detect a
      duplicate after drifting into a known root's neighbourhood ([318-E]).
      319 additionally DEFLATES the merit function of the line search on the
  ...
  [319-C] ADAPTIVE LEVENBERG-MARQUARDT.  The fixed --lm-damping of 318 is now a
  ...
  [319-D] PARABOLIC LINE REFINEMENT.  After the geometric lambda grid the line
  ...
  [319-E] ORACLE-SAMPLE REUSE.  The line search now returns the full residual
  ...
  [319-G] SWARM ORACLE REUSE + FAIR IRP TRIGGER.  The batched swarm prefilter
  ...
  [319-F] CERTIFIED ROOT CONDITIONING.  Every accepted root is finished with an
...
hypercube (~2n+4 points) to build a local affine/curvature jet.  The whole
...
epochs -- i.e. O(log) of the global monomial budget -- and there is NO global
precompute, so the geometry is generated in O(1).
...
def universal_atlas_start(target: TargetTrack, n: int, trial: int, seed: int, powers: Sequence[float], angles: Sequence[float], radii: Sequence[float], cap: float, roots_found: int, duplicates: int, failures: int, target_count: int, universal_cells: int = 16, universal_shells: int = 5, atlas_selection: str = "diverse-shell", **_: Any) -> tuple[Any, dict[str, Any]]:
    ...
    if mode in {"diverse", "diverse-shell", "shell", "raw-shell"}:
        ...
        meta = {
            "chart": "319-standalone-universal-diverse-shell-atlas",
            "atlas_mode": "319-diverse-shell",
...
def batched_paired_jacobians(target: TargetTrack, Y: Any, h_rel: float = 1e-5, F0: Optional[Any] = None) -> tuple[Any, Any]:
    ...
    overhead is amortised across the batch.  [319-G] a known F0 (residual vectors
...
def swarm_prefilter(target: TargetTrack, n: int, seed: int, radii: Sequence[float], size: int, iters: int, keep: int, line_lams: Sequence[float], sep: float) -> tuple[list[Any], dict[str, Any]]:
    ...
    [319-G] the batch, too, now recycles its Jacobians: the full paired jets are
    only sampled on the FIRST iteration; every later iteration applies the
    rank-1 Broyden secant update per surviving point (zero Jacobian samples),
    and the residual VECTORS from the line search are reused as the next F0.
    """
    t0 = now()
    ...
    for it in range(max(0, int(iters))):
        ...
        if idx.size == 0:
            break
        # [319-G] the residual vectors carried from the line search serve as F0, so
...
def hypercube_delta(target: TargetTrack, y: Any, f: Any, ep: int, seed: int, cloud_nodes: int = 0, lm_damping: float = 0.0, trust_radius: float = 0.0, sampling: str = "paired", A_pre: Optional[Any] = None, lm_mu: Optional[float] = None, max_order: int = 4, return_jac: bool = False) -> tuple[Any, dict[str, Any]]:
    ...
    if A_pre is not None:
        # [319-A] recycled (Broyden-updated) Jacobian: ZERO oracle samples spent here.
    ...
    if not np.all(np.isfinite(A)):
        return np.zeros_like(y), {"hypercube_error": "nonfinite_jacobian", "hypercube_order": 0, "hypercube_nodes": M, "hypercube_sampling": mode}
    ...
    # [319-C] lm_mu, when provided, is the LIVE adaptive trust parameter and
...
def hypercube_inversejet_corrector(target: TargetTrack, y0: Sequence[complex], max_epochs: int, tol: float, accept: float, trial_timeout: float, line_search: int = 12, line_grid: Sequence[float] = (), direction_seed: int = 0, cloud_nodes: int = 0, lm_damping: float = 0.0, trust_radius: float = 0.0, sampling: str = "paired", known_y: Optional[Sequence[Any]] = None, defl_alpha: float = 0.0, broyden: bool = True, jet_refresh: int = 4, adaptive_lm: bool = True, parabolic: bool = True, jet_state: Optional[dict[str, Any]] = None) -> dict[str, Any]:
    """306 hypercube inverse-jet corrector, 319 edition.

    [319-A] the local Jacobian is kept alive across epochs; after a descending
    ...
    a stall.  [319-B] on the base chart, line-search candidates are scored by a
    ...
    residual.  [319-C] the LM damping is a live trust parameter (shrinks after
    strong descent, grows before a fresh-jet retry after a rejection).
    [319-D] a single-sample parabolic refinement follows the lambda grid.
    [319-E] the winning candidate's residual VECTOR is carried into the next
    ...
    # [319-A/E] the caller may hand over the live jet state of a previous call on
    ...
    f = None
    if state is not None and state.get("y") is not None and state.get("f") is not None and np.array_equal(np.asarray(state["y"]), y):
        f = np.asarray(state["f"], dtype=np.complex128).copy()
    if f is None:
        f = target.eval(y)
    r = finite_norm(f)
    ...
    # [319-A/D] secant (Broyden) epochs use a SHORT lambda grid: a good secant step
    ...
    def defl(Y2d: Any) -> Any:
        YY = np.asarray(Y2d, dtype=np.complex128)
        W = np.ones(int(YY.shape[0]), dtype=float)
        for ky in known:
            dist = np.linalg.norm(YY - ky[None, :], axis=1)
            W *= 1.0 + alpha / np.maximum(dist, 1e-12)
        return W
    ...
    if state is not None and state.get("A") is not None:
        ...
        prev_y = np.asarray(state["y"], dtype=np.complex128) if state.get("y") is not None else None
        prev_f = np.asarray(state["f"], dtype=np.complex128) if state.get("f") is not None else None
    # [319-C] the adaptive trust parameter starts UNDAMPED (pure inverse-jet order);
    # damping only switches on after a rejected step.
    mu = float(lm_damping) if float(lm_damping) > 0.0 else 0.0
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        if r <= max(float(tol), float(accept)) and (accept <= 0 or r < accept):
            ...
            break
        rebuild = (A is None) or (not bool(broyden)) or (A_age >= refresh)
        ...
        for attempt in range(2):
            ...
            meta.pop("jac", None)
            last = dict(meta)
            total_hyper += int(meta.get("hypercube_nodes", 0)) + (7 if int(meta.get("hypercube_order", 1)) >= 2 else 0)
            used += 1
            ...
            W = defl(Y) if known else np.ones(int(Y.shape[0]), dtype=float)
            w0 = float(defl(y[None, :])[0]) if known else 1.0
            Rd = np.where(np.isfinite(R), R * W, np.inf)
            better = np.isfinite(R) & ((Rd < r * w0) | (R < best_r))
            if not np.any(better):
                if not rebuild and retries_left > 0:
                    # [319-A] a stale Broyden jet earns a fresh-jet retry.
                    rebuild = True
                    retries_left -= 1
                    continue
                if adaptive_lm and attempt == 0 and mu < mu_max and retries_left > 0:
                    # [319-C] grow the trust damping and retry once on a fresh jet.
                    mu = min(mu_max, max(mu * 5.0, 1e-3))
                    retries_left -= 1
                    continue
                status = "no-hypercube-decrease"
                break
            idx = int(np.argmin(Rd))
            lam, cand_y, cand_f, cand_r = float(L[idx]), Y[idx], F[idx], float(R[idx])
            if parabolic and math.isfinite(cand_r):
                # [319-D] one-sample parabolic refinement: fit r^2(lambda) through the
                    ...
                    if abs(den) > 1e-300:
                        ...
                        if a2 > 0.0:
                            ...
                            if math.isfinite(lam_star) and 0.0 < lam_star <= 1.5 * float(np.max(L)) and abs(lam_star - lam) > 0.05 * max(lam, 1e-12):
                                ...
                                total_line += 1
                                parabolic_evals += 1
                                wc = float(defl(yc[None, :])[0]) if known else 1.0
                                if math.isfinite(rc) and rc * wc < cand_r * float(W[idx]):
    ...
    if state is not None:
        state["A"], state["age"] = (None if A is None else np.asarray(A, dtype=np.complex128)), int(A_age)
        state["y"], state["f"] = y.copy(), (None if f is None else np.asarray(f, dtype=np.complex128).copy())
...
def lazy_irp_hypercube_inversejet_corrector_312(base: TargetTrack, y0: Sequence[complex], max_epochs: int, tol: float, accept: float, trial_timeout: float, line_search: int = 12, line_grid: Sequence[float] = (), direction_seed: int = 0, cloud_nodes: int = 0, irp_layers: int = 2, irp_inner_epochs: int = 2, irp_scales: Optional[Sequence[complex]] = None, irp_chart_top: int = 2, irp_inversion: bool = True, collapse: bool = True, collapse_residual: float = 1e-4, collapse_drop: float = 0.42, collapse_rel_step: float = 0.35, collapse_after: int = 2, local_inner_epochs: int = 3, lazy_direct_epochs: int = 1, lazy_trigger_drop: float = 0.82, lazy_trigger_after: int = 1, lazy_bad_cond: float = 1e10, lazy_log_energy: float = 8.0, eager_irp: bool = False, rescue_collapsed: bool = False, lm_damping: float = 0.0, trust_radius: float = 0.0, sampling: str = "paired", known_y: Optional[Sequence[Any]] = None, dup_sep: float = 0.0, ghost_escapes: int = 2, ghost_kick: float = 0.45, ghost_gate: float = 1e4, defl_alpha: float = 0.0, broyden: bool = True, jet_refresh: int = 4, adaptive_lm: bool = True, parabolic: bool = True) -> dict[str, Any]:
    ...
    jet_state: dict[str, Any] = {}  # [319-A] Broyden jet handed over between inner-corrector calls.
    last: dict[str, Any] = {}
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        if r0 <= max(float(tol), float(accept)) and (accept <= 0 or r0 < accept):
            ok, status = True, "converged"
            break
        loc = hypercube_inversejet_corrector(base, y, max(1, int(local_inner_epochs if collapsed else lazy_direct_epochs)), tol, accept, 0.0 if deadline is None else max(0.0, deadline - now()), line_search, line_grid, int(direction_seed) + 1000003 * ep + 17, cloud_nodes, lm_damping, trust_radius, sampling, known_y=(known if float(defl_alpha) > 0.0 else None), defl_alpha=float(defl_alpha), broyden=bool(broyden), jet_refresh=int(jet_refresh), adaptive_lm=bool(adaptive_lm), parabolic=bool(parabolic), jet_state=jet_state)  # [319-A/B/C/D/E]
        absorb(loc)
        last = dict(loc)
        ...
        if improved:
            ...
            stagnation_hits += 1
        # [319-A] the drop threshold is judged PER inner epoch (geometric compounding),
        ...
        if not trigger:
            ...
            if ghost_left > 0 and (accept <= 0 or not (math.isfinite(best_r) and best_r < accept)) and math.isfinite(best_r) and (best_r <= ghost_gate_r or best_r <= 1e-3 * max(r_init, 1e-300)):
                ...
                jet_state.clear()  # [319-A] the kick is a discontinuous jump: the secant is void.
                epochs_done = ep + 1
                status = "ghost-escape"
                continue
        ...
        for layer in range(max(1, int(irp_layers))):
            ...
            for _, ct, u0, cm in shortlist:
                loc2 = hypercube_inversejet_corrector(ct, u0, max(1, int(irp_inner_epochs)), tol, accept, 0.0, line_search, line_grid, int(direction_seed) + 1000003 * ep + 65537 * layer, cloud_nodes, lm_damping, trust_radius, sampling, broyden=bool(broyden), jet_refresh=int(jet_refresh), adaptive_lm=bool(adaptive_lm), parabolic=bool(parabolic))
                absorb(loc2)
        ...
        if any_irp and irp_r < r0:
            y = irp_y.copy()
            jet_state.clear()  # [319-A] chart round-trip: the base-chart secant is void.
            epochs_done = ep + 1
...
def polish_geometric_root(geometry: Any, chart: LinearChart, y: Any, accept: float, max_steps: int) -> tuple[Any, Any, float, dict[str, Any]]:
    ...
    # [319-F] certified root conditioning: SVD of the final local jet at the root.
    try:
        ...
        if S.size:
            ...
            meta["root_smin"] = smin
            meta["root_cond"] = float(smax / max(smin, 1e-300))
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    if bool(args.swarm) and int(args.count) > 0 and not user_starts:
        ...
        swarm_starts_list, swarm_meta = swarm_prefilter(
            target, n, geometry.seed + 0x319000, parse_float_list(args.rays, DEFAULT_RADII, positive=True),
    ...
    priority_starts: list[tuple[str, Any]] = (
        [("319-explicit-start", s) for s in user_starts]
        + [("319-swarm-start", s) for s in swarm_starts_list]
        + [("319-singular-direction-seed", s) for s in eigen_starts]
    )
    starts = user_starts
    ...
    for trial in range(int(args.pool)):
        ...
        if explicit:
            ...
            if tag == "319-swarm-start" and roots:
                # [319-B/E] swarm survivors have already descended several Newton
                ...
                if dmin <= skip_sep:
                    duplicates += 1
                    trials.append({"trial": int(trial), "accepted": False, "status": "swarm-start-skipped-near-root", "r1": float("nan"), "epochs": 0, "seconds": 0.0, "chart": tag, "skip_distance": dmin})
                    continue
            geom = {"chart": tag, "atlas_mode": tag, "homothety": 1.0, "theta_deg": None}
            if tag in {"319-swarm-start", "319-singular-direction-seed"}:
            ...
            if bool(args.origin_seed) and (trial == 0 if int(args.origin_seed_period) <= 0 else trial % max(1, int(args.origin_seed_period)) == 0):
                ...
                if y_origin is not None:
                    y_raw = y_origin
                    geom = {"chart": "319-origin-affine-finite-difference-seed", "atlas_mode": "origin-affine-seed", "homothety": 1.0, "theta_deg": None, **ometa}
            if y_raw is None:
        ...
        if dup is not None:
            ...
            trials.append(rec)
            continue
        root = {"id": len(roots), "source": "319-standalone-local-jet-geometry-fully-geometric", "trial": int(trial), "z_complex": np.asarray(z, dtype=np.complex128).copy(), "y_complex": np.asarray(y_final, dtype=np.complex128).copy(), "residual": float(geo_residual), "geometric_residual": float(geo_residual), "realness": realness(z), "cond": (float(polish_meta["root_cond"]) if polish_meta.get("root_cond") else None), "epochs": int(loc.get("epochs", 0)), "seconds": float(loc.get("seconds", 0.0)), **{k: v for k, v in loc.items() if k != "y"}, **geom, **smeta, **polish_meta}
    ...
    result = {
        ...
        "mode": "319-standalone-local-jet-geometry-fully-geometric-swarm-lazy-irp-hypercube-inversejet",
        "flow_formula": "any system as black-box geometric oracle -> lazily-sampled local-jet field (paired central differences + [319-A] Broyden-recycled jets, O(n) samples on rebuild epochs / O(1) on secant epochs, no global precompute) -> [318-B] batched swarm prefilter -> universal Mobius atlas + singular-direction starts -> StartOpt -> 312 lazy IRP (kind-diverse charts, ghost escapes) / 306 hypercube inverse-jet ([319-B] deflated line search, [319-C] adaptive LM, [319-D] parabolic refinement, [319-E] oracle-sample reuse, SVD fallback) -> early duplicate abort -> jet-Newton polish + [319-F] certified root conditioning, ALL on the jet geometry -> faithful (jet) residual acceptance",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="319 standalone NumPy local-jet-geometry fully-geometric swarm + lazy IRP / hypercube inverse-jet engine.")
    ...
    # [319-A] Broyden-recycled jets.
    p.add_argument("--broyden", dest="broyden", action="store_true", default=True, help="[319-A] Recycle the local Jacobian across epochs with rank-1 Broyden secant updates; full paired jets only every --jet-refresh epochs or after a stall.")
    p.add_argument("--no-broyden", dest="broyden", action="store_false", help="Rebuild the full paired jet at every epoch (318 behaviour).")
    p.add_argument("--jet-refresh", type=int, default=4, help="[319-A] Epochs between full paired-jet rebuilds when --broyden is on.")
    # [319-B] deflated line search.
    p.add_argument("--deflation-alpha", type=float, default=0.15, help="[319-B] Root-repulsion radius of the deflated line-search merit r*prod(1+alpha/||y-y_k||) on the base chart (0 = off). Acceptance stays on the raw residual.")
    # [319-C] adaptive Levenberg-Marquardt.
    p.add_argument("--adaptive-lm", dest="adaptive_lm", action="store_true", default=True, help="[319-C] Live LM trust parameter: shrinks after strong descent, grows before a fresh-jet retry after a rejection.")
    p.add_argument("--no-adaptive-lm", dest="adaptive_lm", action="store_false", help="Static --lm-damping only (318 behaviour).")
    # [319-D] parabolic line refinement.
    p.add_argument("--parabolic-line", dest="parabolic_line", action="store_true", default=True, help="[319-D] Spend one extra oracle sample on the parabolic vertex of the line search.")
    ...
    p.add_argument("--lazy-direct-epochs", type=int, default=2, help="[319-A] Inner direct-corrector epochs per outer 312 epoch (319 default 2: the Broyden secant needs a compound drop before the IRP trigger judges it).")
    ...
    p.add_argument("--outdir", default="/tmp/319_standalone_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(args.self_test):
        ...
        args.out = args.out or "/tmp/319_standalone_out/self_test_319.json"
    ...
    out = Path(args.out) if args.out else Path(args.outdir) / f"319_standalone_{cases[0].replace(',', 'x') if cases else 'case'}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")
    print("=" * 120, flush=True)
    print("319 STANDALONE LOCAL-JET GEOMETRY -- FULLY GEOMETRIC SWARM + LAZY IRP / HYPERCUBE INVERSE-JET", flush=True)
    print("NumPy + stdlib only; no local flow imports. Broyden-recycled jets, deflated (root-repelling) line search,", flush=True)
    print("adaptive LM trust, parabolic line refinement, oracle-sample reuse, certified root conditioning; residual is faithful (true oracle).", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
        g = r.get("geometry", {})
