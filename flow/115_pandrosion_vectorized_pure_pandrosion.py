#!/usr/bin/env python3
"""
115_pandrosion_vectorized_pure_thales_engine.py

Autonomous vectorized PURE Pandrosion Thales root extractor.
...
class DenseKostlanSystem:
    ...
    def slope_matrix(self, a: Sequence[complex], b: Sequence[complex]) -> "np.ndarray":
        ...
        This preserves 114's pure Pandrosion formula while removing the slow
...
def pandrosion_corrector(
    ...
    """Pure derivative-free Pandrosion corrector.

    This corrector does not call or build a Jacobian.  At the current point a it
    builds a second geometric endpoint b and constructs the exact telescopic
    slope Q_G(a,b) satisfying

        G(b) - G(a) = Q_G(a,b) @ (b-a).

    The Pandrosion update is then

        Q_G(a,b) delta = -G(a),      a_next = a + lambda * delta.

    The damping lambda is a Thales proportional line-search inside the same
    update formula; there is no fallback to Newton, least-squares, homotopy, or
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        # Single universal endpoint formula for the slope.  It mixes inertial
        # motion with a deterministic complex direction so Q is not a collapsed
        # infinitesimal derivative.  The probe shrinks slowly as the residual
        # decreases, which implements the geometric "fewer Thales parallels"
        # idea while staying derivative-free.
        qdir = raw_direction(n, direction_seed + 104729 * (ep + 1), direction_seed ^ 0x114114, True)
        qnorm = max(1e-300, float(np.linalg.norm(qdir)))
        qdir = qdir / qnorm * math.sqrt(max(1, n))
        if prev_delta is None or (not np.all(np.isfinite(prev_delta))):
            inertia = (float(probe_scale) * ynorm) * qdir
        else:
            pdn = max(1e-300, float(np.linalg.norm(prev_delta)))
            inertia = prev_delta / pdn * min(max(pdn, float(probe_scale) * ynorm), 2.5 * ynorm)
        residual_factor = 1.0 / (1.0 + 0.10 * ep + 0.03 * math.log1p(max(0.0, r)))
        thales_probe = max(1e-12, float(probe_scale) * ynorm * residual_factor)
        endpoint_step = 0.72 * inertia + thales_probe * phase(0.61803398875 * (ep + 1)) * qdir
        # Avoid any coordinate endpoint collapse; this preserves the exact slope
        # identity numerically without switching algorithm.
        tiny = 1e-10 * ynorm
        for j in range(n):
            if abs(endpoint_step[j]) < tiny:
                endpoint_step[j] += tiny * phase(0.37 + j + ep)
        b = y + endpoint_step
        try:
    ...
    return {
        ...
        "corrector": "pure-pandrosion-exact-telescopic-slope",
    }
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    # Fully autonomous 114 keeps A simple and robust; the projective/homothetic flow provides the main geometry.
    ...
    for trial in range(int(args.pool)):
        ...
        loc = pandrosion_corrector(target, y0, max_epochs=int(args.epochs), tol=float(args.tol), accept=float(args.accept), trial_timeout=float(args.trial_timeout), line_search=int(args.line_search), probe_scale=float(getattr(args, "probe_scale", 0.035)), direction_seed=system.seed + 7919 * trial)
        ...
        roots.append({
            "id": rid,
            "source": "115-autonomous-vectorized-pure-pandrosion-single-flow",
            "trial": int(trial),
    ...
    result = {
        "script": "115_pandrosion_vectorized_pure_thales_engine.py",
        "autonomous": True,
        "dependencies": {"python_scripts": [], "numpy": bool(np is not None)},
        "mode": "autonomous-vectorized-pure-pandrosion-single-flow/thales-power-riemann-startopt-exact-slope",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--probe-scale", type=float, default=0.035, help="pure Pandrosion endpoint scale for Q(a,b)")
    ...
    p.add_argument("--outdir", default="/mnt/data/115_vectorized_pure_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "115_pandrosion_vectorized_pure_thales_engine.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
        first = cases[0].replace(",", "x") if cases else "case"
        out = Path(args.outdir) / f"115_vectorized_pure_{first}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")

    print("=" * 120, flush=True)
    print("115 autonomous VECTORIZED PURE Thales Pandrosion - single flow, no fallback", flush=True)
    print("No dependency on previous Python scripts; vectorized pure exact-slope Q(a,b); no Newton/Jacobian fallback path.", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
