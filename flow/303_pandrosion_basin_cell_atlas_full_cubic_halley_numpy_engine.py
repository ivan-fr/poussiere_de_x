#!/usr/bin/env python3
"""
303_pandrosion_basin_cell_atlas_full_cubic_halley_numpy_engine.py
...
def basin_cell_atlas_start(
    ...
    target_count: int,
    atlas_cells: int = 12,
    cell_probe_radius: float = 0.16,
    cell_descent_min: float = 1.03,
    cell_log_max: float = 64.0,
    cell_equal_gap_min: float = 1e-8,
    cell_cycle: bool = True,
) -> tuple[Any, dict[str, Any]]:
    """303 no-score basin-cell atlas.

    This is deliberately not a ranking layer.  It constructs candidate chart
    cells and accepts cells satisfying geometric predicates:
      1. finite residual;
      2. controlled logarithmic scale;
      3. at least one local probe decreases residual by cell_descent_min;
      4. no near equal-value pole F(probe) ~= F(cell center).

    Among admissible cells, selection is cyclic/deterministic to preserve basin
    diversity.  There is no scalar objective and no argmin score.
    """
    ensure_numpy()
    m = max(1, int(atlas_cells))
    cell_records = []
    for k in range(m):
        y, meta = mobius_homothety_start(
            n, trial * m + k, seed + 0x303CE11 + 104729 * k,
        ...
        fresh = raw_direction(n, trial + 8191 * (k + 1), seed + 0x303D17 + k, True)
        fresh = np.asarray(fresh, dtype=np.complex128)
        fresh = fresh / max(1e-300, float(np.linalg.norm(fresh))) * ynorm
        probes = [0.90*y, 1.10*y, y + float(cell_probe_radius)*fresh, y - float(cell_probe_radius)*fresh]
        ...
        descent = (r0 / min_r) if min_r > 0 and math.isfinite(min_r) else 0.0
        ...
        admissible = bool(descent >= float(cell_descent_min) and min_gap >= float(cell_equal_gap_min))
        if admissible:
            # Cell representative is the first probe that strictly decreases;
            # not the best probe.  This keeps the rule predicate-based, not
            # score-based.
            rep = y.copy()
            for ii, rr in enumerate(RP):
                if math.isfinite(float(rr)) and float(rr) < r0 / float(cell_descent_min):
                    ...
                    break
            cell_records.append({
                "index": int(k), "y": rep, "meta": meta, "residual": float(r0),
                "log_energy": float(log0), "descent_ratio": float(descent),
                "equal_gap_min": float(min_gap), "representative_changed": bool(not np.allclose(rep, y)),
            })

    if cell_records:
        # Cyclic admissible-cell choice: rotate through cells as roots/duplicates
        # accumulate, so the atlas covers basin doors instead of collapsing to
        # the numerically prettiest one.
        pick = (trial + roots_found + 2 * duplicates) % len(cell_records) if bool(cell_cycle) else 0
        rec = cell_records[int(pick)]
        meta = dict(rec["meta"])
        meta.update({
            "atlas_mode": "303-no-score-basin-cell-atlas",
            "atlas_cells": int(m),
            "atlas_admissible_cells": int(len(cell_records)),
            "atlas_selected_cell": int(rec["index"]),
            "atlas_selection_rule": "cyclic-admissible-cell" if bool(cell_cycle) else "first-admissible-cell",
            ...
            "chart": "303-no-score-cell-atlas/" + str(meta.get("chart", "single-flow")),
        })
        return np.asarray(rec["y"], dtype=np.complex128).copy(), meta

    # If no cell is certified admissible, still advance deterministically with
    # the base 301 start, but mark it as an atlas miss.  This is not selected by
    # score; it is the default chart cell.
    y, meta = mobius_homothety_start(
        n, trial, seed, powers, angles, radii, cap,
        roots_found=roots_found, duplicates=duplicates, failures=failures,
        target_count=target_count,
    )
    meta.update({
        "atlas_mode": "303-no-score-basin-cell-atlas",
        "atlas_cells": int(m),
        "atlas_admissible_cells": 0,
        "atlas_selected_cell": None,
        "atlas_selection_rule": "default-301-cell-after-no-admissible-cell",
        "chart": "303-no-score-cell-atlas/default-301",
    })
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    for trial in range(int(args.pool)):
        if len(roots) >= int(args.count):
            break
        y_raw, geom = basin_cell_atlas_start(
            ...
            atlas_cells=int(args.atlas_cells), cell_probe_radius=float(args.cell_probe_radius),
            cell_descent_min=float(args.cell_descent_min), cell_log_max=float(args.cell_log_max),
            cell_equal_gap_min=float(args.cell_equal_gap_min), cell_cycle=bool(args.cell_cycle)
        )
        ...
        roots.append({
            "id": rid,
            "source": "303-basin-cell-atlas-full-cubic-halley",
            "trial": int(trial),
    ...
    result = {
        "script": "303_pandrosion_basin_cell_atlas_full_cubic_halley_numpy_engine.py",
        "autonomous": True,
        "dependencies": {"python_scripts": [], "numpy": bool(np is not None)},
        "mode": "303-basin-cell-atlas-full-cubic-halley",
        ...
        "parameters": {
            ...
            "atlas_cells": int(args.atlas_cells),
            ...
            "cell_cycle": bool(args.cell_cycle),
        },
        "roots": encoded_roots,
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="303 standalone NumPy Pandrosion engine: no-score basin-cell atlas plus anchored full cubic Halley.")
    ...
    p.add_argument("--atlas-cells", type=int, default=12, help="303: number of chart cells tested by predicate, not score")
    p.add_argument("--cell-probe-radius", type=float, default=0.16, help="303: transverse/radial cell probe radius")
    p.add_argument("--cell-descent-min", type=float, default=1.03, help="303: required residual descent ratio for an admissible cell")
    p.add_argument("--cell-log-max", type=float, default=64.0, help="303: maximum logarithmic scale energy for admissible cells")
    p.add_argument("--cell-equal-gap-min", type=float, default=1e-8, help="303: minimum equal-value pole gap for admissible cells")
    p.add_argument("--cell-cycle", action="store_true", default=True, help="303: cycle through admissible cells instead of taking the first")
    p.add_argument("--no-cell-cycle", dest="cell_cycle", action="store_false")
    p.add_argument("--out", "--thales-out", "--useful-out", default=None)
    p.add_argument("--outdir", default="/mnt/data/303_basin_cell_atlas_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(getattr(args, "self_test", False)):
        ...
        args.out = args.out or "/mnt/data/303_basin_cell_atlas_out/self_test_303.json"
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "303_pandrosion_basin_cell_atlas_full_cubic_halley_numpy_engine.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
        first = cases[0].replace(",", "x") if cases else "case"
        out = Path(args.outdir) / f"303_basin_cell_atlas_{first}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")

    print("=" * 120, flush=True)
    print("303 autonomous NO-SCORE BASIN-CELL ATLAS + ANCHORED FULL-CUBIC HALLEY NumPy Pandrosion", flush=True)
