#!/usr/bin/env python3
"""421 Pandrosion full-IRP compact engine, fast full-n, no external fallback.
...
421 adds one important performance fix: when the cascade reaches the final
...
OUT = Path("/private/tmp/421_full_irp_fast_fulln_out/self_test_421.json")
...
def corrector(target: Target, y0: np.ndarray, cfg: Config) -> dict[str, Any]:
    ...
    return {
        ...
        "epochs": len(history),
        "corrector": "421-full-pandrosion-irp-q-residual-cascade-fast-fulln-no-fallback",
...
def run_self_test(cfg: Config) -> dict[str, Any]:
    ...
    return {
        "mode": "421-self-test-random-identity-fast-fulln-irp",
...
def run_benchmark(cfg: Config, dims: Sequence[int], seeds: int) -> dict[str, Any]:
    ...
    return {"mode": "421-benchmark-full-irp-fast-fulln-no-fallback", "success": True, "rows": rows}
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="421 compact standalone full Pandrosion IRP engine with fast full-n identity stage, no external fallback.")
...
def main() -> int:
    ...
    if bool(args.self_test):
        ...
        rec = corrector(target, y0, cfg)
        result = {
            "mode": "421-standalone-full-pandrosion-irp-fast-fulln-no-fallback",
            "system": smeta,
