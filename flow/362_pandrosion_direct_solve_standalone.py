"""Pandrosion 362: pure finite-slope root solver with a hard sample cap.

The corrector has one numerical path: at every epoch it rebuilds an exact
coordinate-telescopic matrix satisfying F(b)-F(a)=Q(a,b)(b-a), solves that
finite-slope model, and selects one of five dyadic steps.  Root-equivalent
oracle normalizations are frozen inside each chart.  There are no derivatives,
homotopies, quasi-Newton updates, rescue solvers, restarts, or SciPy calls.
"""
...
def _pure_telescope(target: Target, x: np.ndarray, fx: np.ndarray, radius: float,
    ...
    n = len(x)
    b = x + radius*direction(n, epoch, seed)/math.sqrt(max(1, n))
    dz = b-x
...
def _pure_slope_step(Q: np.ndarray, f: np.ndarray, x: np.ndarray,
                     cap_factor: float) -> tuple[np.ndarray, str]:
    ...
    try:
        step = np.linalg.solve(q, rhs)
        mode = "pure-pandrosion-telescopic-solve"
    except Exception:
        return np.zeros_like(x), "failed"
    ...
    if size > cap:
        step = step*(cap/size)
    return np.asarray(step, np.complex128), mode
...
def correct(target: Target, y0: Any, args: argparse.Namespace, known: Sequence[np.ndarray],
    ...
    """362: one pure Pandrosion trajectory under the hard 40*n+100 cap.

    Every model is rebuilt from an exact finite telescopic identity.  The five
    dyadic candidates are steps of that same model; there is no homotopy,
    fallback, restart, residual escape, Broyden update, or derivative path.
    """
    del known
    n = target.oracle.n
    ...
    lines = floats(args.line_grid, [1.0, .5, .25, .125, .0625])
    lines = np.asarray(lines[:5], float)
    if len(lines) != 5:
        lines = np.asarray([1.0, .5, .25, .125, .0625], float)
    seed = mix64(int(target.oracle.seed)+0x362A71A5)
    ...
    for ep in range(max(1, int(epochs))):
        ...
        if used+n+len(lines) > budget:
            status = "strict-budget-exhausted"
            break
        try:
            Q, reference, defect, slope_used = _pure_telescope(
                target, x, fx,
                float(args.pandrosion_radius)*(10.0**min(singular_rebuilds, 4)),
                ep, seed,
            )
        ...
        step, mode = _pure_slope_step(Q, fx, x, float(args.pandrosion_step_cap))
        if mode == "failed" or not np.all(np.isfinite(step)) or norm(step) <= 1e-15:
            # A black-box difference can round to zero at a stationary point.
            # Stay in the same algorithm and rebuild the next exact telescope
            # with the next deterministic chart direction.
            status = "singular-telescope-rebuild"
            singular_rebuilds += 1
            done = ep+1
            continue
        singular_rebuilds = 0
        Y = x[None, :]+lines[:, None]*step[None, :]
        normalized = target.eval_batch(Y)
        line_evals += len(Y)
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    return {"script":Path(__file__).name,"version":362,"standalone":True,"case":f"{n},{d}","n":n,"degree":d,
      "method":{"family":"pure-pandrosion-frozen-chart-telescopic-slopes","derivatives":False,"jacobian":False,"newton":False,
        ...
        "globalization":"five dyadic steps from the same finite-slope model"},
...
def parser() -> argparse.ArgumentParser:
    p=argparse.ArgumentParser(description="Pandrosion 362 pure telescopic finite-slope engine")
    ...
    p.add_argument("--out"); p.add_argument("--outdir",default="/tmp/362_pandrosion")
    return p
...
def self_test(args: argparse.Namespace) -> dict[str,Any]:
    ...
    checks.append({"name":"two-roots-pure-pandrosion","passed":roots["summary"]["success"],
    ...
    checks.append({"name":"stationary-coupled-pure-pandrosion","passed":coupled_loc.ok
    ...
    checks.append({"name":"multiple-root-pure-pandrosion","passed":multiple_loc.ok,
    ...
    checks.append({"name":"pure-method-audit","passed":all(not flags[k] for k in forbidden),
        "method":flags})
...
def main(argv:Optional[Sequence[str]]=None)->int:
    ...
    out=Path(args.out) if args.out else Path(args.outdir)/("self_test.json" if args.self_test else f"362_{cases[0].replace(',','x')}.json")
    ...
    if args.self_test:
        print(f"362 self-test: {'PASS' if final['passed'] else 'FAIL'} ({sum(int(c['passed']) for c in final['checks'])}/{len(final['checks'])})")
        print(f"out={out}"); return 0 if final["passed"] else 1
    for result in (results if len(cases)>1 else [final]):
        s=result["summary"]; print(f"362 case={result['case']} oracle={result['oracle']['kind']} roots={s['unique']}/{s['requested']} trials={s['trials']} samples={s['oracle_samples']} seconds={s['seconds']:.3f}")
