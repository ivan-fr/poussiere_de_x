#!/usr/bin/env python3
"""Large diagnostic benchmark for 306 Universal Atlas + Hypercube Inverse-Jet Pandrosion.

No Docker. Runs the standalone 306 engine over a Smale-17-style Kostlan suite,
...
OUTDIR = ROOT / "benchmarks" / "306_smale17_large"
...
def load_engine():
    spec = importlib.util.spec_from_file_location("pandrosion306", ENGINE)
...
def main() -> int:
    ...
    for n, d in CASES:
        ...
        for seed in SEEDS:
            run_no += 1
            out = RUNS_DIR / f"306_ks{n}x{d}_seed{seed}.json"
            cmd = [sys.executable, str(ENGINE), "--cases", f"{n},{d}", "--seed-index", str(seed), "--count", str(count), "--out", str(out), *BASE_ARGS]
            print(f"RUN {run_no}/{total} 306 ks({n},{d}) seed={seed} count={count}", flush=True)
            t = time.time()
            ...
            row = {
                "engine": 306, "n": n, "d": d, "seed": seed, "count": count,
            ...
            if proc.returncode != 0:
                (RUNS_DIR / f"306_ks{n}x{d}_seed{seed}.log").write_text(proc.stdout[-20000:])
...
def write_outputs(rows, total_runs, elapsed):
    ...
    report = []
    report.append("# Large 306 universal-atlas hypercube-inversejet Smale-17 diagnostic benchmark\n")
    report.append("Empirical stress benchmark for the fixed universal atlas plus hypercube inverse-jet idea. This is not a proof and not a homotopy baseline.\n")
