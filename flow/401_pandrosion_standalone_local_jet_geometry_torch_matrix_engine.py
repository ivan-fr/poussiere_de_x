#!/usr/bin/env python3
"""
401_pandrosion_standalone_local_jet_geometry_torch_matrix_engine.py

Standalone PyTorch/NumPy engine that reuses the 317 local-jet geometry but turns
the local root search into a Torch-backed MATRIX COMPUTATION.
...
Mathematical idea:
  ...
  2. Do not call a multivariate algebraic solver, Groebner/resultant machinery, or a
     ...
     With the SVD J = U diag(sigma) V*, the correction is

         delta = - V diag(sigma / (sigma^2 + mu)) U* F(z),

     with spectral truncation for tiny singular values and an optional trust radius.

  3. The quadratic/cubic matrix-jet defect terms from 317 are kept, but every occurrence of
     "invert this local linear system" is routed through the same SVD matrix filter.
...
resolution.  The 401 difference from 400 is that the SVD/pseudo-inverse kernels can
run through PyTorch (`torch.linalg.svd`) with CPU/MPS/CUDA device selection, while a
NumPy fallback remains available for exact compatibility.
...
TORCH_RESOLVED_DEVICE = "cpu"
TORCH_RESOLVED_DTYPE = "complex128"
...
def resolve_torch_device(raw: str) -> str:
    ...
    if mode == "auto":
        if torch.cuda.is_available():
            return "cuda"
        # As of this torch build, torch.linalg.svd on MPS falls back to CPU.
        # Prefer CPU automatically for the SVD-heavy 401 matrix kernel; explicit
        # --torch-device mps is still accepted for experimentation/future builds.
        return "cpu"
...
def configure_matrix_backend(args: argparse.Namespace) -> None:
    global MATRIX_BACKEND, TORCH_DEVICE, TORCH_COMPLEX_DTYPE, TORCH_RESOLVED_DEVICE, TORCH_RESOLVED_DTYPE
...
def matrix_svd_step(A: Any, rhs: Any, rcond: float = 1e-12, condition_cap: float = 1e12, damping: float = 0.0) -> tuple[Any, dict[str, Any]]:
    """Apply the regularised pseudo-inverse of A to rhs using only matrix SVD.

    The returned vector is the minimiser of ||A x - rhs||^2 + mu ||x||^2 on the
    retained singular subspace.  ``damping`` is dimensionless; internally
    mu = damping * mean(sigma^2), so the scale follows the sampled jet.
    """
...
def matrix_least_squares_fit(X: Any, Y: Any, rcond: float = 1e-12, condition_cap: float = 1e12) -> tuple[Any, dict[str, Any]]:
    """Fit B in X @ B ~= Y by SVD pseudo-inverse, not by a solver call."""
    if MATRIX_BACKEND in {"torch", "auto"}:
        try:
...
def make_system_401(args: argparse.Namespace, n: int, d: int) -> tuple[LocalJetGeometry, Any, str, str]:
...
def universal_atlas_start(target: TargetTrack, n: int, trial: int, seed: int, powers: Sequence[float], angles: Sequence[float], radii: Sequence[float], cap: float, roots_found: int, duplicates: int, failures: int, target_count: int, universal_cells: int = 16, universal_shells: int = 5, atlas_selection: str = "diverse-shell", **_: Any) -> tuple[Any, dict[str, Any]]:
    ...
    if mode in {"diverse", "diverse-shell", "shell", "raw-shell"}:
        ...
        meta = {
            "chart": "401-standalone-universal-diverse-shell-atlas",
            "atlas_mode": "401-diverse-shell",
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        if not trigger:
            if improved:
                ...
                status = "local-collapsed" if collapsed else "direct-401-torch-matrixjet-running"
                continue
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    geometry, base, source, backend = make_system_401(args, n, d)
    ...
    for trial in range(int(args.pool)):
        ...
        if explicit:
            ...
            geom = {"chart": "401-explicit-start", "atlas_mode": "explicit-start", "homothety": 1.0, "theta_deg": None}
        else:
            y_raw = None
            geom: dict[str, Any] = {}
            if bool(args.origin_seed) and (trial == 0 if int(args.origin_seed_period) <= 0 else trial % max(1, int(args.origin_seed_period)) == 0):
                ...
                if y_origin is not None:
                    y_raw = y_origin
                    geom = {"chart": "401-origin-affine-torch-matrix-seed", "atlas_mode": "origin-affine-torch-matrix-seed", "homothety": 1.0, "theta_deg": None, **ometa}
            if y_raw is None:
    ...
    result = {
        ...
        "mode": "401-standalone-local-jet-geometry-torch-matrix-svd",
        "flow_formula": "any system as black-box geometric oracle -> lazily-sampled local-jet field (O(n)/step, no global precompute) -> universal Mobius atlas starts -> StartOpt -> lazy IRP charts -> hypercube Torch matrix-jet SVD correction + matrix polish, ALL on the jet geometry -> faithful residual acceptance",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="401 standalone PyTorch/NumPy local-jet-geometry matrix/SVD engine.")
    ...
    p.add_argument("--matrix-backend", choices=["torch", "numpy", "auto"], default="torch", help="Matrix kernel backend. torch uses torch.linalg.svd; numpy preserves 400 behavior.")
    p.add_argument("--torch-device", choices=["auto", "cpu", "mps", "cuda"], default="auto", help="Torch device for matrix kernels. auto uses CUDA when available, otherwise CPU because SVD on MPS currently falls back to CPU.")
    ...
    p.add_argument("--outdir", default="/private/tmp/401_standalone_torch_matrix_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(args.self_test):
        ...
        args.out = args.out or "/private/tmp/401_standalone_torch_matrix_out/self_test_401.json"
    ...
    out = Path(args.out) if args.out else Path(args.outdir) / f"401_torch_matrix_{cases[0].replace(',', 'x') if cases else 'case'}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")
    print("=" * 120, flush=True)
    print("401 STANDALONE LOCAL-JET GEOMETRY -- PYTORCH MATRIX/SVD CALCULUS", flush=True)
    print("PyTorch + NumPy + stdlib only; no local flow imports. The geometry is 317's lazily-sampled local-jet field;", flush=True)
    print(f"matrix_backend={MATRIX_BACKEND}, torch={torch.__version__}, device={TORCH_RESOLVED_DEVICE}, complex_dtype={TORCH_RESOLVED_DTYPE}", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
        g = r.get("geometry", {})
