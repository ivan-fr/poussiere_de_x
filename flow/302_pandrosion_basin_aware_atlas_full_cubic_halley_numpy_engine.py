#!/usr/bin/env python3
"""
302_pandrosion_basin_aware_atlas_full_cubic_halley_numpy_engine.py
...
def basin_aware_atlas_start(
    ...
    failures: int,
    target_count: int,
    atlas_candidates: int = 8,
    atlas_refine_top: int = 4,
    atlas_probe_radius: float = 0.18,
    atlas_log_weight: float = 0.002,
    atlas_descent_weight: float = 0.08,
) -> tuple[Any, dict[str, Any]]:
    """302 basin-gateway atlas selection before local correction.

    301 samples one Mobius/Riemann/homothety start per trial.  302 samples a
    small atlas packet of starts in different charts/scales, evaluates the
    geometry of F at those starts, then chooses the best basin doorway.  This is
    still not homotopy and not a solver fallback: it is a pre-corrector map of
    the residual/log-scale/descent geometry of the same system F.
    """
    ensure_numpy()
    m = max(1, int(atlas_candidates))
    starts = []
    metas = []
    for k in range(m):
        yy, meta = mobius_homothety_start(
            n, trial * m + k, seed + 0x302A77 + 104729 * k,
            powers, angles, radii, cap,
            roots_found=roots_found, duplicates=duplicates, failures=failures,
            target_count=target_count,
        )
        starts.append(np.asarray(yy, dtype=np.complex128))
        metas.append(meta)
    Y = np.asarray(starts, dtype=np.complex128)
    R = target.residuals_batch(Y)
    logE = _log_stability_batch_201(Y)
    base_scores = np.asarray([
        math.log1p(max(0.0, float(R[i]))) + float(atlas_log_weight) * math.log1p(float(logE[i]))
        if math.isfinite(float(R[i])) else float('inf')
        for i in range(m)
    ], dtype=float)

    refine_top = max(1, min(int(atlas_refine_top), m))
    order = np.argsort(base_scores)
    best_idx = int(order[0]) if len(order) else 0
    best_score = float(base_scores[best_idx]) if len(base_scores) else float('inf')
    best_descent_ratio = None
    refined = 0
    evals = int(len(Y))

    for idx0 in order[:refine_top]:
        idx = int(idx0)
        y = Y[idx]
        r0 = float(R[idx]) if idx < len(R) else float('inf')
        if not math.isfinite(r0):
            continue
        ynorm = max(1.0, float(np.linalg.norm(y)))
        probes = [y]
        # local basin-door probes: radial shrink/expand plus one fresh complex
        # transverse direction.  A true basin doorway has nearby lower residual
        # and stable log scale, before the cubic corrector is even called.
        probes.append(0.85 * y)
        probes.append(1.15 * y)
        fresh = raw_direction(n, trial + 7919 * (idx + 1), seed + 0x302F00 + idx, True)
        fresh = np.asarray(fresh, dtype=np.complex128)
        fresh = fresh / max(1e-300, float(np.linalg.norm(fresh))) * ynorm
        probes.append(y + float(atlas_probe_radius) * fresh)
        probes.append(y - float(atlas_probe_radius) * fresh)
        P = np.asarray(probes, dtype=np.complex128)
        RP = target.residuals_batch(P)
        evals += int(len(P))
        LP = _log_stability_batch_201(P)
        min_r = float(np.nanmin(RP)) if np.any(np.isfinite(RP)) else float('inf')
        min_log = float(np.nanmin(LP)) if np.any(np.isfinite(LP)) else float(logE[idx])
        descent_ratio = (r0 / min_r) if math.isfinite(min_r) and min_r > 0 else 1.0
        score = math.log1p(max(0.0, min_r)) + float(atlas_log_weight) * math.log1p(max(0.0, min_log)) - float(atlas_descent_weight) * math.log1p(max(0.0, descent_ratio))
        refined += 1
        if score < best_score:
            best_score = float(score)
            best_idx = idx
            best_descent_ratio = float(descent_ratio)
            # choose the local probe itself if it is materially better; this is
            # still atlas geometry, before startopt/corrector.
            if math.isfinite(min_r) and min_r < 0.95 * r0:
                pidx = int(np.nanargmin(RP))
                Y[idx] = P[pidx].copy()
                R[idx] = float(RP[pidx])
                logE[idx] = float(LP[pidx])

    diversity_override = False
    meta = dict(metas[best_idx])
    meta.update({
        "atlas_mode": "302-basin-aware-gateway-map",
        "atlas_candidates": int(m),
        "atlas_refine_top": int(refine_top),
        "atlas_refined": int(refined),
        "atlas_evals": int(evals),
        "atlas_selected_index": int(best_idx),
        "atlas_selected_residual": float(R[best_idx]) if best_idx < len(R) and math.isfinite(float(R[best_idx])) else None,
        "atlas_selected_log_energy": float(logE[best_idx]) if best_idx < len(logE) else None,
        "atlas_score": float(best_score),
        "atlas_descent_ratio": best_descent_ratio,
        "atlas_diversity_override": bool(diversity_override),
        "atlas_log_weight": float(atlas_log_weight),
        "atlas_descent_weight": float(atlas_descent_weight),
        "chart": "302-basin-aware-atlas/" + str(meta.get("chart", "single-flow")),
    })
    return Y[best_idx].copy(), meta
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    for trial in range(int(args.pool)):
        if len(roots) >= int(args.count):
            break
        y_raw, geom = basin_aware_atlas_start(
            ...
            atlas_candidates=int(args.atlas_candidates), atlas_refine_top=int(args.atlas_refine_top),
            atlas_probe_radius=float(args.atlas_probe_radius), atlas_log_weight=float(args.atlas_log_weight),
            atlas_descent_weight=float(args.atlas_descent_weight)
        )
        ...
        roots.append({
            "id": rid,
            "source": "302-basin-aware-atlas-full-cubic-halley",
            "trial": int(trial),
    ...
    result = {
        "script": "302_pandrosion_basin_aware_atlas_full_cubic_halley_numpy_engine.py",
        "autonomous": True,
        "dependencies": {"python_scripts": [], "numpy": bool(np is not None)},
        "mode": "302-basin-aware-atlas-full-cubic-halley",
        ...
        "parameters": {
            ...
            "atlas_candidates": int(args.atlas_candidates),
            "atlas_refine_top": int(args.atlas_refine_top),
            "atlas_probe_radius": float(args.atlas_probe_radius),
            "atlas_log_weight": float(args.atlas_log_weight),
            "atlas_descent_weight": float(args.atlas_descent_weight),
        },
        "roots": encoded_roots,
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="302 standalone NumPy Pandrosion engine: basin-aware atlas plus anchored full cubic Halley.")
    ...
    p.add_argument("--atlas-candidates", type=int, default=8, help="302: chart/start candidates scored as a basin-gateway atlas per trial")
    p.add_argument("--atlas-refine-top", type=int, default=4, help="302: top atlas starts locally probed for basin-door descent")
    p.add_argument("--atlas-probe-radius", type=float, default=0.18, help="302: relative transverse radius for atlas basin-door probes")
    p.add_argument("--atlas-log-weight", type=float, default=0.002, help="302: log-scale stability weight in atlas scoring")
    p.add_argument("--atlas-descent-weight", type=float, default=0.08, help="302: reward for local residual descent in atlas scoring")
    p.add_argument("--out", "--thales-out", "--useful-out", default=None)
    p.add_argument("--outdir", default="/mnt/data/302_basin_aware_atlas_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(getattr(args, "self_test", False)):
        ...
        args.out = args.out or "/mnt/data/302_basin_aware_atlas_out/self_test_302.json"
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "302_pandrosion_basin_aware_atlas_full_cubic_halley_numpy_engine.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
        first = cases[0].replace(",", "x") if cases else "case"
        out = Path(args.outdir) / f"302_basin_aware_atlas_{first}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")

    print("=" * 120, flush=True)
    print("302 autonomous BASIN-AWARE ATLAS + ANCHORED FULL-CUBIC HALLEY NumPy Pandrosion", flush=True)
