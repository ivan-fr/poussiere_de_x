#!/usr/bin/env python3
"""
409_pandrosion_standalone_local_jet_geometry_coded_probe_irp_engine.py
...
Mathematical idea:
  ...
  2. Do not call a multivariate algebraic solver, Groebner/resultant machinery, or a
     ...
     The 409 default is adaptive-directional-sketch with a coded one-sided
  ...
  3. 409 prepares waves of starts with a fused batched direct prepass.  The prepass
...
resolution.  The 409 difference from 406 is that the local jet itself can be
directional:
...
409 adds an even smaller coded probe basis D = P R, q << k, and solves

    delta = D v,       min_v ||J D v + F||^2 + mu ||v||^2.

In auto mode, if this compressed Pandrosion IRP correction has a poor reduced
residual, 409 falls back to the 408-style one-sided/central basis.  This can beat
dense matmul only by changing the problem size and avoiding the dense jet.  The
full hypercube, complex GEMM, real-packed GEMM, and SVD paths remain available as
...
def configure_matrix_backend(args: argparse.Namespace) -> None:
    global MATRIX_BACKEND, MATRIX_ALGORITHM, BATCH_KERNEL, TORCH_DEVICE, TORCH_COMPLEX_DTYPE, TORCH_REAL_DTYPE, TORCH_RESOLVED_DEVICE, TORCH_RESOLVED_DTYPE, TORCH_RESOLVED_REAL_DTYPE, NS_ITERS, NS_DAMPING_FLOOR, SKETCH_DIM, SKETCH_FACTOR, SKETCH_MIN_N, SKETCH_SEED, SKETCH_MODE, SKETCH_SOLVER, DIRECTIONAL_JET, DIRECTIONAL_JET_MIN_N, DIRECTIONAL_JET_FACTOR, DIRECTIONAL_DIFF_MODE, DIRECTIONAL_AUTO_CENTRAL_CAP, DIRECTIONAL_CODED_PROBE, DIRECTIONAL_CODED_FACTOR, DIRECTIONAL_CODED_MIN, DIRECTIONAL_CODED_MAX
    ...
    SKETCH_SEED = int(getattr(args, "sketch_seed", 409))
...
def selected_matrix_algorithm_409(dim: int = 0) -> str:
...
def _coded_probe_basis_np(k: int, q: int, salt: int = 0) -> Any:
    kk, qq = int(k), int(q)
    if qq >= kk:
        return np.eye(kk, dtype=np.complex128)
    seed = int(splitmix64(int(SKETCH_SEED) + 1000003 * kk + 9176 * qq + 53 * int(salt) + 0x409C0DE) & 0xFFFFFFFF)
    rng = np.random.default_rng(seed)
    raw = rng.choice([-1.0, 1.0], size=(kk, qq)).astype(np.float64) / math.sqrt(float(kk))
    qmat, _ = np.linalg.qr(raw, mode="reduced")
    return qmat.astype(np.complex128, copy=False)
...
def matrix_svd_step(A: Any, rhs: Any, rcond: float = 1e-12, condition_cap: float = 1e12, damping: float = 0.0) -> tuple[Any, dict[str, Any]]:
    ...
    In 409 the default path is real-packed GEMM/Newton-Schulz on regularised
    ...
    if MATRIX_BACKEND in {"torch", "auto"}:
        try:
            selected_algorithm = selected_matrix_algorithm_409(int(np.asarray(A).shape[-1]))
...
def matrix_least_squares_fit(X: Any, Y: Any, rcond: float = 1e-12, condition_cap: float = 1e12) -> tuple[Any, dict[str, Any]]:
    ...
    if MATRIX_BACKEND in {"torch", "auto"}:
        try:
            selected_algorithm = selected_matrix_algorithm_409(int(np.asarray(X).shape[-1]))
            if selected_algorithm == "sketch-ns":
...
def make_system_409(args: argparse.Namespace, n: int, d: int) -> tuple[LocalJetGeometry, Any, str, str]:
...
def universal_atlas_start(target: TargetTrack, n: int, trial: int, seed: int, powers: Sequence[float], angles: Sequence[float], radii: Sequence[float], cap: float, roots_found: int, duplicates: int, failures: int, target_count: int, universal_cells: int = 16, universal_shells: int = 5, atlas_selection: str = "diverse-shell", **_: Any) -> tuple[Any, dict[str, Any]]:
    ...
    if mode in {"diverse", "diverse-shell", "shell", "raw-shell"}:
        ...
        meta = {
            "chart": "409-standalone-universal-diverse-shell-atlas",
            "atlas_mode": "409-diverse-shell",
...
def directional_sketch_delta(target: TargetTrack, y: Any, f: Any, ep: int, seed: int, lm_damping: float = 0.0, trust_radius: float = 0.0, matrix_rcond: float = 1e-12, matrix_condition_cap: float = 1e12) -> Optional[tuple[Any, dict[str, Any]]]:
    ...
    if not bool(DIRECTIONAL_JET):
        return None
    n = int(len(y))
    if n < int(DIRECTIONAL_JET_MIN_N) or selected_matrix_algorithm_409(n) != "sketch-ns":
        return None
    ...
    h = 1e-5 * yn
    salt = int(seed) + 65537 * int(ep) + 0x409D1A
    ...
    if bool(DIRECTIONAL_CODED_PROBE) and coded_q < k:
        R = _coded_probe_basis_np(k, coded_q, salt=salt + 0xC0DEC0DE)
        C = P @ R
        try:
    ...
    return delta, {
        ...
        "directional_jet": True,
        "directional_jet_algorithm": "409-coded-probe-or-full-directional-oracle-sampled-jp-subspace",
        ...
        "directional_coded_probe": bool(probe_kind == "coded-probe"),
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        if not trigger:
            if improved:
                ...
                status = "local-collapsed" if collapsed else "direct-409-pandrosion-irp-gemm-running"
                continue
...
def torch_batched_gemm_fit_409(X: Any, Y: Any) -> Any:
    XX_np = np.asarray(X, dtype=np.complex128)
    YY_np = np.asarray(Y, dtype=np.complex128)
    device, dtype = torch_device(), torch_complex_dtype()
    XX = torch.as_tensor(XX_np, dtype=dtype, device=device)
...
def torch_batched_gemm_step_409(A: Any, rhs: Any, damping: float = 0.0) -> Any:
    AA_np = np.asarray(A, dtype=np.complex128)
    bb_np = np.asarray(rhs, dtype=np.complex128)
    device, dtype = torch_device(), torch_complex_dtype()
    AA = torch.as_tensor(AA_np, dtype=dtype, device=device)
...
def torch_batched_gemm_candidates_409(dY: Any, dF: Any, F: Any, Y: Any, lambdas: Any, damping: float = 0.0, trust_radius: float = 0.0, yn: Optional[Any] = None) -> Any:
    """Fused 409 batch prepass kernel: fit local jets, step, and candidates in Torch."""
...
def torch_batched_realpack_candidates_409(dY: Any, dF: Any, F: Any, Y: Any, lambdas: Any, damping: float = 0.0, trust_radius: float = 0.0, yn: Optional[Any] = None) -> Any:
    """Fused 409 real-packed prepass kernel using real GEMM/Newton-Schulz."""
    dY_np = np.asarray(dY, dtype=np.complex128)
    dF_np = np.asarray(dF, dtype=np.complex128)
    F_np = np.asarray(F, dtype=np.complex128)
    Y_np = np.asarray(Y, dtype=np.complex128)
...
def torch_batched_sketch_candidates_409(dY: Any, dF: Any, F: Any, Y: Any, lambdas: Any, damping: float = 0.0, trust_radius: float = 0.0, yn: Optional[Any] = None) -> Any:
    """Fused 409 subspace-sketch prepass kernel.
    ...
    k = sketch_dim_for(n)
    if k >= n:
        return torch_batched_gemm_candidates_409(dY_np, dF_np, F_np, Y_np, L_np, damping, trust_radius, yn)
    device, dtype = torch_device(), torch_complex_dtype()
    X = torch.as_tensor(dY_np, dtype=dtype, device=device)
    Z = torch.as_tensor(dF_np, dtype=dtype, device=device)
    Fv = torch.as_tensor(F_np, dtype=dtype, device=device)
...
def selected_batch_kernel_409() -> str:
    if BATCH_KERNEL in {"sketch", "realpack", "complex"}:
        return BATCH_KERNEL
    selected_algorithm = selected_matrix_algorithm_409(128)
    if selected_algorithm == "sketch-ns":
        return "sketch"
    return "realpack" if selected_algorithm in {"realpack-ns", "auto"} else "complex"


def batch_direct_gemm_prepass_409(target: TargetTrack, Y0: Any, max_epochs: int, tol: float, accept: float, line_search: int, line_grid: Sequence[float], direction_seed: int, cloud_nodes: int, lm_damping: float, trust_radius: float) -> list[dict[str, Any]]:
    ...
    configured_kernel = selected_batch_kernel_409()
    ...
    for ep in range(max(0, int(max_epochs))):
        ...
        try:
            if configured_kernel == "sketch":
                C = torch_batched_sketch_candidates_409(dY, dF, Fa, Ya, lambdas, float(lm_damping), float(trust_radius), yn)
            elif configured_kernel == "realpack":
                C = torch_batched_realpack_candidates_409(dY, dF, Fa, Ya, lambdas, float(lm_damping), float(trust_radius), yn)
            else:
                C = torch_batched_gemm_candidates_409(dY, dF, Fa, Ya, lambdas, float(lm_damping), float(trust_radius), yn)
        except Exception as exc:
    ...
    for i in range(B):
        out.append({
            ...
            "batch_prepass_algorithm": f"409-{configured_kernel}-fused-gemm-batched-direct-before-irp",
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    geometry, base, source, backend = make_system_409(args, n, d)
    ...
    def build_trial_start(trial: int) -> dict[str, Any]:
        ...
        if explicit:
            ...
            geom = {"chart": "409-explicit-start", "atlas_mode": "explicit-start", "homothety": 1.0, "theta_deg": None}
        else:
            y_raw = None
            geom: dict[str, Any] = {}
            if bool(args.origin_seed) and (trial == 0 if int(args.origin_seed_period) <= 0 else trial % max(1, int(args.origin_seed_period)) == 0):
                ...
                if y_origin is not None:
                    y_raw = y_origin
                    geom = {"chart": "409-origin-affine-gemm-matrix-seed", "atlas_mode": "origin-affine-gemm-matrix-seed", "homothety": 1.0, "theta_deg": None, **ometa}
            if y_raw is None:
    ...
    result = {
        ...
        "mode": "409-standalone-pandrosion-irp-directional-sketch-local-jet-matrix",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="409 standalone Pandrosion IRP directional-sketch local-jet matrix engine.")
    ...
    p.add_argument("--batch-kernel", choices=["auto", "sketch", "realpack", "complex"], default="auto", help="Batched prepass kernel. auto follows the selected 409 matrix algorithm.")
    ...
    p.add_argument("--matrix-backend", choices=["torch", "numpy", "auto"], default="torch", help="Matrix kernel backend. torch+adaptive-directional-sketch is the 409 default; numpy preserves the SVD fallback behavior.")
    ...
    p.add_argument("--torch-real-dtype", choices=["auto", "float16", "bfloat16", "float32", "float64"], default="auto", help="Torch real dtype for 409 real-packed kernels. auto uses float32.")
    ...
    p.add_argument("--sketch-seed", type=int, default=409, help="Deterministic seed for the randomized sketch basis.")
    ...
    p.add_argument("--directional-coded-probe", action="store_true", default=True, help="Try the 409 coded probe D=P R with q<<k before the full directional sketch.")
    ...
    p.add_argument("--outdir", default="/private/tmp/409_standalone_batched_pandrosion_irp_gemm_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(args.self_test):
        ...
        args.out = args.out or "/private/tmp/409_standalone_batched_pandrosion_irp_gemm_out/self_test_409.json"
    ...
    out = Path(args.out) if args.out else Path(args.outdir) / f"409_batched_pandrosion_irp_gemm_{cases[0].replace(',', 'x') if cases else 'case'}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")
    print("=" * 120, flush=True)
    print("409 STANDALONE PANDROSION IRP -- CODED DIRECTIONAL MATRIX-JET CALCULUS", flush=True)
    ...
    print(f"matrix_backend={MATRIX_BACKEND}, algorithm={MATRIX_ALGORITHM}->{selected_matrix_algorithm_409(first_n)}, torch={torch.__version__}, device={TORCH_RESOLVED_DEVICE}, complex_dtype={TORCH_RESOLVED_DTYPE}, real_dtype={TORCH_RESOLVED_REAL_DTYPE}, batch_kernel={selected_batch_kernel_409()}, sketch_dim={sketch_dim_for(first_n) if first_n else 0}, directional_jet={bool(DIRECTIONAL_JET)}, directional_diff={DIRECTIONAL_DIFF_MODE}, coded_probe={bool(DIRECTIONAL_CODED_PROBE)}, directional_q={directional_q}, directional_k={directional_k}, batch_wave={int(args.batch_wave)}, irp=on", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
        g = r.get("geometry", {})
