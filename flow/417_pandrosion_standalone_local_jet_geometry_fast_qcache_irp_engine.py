#!/usr/bin/env python3
"""
417_pandrosion_standalone_local_jet_geometry_fast_qcache_irp_engine.py
...
417 keeps the 412 path and adds two conservative hot-path optimizations:
...
The generic polynomial/local-jet fallback logic remains inherited from 412.
...
def configure_matrix_backend(args: argparse.Namespace) -> None:
    ...
    SKETCH_SEED = int(getattr(args, "sketch_seed", 417))
...
def directional_sketch_delta(target: TargetTrack, y: Any, f: Any, ep: int, seed: int, lm_damping: float = 0.0, trust_radius: float = 0.0, matrix_rcond: float = 1e-12, matrix_condition_cap: float = 1e12) -> Optional[tuple[Any, dict[str, Any]]]:
    ...
    if bool(DIRECTIONAL_CODED_PROBE) and coded_q < k:
        C = _coded_composite_basis_np(n, k, coded_q, salt=salt)
        direct_identity = _identity_target_coded_projection(target, yy, ff, C, n, k, coded_q)
        if direct_identity is not None:
            u_np, lin, rmeta, samples, JD = direct_identity
            active_basis, active_dim, probe_kind, diff_used = C, coded_q, "coded-probe", "identity-target-direct"
            accepted = True
        else:
            try:
    ...
    return delta, {
        ...
        "directional_jet": True,
        "directional_jet_algorithm": "417-fast-qcache-coded-probe-or-full-directional-oracle-sampled-jp-subspace",
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    result = {
        ...
        "mode": "417-standalone-pandrosion-irp-fast-qcache-directional-sketch-local-jet-matrix",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="417 standalone Pandrosion IRP fast-qcache directional-sketch local-jet matrix engine.")
    ...
    p.add_argument("--batch-kernel", choices=["auto", "sketch", "realpack", "complex"], default="auto", help="Batched prepass kernel. auto follows the selected 417 matrix algorithm.")
    ...
    p.add_argument("--matrix-backend", choices=["torch", "numpy", "auto"], default="torch", help="Matrix kernel backend. torch+adaptive-directional-sketch is the 417 default; numpy preserves the SVD fallback behavior.")
    ...
    p.add_argument("--torch-real-dtype", choices=["auto", "float16", "bfloat16", "float32", "float64"], default="auto", help="Torch real dtype for 417 real-packed kernels. auto uses float32.")
    ...
    p.add_argument("--sketch-seed", type=int, default=417, help="Deterministic seed for the randomized sketch basis.")
    ...
    p.add_argument("--directional-coded-probe", action="store_true", default=True, help="Try the 417 composite-cached coded probe D=P R with q<<k before the full directional sketch.")
    ...
    p.add_argument("--directional-fast-projector", action="store_true", default=True, help="Try the 417 diagonal projected reduced solver before SVD/Newton-Schulz.")
    ...
    p.add_argument("--outdir", default="/private/tmp/417_standalone_fast_qcache_pandrosion_irp_gemm_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(args.self_test):
        ...
        args.out = args.out or "/private/tmp/417_standalone_fast_qcache_pandrosion_irp_gemm_out/self_test_417.json"
    ...
    out = Path(args.out) if args.out else Path(args.outdir) / f"417_fast_qcache_pandrosion_irp_gemm_{cases[0].replace(',', 'x') if cases else 'case'}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")
    print("=" * 120, flush=True)
    print("417 STANDALONE PANDROSION IRP -- FAST-QCACHE CODED MATRIX-JET CALCULUS", flush=True)
