#!/usr/bin/env python3
"""
410_pandrosion_standalone_local_jet_geometry_fast_projected_coded_irp_engine.py
...
def configure_matrix_backend(args: argparse.Namespace) -> None:
    ...
    SKETCH_SEED = int(getattr(args, "sketch_seed", 410))
...
def _fast_projected_coded_basis_np(k: int, q: int, salt: int = 0) -> Any:
    ...
    seed = int(splitmix64(int(SKETCH_SEED) + 1000003 * kk + 9176 * qq + 53 * int(salt) + 0x410C0DE) & 0xFFFFFFFF)
    rng = np.random.default_rng(seed)
...
def directional_sketch_delta(target: TargetTrack, y: Any, f: Any, ep: int, seed: int, lm_damping: float = 0.0, trust_radius: float = 0.0, matrix_rcond: float = 1e-12, matrix_condition_cap: float = 1e12) -> Optional[tuple[Any, dict[str, Any]]]:
    ...
    h = 1e-5 * yn
    salt = int(seed) + 65537 * int(ep) + 0x410D1A
    ...
    return delta, {
        ...
        "directional_jet": True,
        "directional_jet_algorithm": "410-coded-probe-or-full-directional-oracle-sampled-jp-subspace",
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    result = {
        ...
        "mode": "410-standalone-pandrosion-irp-directional-sketch-local-jet-matrix",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="410 standalone Pandrosion IRP directional-sketch local-jet matrix engine.")
    ...
    p.add_argument("--batch-kernel", choices=["auto", "sketch", "realpack", "complex"], default="auto", help="Batched prepass kernel. auto follows the selected 410 matrix algorithm.")
    ...
    p.add_argument("--matrix-backend", choices=["torch", "numpy", "auto"], default="torch", help="Matrix kernel backend. torch+adaptive-directional-sketch is the 410 default; numpy preserves the SVD fallback behavior.")
    ...
    p.add_argument("--torch-real-dtype", choices=["auto", "float16", "bfloat16", "float32", "float64"], default="auto", help="Torch real dtype for 410 real-packed kernels. auto uses float32.")
    ...
    p.add_argument("--sketch-seed", type=int, default=410, help="Deterministic seed for the randomized sketch basis.")
    ...
    p.add_argument("--directional-coded-probe", action="store_true", default=True, help="Try the 410 coded probe D=P R with q<<k before the full directional sketch.")
    ...
    p.add_argument("--directional-fast-projector", action="store_true", default=True, help="Try the 410 diagonal projected reduced solver before SVD/Newton-Schulz.")
    ...
    p.add_argument("--outdir", default="/private/tmp/410_standalone_batched_pandrosion_irp_gemm_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(args.self_test):
        ...
        args.out = args.out or "/private/tmp/410_standalone_batched_pandrosion_irp_gemm_out/self_test_410.json"
    ...
    out = Path(args.out) if args.out else Path(args.outdir) / f"410_batched_pandrosion_irp_gemm_{cases[0].replace(',', 'x') if cases else 'case'}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")
    print("=" * 120, flush=True)
    print("410 STANDALONE PANDROSION IRP -- FAST PROJECTED CODED MATRIX-JET CALCULUS", flush=True)
