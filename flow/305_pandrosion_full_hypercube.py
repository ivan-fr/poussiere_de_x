#!/usr/bin/env python3
"""
305_pandrosion_full_hypercube.py
...
def hypercube_inversejet_corrector(
    ...
    """305 local corrector: hypercube inverse-jet only, no Halley/Q fallback."""
    ensure_numpy()
    y = np.asarray(y0, dtype=np.complex128).copy()
    t0 = now()
    deadline = t0 + trial_timeout if trial_timeout and trial_timeout > 0 else None
    ...
    return {
        ...
        "seconds": float(now() - t0),
        "slope_cond": None,
        "corrector": "305-hypercube-tensor-inversejet-no-halley",
        "raw_pandrosion_step_accepted": False,
        "probe_total_evals": 0,
        "line_search_batch_numpy": True,
        "line_search_evals": int(total_line_evals),
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    for trial in range(int(args.pool)):
        ...
        loc = hypercube_inversejet_corrector(target, y0, max_epochs=int(args.epochs), tol=float(args.tol), accept=float(args.accept), trial_timeout=float(args.trial_timeout), line_search=int(args.line_search), line_grid=parse_float_list(args.line_grid, []), direction_seed=system.seed + 7919 * trial, cloud_nodes=int(getattr(args, "hypercube_nodes", 0)))
        ...
        roots.append({
            "id": rid,
            "source": "305-full-universal-atlas-hypercube-inversejet",
            "trial": int(trial),
    ...
    result = {
        ...
        "mode": "305-full-universal-atlas-hypercube-inversejet",
        "flow_formula": "304 universal atlas: simplex+hypercube+projective+fixed Mobius/Riemann homothety cells -> StartOpt(y) -> 305/306 hypercube least-squares Jacobian + tensor inverse-jet delta1+delta2+delta3 -> batched line search; no Halley, no anchored Q, no Newton fallback",
        "case": f"{n},{d}",
        "family": "ks",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="305 standalone NumPy Pandrosion engine: 304 fixed universal atlas plus hypercube inverse-jet corrector.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "305_pandrosion_full_hypercube.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
    ...
    print("305 autonomous FULL UNIVERSAL ATLAS + HYPERCUBE INVERSE-JET NumPy Pandrosion", flush=True)
    print("No dependency on previous Python scripts; 304 universal atlas/startopt plus 305 hypercube tensor inverse-jet corrector; no Halley/anchored-Q/Newton fallback path.", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
