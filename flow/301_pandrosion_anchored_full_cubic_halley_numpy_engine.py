#!/usr/bin/env python3
"""
301_pandrosion_anchored_full_cubic_halley_numpy_engine.py
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    for trial in range(int(args.pool)):
        ...
        roots.append({
            "id": rid,
            "source": "301-anchored-full-cubic-halley",
            "trial": int(trial),
    ...
    result = {
        "script": "301_pandrosion_anchored_full_cubic_halley_numpy_engine.py",
        "autonomous": True,
        "dependencies": {"python_scripts": [], "numpy": bool(np is not None)},
        "mode": "301-anchored-full-cubic-halley",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="301 standalone NumPy Pandrosion engine: anchored divided-difference full cubic Halley.")
    ...
    p.add_argument("--outdir", default="/mnt/data/301_full_cubic_halley_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(getattr(args, "self_test", False)):
        ...
        args.out = args.out or "/mnt/data/301_full_cubic_halley_out/self_test_301.json"
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "301_pandrosion_anchored_full_cubic_halley_numpy_engine.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
        first = cases[0].replace(",", "x") if cases else "case"
        out = Path(args.outdir) / f"301_full_cubic_halley_{first}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")

    print("=" * 120, flush=True)
    print("301 autonomous ANCHORED FULL-CUBIC HALLEY NumPy Pandrosion", flush=True)
