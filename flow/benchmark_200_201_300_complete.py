#!/usr/bin/env python3
"""Expanded multivariate benchmark: 200 vs 201 vs 300.
...
FLOW = ROOT / "flow"
OUT = ROOT / "benchmarks" / "200_201_300_complete"
...
def main() -> int:
    ...
    md=[]
    md += ["# Complete multivariate benchmark 200 vs 201 vs 300", "", f"Cases: {CASES}", f"Seeds: {SEEDS}", f"Base args: `{' '.join(BASE)}`", "Requested roots: `min(Bezout, 12)`", ""]
    ...
    for tag in ("200","201","300"):
        ...
        t=sum(float(r.get('total_seconds') or 0) for r in sub); trials=sum(int(r.get('trials_used') or 0) for r in sub)
        md.append(f"- **{tag}**: roots {roots}/{req}; successful runs {succ}/{len(sub)}; trials {trials}; failures {fail}; total engine time {t:.3f}s")
    md.append("\n## Per-pair winners\n")
    groups={}
    for r in rows: groups.setdefault((r['case'],r['seed_index']),{})[r['engine']]=r
    metrics={"roots":{},"speed":{},"best_residual":{},"median_residual":{}}
    for g in groups.values():
        if set(g) != {"200","201","300"}: continue
        # roots max
        vals={k:int(v.get('unique_roots') or 0) for k,v in g.items()}; m=max(vals.values()); winners=[k for k,v in vals.items() if v==m]
        metrics['roots'][','.join(winners)]=metrics['roots'].get(','.join(winners),0)+1
        vals={k:float(v.get('total_seconds') or 1e99) for k,v in g.items()}; m=min(vals.values()); winners=[k for k,v in vals.items() if v==m]
        metrics['speed'][','.join(winners)]=metrics['speed'].get(','.join(winners),0)+1
        vals={k:float(v.get('best_residual') or 1e99) for k,v in g.items()}; m=min(vals.values()); winners=[k for k,v in vals.items() if v==m]
        metrics['best_residual'][','.join(winners)]=metrics['best_residual'].get(','.join(winners),0)+1
        vals={k:float(v.get('median_residual') or 1e99) for k,v in g.items()}; m=min(vals.values()); winners=[k for k,v in vals.items() if v==m]
        metrics['median_residual'][','.join(winners)]=metrics['median_residual'].get(','.join(winners),0)+1
