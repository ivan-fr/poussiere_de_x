"""Pandrosion 325: pure finite-slope standalone NumPy root harvester.
...
def correct(target: Target, y0: Any, args: argparse.Namespace, known: Sequence[np.ndarray],
    ...
    for ep in range(max(1, epochs)):
        ...
        if i is None:
            for retry in range(args.slope_retries):
                ...
                try:
                    ...
                    delta,method=slope_step(Q,f,args.trust_radius,y)
                    if method=="failed" or not np.all(np.isfinite(delta)): continue
                    Y=y[None,:]+L[:,None]*delta[None,:]; F=target.eval_batch(Y); R=norms(F); line_evals+=len(Y)
                    i,merit=line_choice(R,Y,y,r,best_r,known,args.deflation_alpha)
        ...
        for _ in range(args.transport_steps):
            ...
            ti, _ = line_choice(TR, TY, y, r, best_r, known, args.deflation_alpha)
            if ti is None: break
            tr = float(TR[ti])
            if tr >= args.transport_ratio*r and tr >= args.accept: break
            ty, tf = TY[ti].copy(), TF[ti].copy(); prev = ty-y; y, f, r = ty, tf, tr; transports += 1
            if r < best_r: best_y, best_f, best_r = y.copy(), f.copy(), r
...
def swarm(base: Target, args: argparse.Namespace, n: int, seed: int) -> tuple[list[np.ndarray], dict[str, Any]]:
    ...
    F = base.eval_batch(Y); R = norms(F); slope_layers=transports=0
    ...
    for i in np.argsort(R):
        ...
        Y[int(i)],R[int(i)]=loc.y.copy(),loc.residual; slope_layers+=loc.slopes; transports+=loc.transported_steps
    selected=[]; sep=args.swarm_sep or .15*math.sqrt(n)
    for i in np.argsort(R):
    ...
    return [Y[i].copy() for i in selected], {"size":size, "kept":len(selected),
        ...
        "mode":"pure-pandrosion-finite-slope-preconditioned-atlas"}
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    return {"script":Path(__file__).name,"version":325,"standalone":True,"case":f"{n},{d}","n":n,"degree":d,
...
def run_monomial_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    return {"script":Path(__file__).name,"version":325,"standalone":True,"case":f"1,{p}","n":1,"degree":p,
...
def parser() -> argparse.ArgumentParser:
    p=argparse.ArgumentParser(description="Pandrosion 325 pure finite-slope standalone NumPy engine")
    ...
    p.add_argument("--out"); p.add_argument("--outdir",default="/tmp/325_pandrosion")
    return p
...
def main(argv:Optional[Sequence[str]]=None)->int:
    ...
    out=Path(args.out) if args.out else Path(args.outdir)/("self_test.json" if args.self_test else f"325_{cases[0].replace(',','x')}.json")
    ...
    if args.self_test:
        print(f"325 self-test: {'PASS' if final['passed'] else 'FAIL'} ({sum(int(c['passed']) for c in final['checks'])}/{len(final['checks'])})")
        print(f"out={out}"); return 0 if final["passed"] else 1
    for result in (results if len(cases)>1 else [final]):
        s=result["summary"]; print(f"325 case={result['case']} oracle={result['oracle']['kind']} roots={s['unique']}/{s['requested']} trials={s['trials']} samples={s['oracle_samples']} seconds={s['seconds']:.3f}")
