#!/usr/bin/env python3
"""
307_pandrosion_universal_atlas_raw_pandrosion_numpy_engine.py

Standalone NumPy Pandrosion root extractor using the 304 universal atlas and a
raw finite-slope Pandrosion local solver.

This 307 path keeps the same autonomous infrastructure as 306:
  - Kostlan/dense polynomial generator for ks(n,d)
  - dense F evaluation engine
  - exact telescopic finite-slope matrix Q_F(a,b)
  - fixed universal atlas cells and StartOpt-style geometric improvement
  - root clustering and JSON export

The active local solver is raw Pandrosion only:

    choose a finite probe endpoint b != a
    build Q_F(a,b) so that F(b)-F(a) = Q_F(a,b)(b-a)
    solve Q_F(a,b) delta = -F(a)
    set a+ = a + lambda delta, with a damping line search

No Steffensen acceleration, no Halley/cubic layer, no hypercube inverse-jet,
no analytic Jacobian/Hessian, no Newton fallback, no SciPy, and no imports from
previous flow scripts are used by the 307 active flow.

Dependencies: Python stdlib + NumPy.
"""
...
def _universal_complex_cell(n: int, idx: int, layer: int, radius: float) -> "np.ndarray":
    ...
    Geometry mix:
      ...
      - cube-corner sign/phase cells for broad coverage;
    ...
    if mode == 0:
        ...
        # cube-corner phases: fixed low-discrepancy corners in complex signs
        for j in range(n):
...
def universal_atlas_start(
    ...
    simplex/cube-corner/projective cells.  It is not adapted to F and does not use
    ...
    for kk in range(cell_count):
        ...
        cell_candidates.append((
            "native-simplex-cube-corner-projective",
    ...
    if admissible:
        ...
        meta = {
            ...
            "atlas_geometry": "hybrid-simplex+cube-corner+projective+fixed-mobius-shells",
...
def startopt(target: TargetTrack, y0: Any, trial: int, seed: int, steps: int, candidates: int, gains: Sequence[float], micro_epochs: int) -> tuple[Any, dict[str, Any]]:
    ...
    for step in range(max(0, steps)):
        ...
        if micro_epochs > 0:
            ...
            loc = raw_pandrosion_corrector(target, cand_best, max_epochs=micro_epochs, tol=1e-12, accept=0.0, trial_timeout=0.0, line_search=6, direction_seed=seed + 7919 * trial, probe_candidates=6, include_self_probe=False, probe_condition_top=1, probe_curvature_top=0)
...
# ---------------------------------------------------------------------------
# 307: 304 universal atlas + raw finite-slope Pandrosion corrector
# ---------------------------------------------------------------------------

def raw_pandrosion_corrector(
    target: TargetTrack,
    y0: Sequence[complex],
    max_epochs: int,
    tol: float,
    ...
    probe_radii: Sequence[float] = (0.35, 0.7, 1.0, 1.6, 2.6, 4.2),
    include_self_probe: bool = False,
    ...
    probe_curvature_top: int = 0,
    probe_curvature_weight: float = 0.0,
    ...
    """307 local corrector: raw finite-slope Pandrosion only.

    At each epoch the solver chooses a genuine finite probe endpoint b, builds
    the exact telescopic Pandrosion matrix Q_F(a,b), and applies the raw step

        Q_F(a,b) delta = -F(a).

    The only robustness layers are probe selection, a condition-aware trust cap,
    and a batched damping line search.  These do not change the correction into
    Steffensen, Halley, inverse-jet, Newton, or any derivative-based method.
    """
    ensure_numpy()
    ...
    total_line_evals = 0
    raw_used_count = 0
    raw_chosen_lambda: Optional[float] = None
    raw_last_step_norm: Optional[float] = None
    raw_last_trust_factor: Optional[float] = None
    lambdas = _line_lambdas(line_search, line_grid)

    # Raw Pandrosion should not silently collapse to the removable b=a limit.
    # If a caller explicitly enables self probes, the scoring still penalizes
    # equal-value poles and the active line search must decrease the residual.
    probe_radii2 = [float(r) for r in probe_radii if float(r) > 0]
    if not probe_radii2:
        probe_radii2 = [0.35, 0.7, 1.0, 1.6, 2.6, 4.2]
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        try:
            ...
            status = f"raw-slope-solve-error:{type(exc).__name__}"
            break
        if not np.all(np.isfinite(delta)):
            status = "nonfinite-raw-pandrosion-step"
            break

        # Damped raw step.  The cap is a trust-region guard, not a second-order
        # correction.  It keeps the finite-slope step inside the atlas chart.
        ynorm = max(1.0, float(np.linalg.norm(y)))
        dnorm = float(np.linalg.norm(delta))
        cap_factor = 18.0
        if float(trust_cond_weight) > 0 and last_cond is not None and math.isfinite(float(last_cond)):
            cap_factor = max(
                float(trust_cond_min),
                18.0 / (1.0 + float(trust_cond_weight) * math.log1p(max(0.0, float(last_cond)))),
            )
        raw_last_trust_factor = float(cap_factor)
        if dnorm > cap_factor * ynorm:
            delta = delta * ((cap_factor * ynorm) / max(dnorm, 1e-300))
            dnorm = float(np.linalg.norm(delta))
        raw_last_step_norm = float(dnorm)
        ...
        if not np.any(better):
            status = "no-raw-pandrosion-decrease"
            break
    ...
    return {
        ...
        "corrector": "307-raw-finite-slope-pandrosion-no-steffensen-no-halley-no-hypercube",
        "raw_pandrosion_step_accepted": True,
        "raw_pandrosion_used_count": int(raw_used_count),
        "raw_pandrosion_step_norm": raw_last_step_norm,
        "raw_pandrosion_chosen_lambda": raw_chosen_lambda,
        "raw_pandrosion_trust_factor": raw_last_trust_factor,
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    probe_radii = parse_float_list(args.probe_radii, [0.35, 0.7, 1.0, 1.6, 2.6, 4.2], positive=False)
    ...
    for trial in range(int(args.pool)):
        ...
        loc = raw_pandrosion_corrector(target, y0, max_epochs=int(args.epochs), tol=float(args.tol), accept=float(args.accept), trial_timeout=float(args.trial_timeout), line_search=int(args.line_search), line_grid=parse_float_list(args.line_grid, []), direction_seed=system.seed + 7919 * trial, probe_scale=float(args.probe_scale), probe_candidates=int(args.probe_candidates), probe_radii=probe_radii, include_self_probe=bool(args.probe_self), probe_condition_top=int(args.probe_condition_top), probe_condition_weight=float(args.probe_condition_weight), probe_axis=bool(args.probe_axis), probe_equal_value_weight=float(args.probe_equal_value_weight), probe_equal_value_eps=float(args.probe_equal_value_eps), probe_curvature_top=int(args.probe_curvature_top), probe_curvature_weight=float(args.probe_curvature_weight), probe_curvature_mid=float(args.probe_curvature_mid), trust_cond_weight=float(args.trust_cond_weight), trust_cond_min=float(args.trust_cond_min), probe_log_weight=float(args.probe_log_weight), probe_log_delta_weight=float(args.probe_log_delta_weight))
        ...
        rec = {
            ...
            "corrector": loc.get("corrector", "raw-finite-slope-pandrosion"),
            ...
            "raw_pandrosion_used_count": loc.get("raw_pandrosion_used_count"),
            "raw_pandrosion_chosen_lambda": loc.get("raw_pandrosion_chosen_lambda"),
            "raw_pandrosion_step_norm": loc.get("raw_pandrosion_step_norm"),
        ...
        roots.append({
            "id": rid,
            "source": "307-universal-atlas-raw-pandrosion",
            "trial": int(trial),
    ...
    result = {
        "script": "307_pandrosion_universal_atlas_raw_pandrosion_numpy_engine.py",
        "autonomous": True,
        "dependencies": {"python_scripts": [], "numpy": bool(np is not None)},
        "mode": "307-universal-atlas-raw-pandrosion",
        "flow_formula": "304 universal atlas: simplex+cube-corner+projective+fixed Mobius/Riemann homothety cells -> StartOpt(y) -> raw finite-slope Pandrosion Q_F(a,b) delta=-F(a) -> damped batched line search; no Steffensen, no Halley, no hypercube inverse-jet, no Newton fallback",
        "case": f"{n},{d}",
        "family": "ks",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="307 standalone NumPy Pandrosion engine: 304 fixed universal atlas plus raw finite-slope Pandrosion corrector.")
    ...
    p.add_argument("--probe-radii", default="0.35,0.7,1,1.6,2.6,4.2", help="positive multipliers for raw finite-slope b-probes")
    p.add_argument("--probe-self", action="store_true", default=False, help="include the removable b=a limit; disabled by default so the raw solver uses genuine finite probes")
    ...
    p.add_argument("--probe-curvature-top", type=int, default=0, help="optional finite-slope curvature score for probe selection only; 0 disables")
    p.add_argument("--probe-curvature-weight", type=float, default=0.0, help="optional penalty for finite-slope curvature variation along probe segment")
    ...
    p.add_argument("--probe-log-weight", type=float, default=0.0005, help="penalty for absolute logarithmic/projective scale energy of probes")
    p.add_argument("--probe-log-delta-weight", type=float, default=0.0015, help="penalty for probes that distort the current logarithmic scale")
    ...
    p.add_argument("--universal-shells", type=int, default=5, help="304: fixed shell layers for simplex/cube-corner/projective atlas")
    ...
    p.add_argument("--outdir", default="/mnt/data/307_raw_pandrosion_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(getattr(args, "self_test", False)):
        ...
        args.out = args.out or "/mnt/data/307_raw_pandrosion_out/self_test_307.json"
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "307_pandrosion_universal_atlas_raw_pandrosion_numpy_engine.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
        first = cases[0].replace(",", "x") if cases else "case"
        out = Path(args.outdir) / f"307_raw_pandrosion_{first}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")

    print("=" * 120, flush=True)
    print("307 autonomous UNIVERSAL FIXED ATLAS + RAW FINITE-SLOPE PANDROSION NumPy engine", flush=True)
    print("No dependency on previous Python scripts; 304 universal atlas/startopt plus raw finite-slope Pandrosion; no Steffensen, no Halley, no hypercube, no Newton fallback path.", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
