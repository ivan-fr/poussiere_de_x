#!/usr/bin/env python3
"""
404_pandrosion_standalone_local_jet_geometry_fused_batched_irp_gemm_engine.py
...
Mathematical idea:
  ...
  2. Do not call a multivariate algebraic solver, Groebner/resultant machinery, or a
     ...
     The 404 default forms regularised normal equations and applies an approximate
     inverse by Newton-Schulz matrix products, so the heavy kernel is GEMM-like.

  3. 404 prepares waves of starts with a fused batched GEMM/Newton-Schulz direct
     prepass, then hands every candidate to the normal Pandrosion lazy IRP chart
     corrector.
...
resolution.  The 404 difference from 403 is that the batched prepass keeps
fit -> step -> line-candidate construction inside Torch before returning to the
black-box oracle. The matrix kernel remains a GEMM-first pseudo-inverse:
...
def configure_matrix_backend(args: argparse.Namespace) -> None:
    global MATRIX_BACKEND, MATRIX_ALGORITHM, TORCH_DEVICE, TORCH_COMPLEX_DTYPE, TORCH_RESOLVED_DEVICE, TORCH_RESOLVED_DTYPE, NS_ITERS, NS_DAMPING_FLOOR
    ...
    algorithm = str(getattr(args, "matrix_algorithm", "gemm-ns") or "gemm-ns").strip().lower().replace("_", "-")
    MATRIX_ALGORITHM = algorithm if algorithm in {"gemm-ns", "svd", "auto"} else "gemm-ns"
...
def matrix_svd_step(A: Any, rhs: Any, rcond: float = 1e-12, condition_cap: float = 1e12, damping: float = 0.0) -> tuple[Any, dict[str, Any]]:
    ...
    In 404 the default path is GEMM-first Newton-Schulz on regularised normal
    equations, while ``--matrix-algorithm svd`` preserves the 402 SVD filter.
    """
    if MATRIX_BACKEND in {"torch", "auto"}:
        try:
            if MATRIX_ALGORITHM in {"gemm-ns", "auto"}:
            ...
            if MATRIX_ALGORITHM == "auto":
                try:
                    x, meta = torch_matrix_svd_step(A, rhs, rcond, condition_cap, damping)
                    meta["matrix_algorithm_fallback"] = f"gemm-error:{type(exc).__name__}"
                    return x, meta
                except Exception:
                    pass
...
def make_system_404(args: argparse.Namespace, n: int, d: int) -> tuple[LocalJetGeometry, Any, str, str]:
...
def universal_atlas_start(target: TargetTrack, n: int, trial: int, seed: int, powers: Sequence[float], angles: Sequence[float], radii: Sequence[float], cap: float, roots_found: int, duplicates: int, failures: int, target_count: int, universal_cells: int = 16, universal_shells: int = 5, atlas_selection: str = "diverse-shell", **_: Any) -> tuple[Any, dict[str, Any]]:
    ...
    if mode in {"diverse", "diverse-shell", "shell", "raw-shell"}:
        ...
        meta = {
            "chart": "404-standalone-universal-diverse-shell-atlas",
            "atlas_mode": "404-diverse-shell",
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        if not trigger:
            if improved:
                ...
                status = "local-collapsed" if collapsed else "direct-404-pandrosion-irp-gemm-running"
                continue
...
def torch_batched_gemm_fit_404(X: Any, Y: Any) -> Any:
    XX_np = np.asarray(X, dtype=np.complex128)
    YY_np = np.asarray(Y, dtype=np.complex128)
...
def torch_batched_gemm_step_404(A: Any, rhs: Any, damping: float = 0.0) -> Any:
    AA_np = np.asarray(A, dtype=np.complex128)
    bb_np = np.asarray(rhs, dtype=np.complex128)
    device, dtype = torch_device(), torch_complex_dtype()
    AA = torch.as_tensor(AA_np, dtype=dtype, device=device)
...
def torch_batched_gemm_candidates_404(dY: Any, dF: Any, F: Any, Y: Any, lambdas: Any, damping: float = 0.0, trust_radius: float = 0.0, yn: Optional[Any] = None) -> Any:
    """Fused 404 batch prepass kernel: fit local jets, step, and candidates in Torch."""
...
def batch_direct_gemm_prepass_404(target: TargetTrack, Y0: Any, max_epochs: int, tol: float, accept: float, line_search: int, line_grid: Sequence[float], direction_seed: int, cloud_nodes: int, lm_damping: float, trust_radius: float) -> list[dict[str, Any]]:
    ...
    for ep in range(max(0, int(max_epochs))):
        ...
        C = torch_batched_gemm_candidates_404(dY, dF, Fa, Ya, lambdas, float(lm_damping), float(trust_radius), yn)
        ...
        for row, idx in enumerate(active):
            ...
            if np.any(np.isfinite(scores)):
                ...
                last_meta[idx] = {"batch_prepass_last_lambda": float(lambdas[j]), "batch_prepass_last_residual": float(scores[j])}
    out = []
    for i in range(B):
        out.append({
            "y": best_y[i].copy(),
            ...
            "batch_prepass_algorithm": "404-fused-gemm-batched-direct-before-irp",
            **last_meta[i],
        })
    return out
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    geometry, base, source, backend = make_system_404(args, n, d)
    ...
    def build_trial_start(trial: int) -> dict[str, Any]:
        ...
        if explicit:
            ...
            geom = {"chart": "404-explicit-start", "atlas_mode": "explicit-start", "homothety": 1.0, "theta_deg": None}
        else:
            y_raw = None
            geom: dict[str, Any] = {}
            if bool(args.origin_seed) and (trial == 0 if int(args.origin_seed_period) <= 0 else trial % max(1, int(args.origin_seed_period)) == 0):
                ...
                if y_origin is not None:
                    y_raw = y_origin
                    geom = {"chart": "404-origin-affine-gemm-matrix-seed", "atlas_mode": "origin-affine-gemm-matrix-seed", "homothety": 1.0, "theta_deg": None, **ometa}
            if y_raw is None:
    ...
    while trial < int(args.pool) and len(roots) < int(args.count):
        ...
        if not wave:
            break
        use_prepass = bool(args.batch_prepass) and len(wave) > 1 and int(args.batch_prepass_epochs) > 0 and MATRIX_BACKEND in {"torch", "auto"} and MATRIX_ALGORITHM in {"gemm-ns", "auto"}
        if use_prepass:
            pre = batch_direct_gemm_prepass_404(target, np.asarray([w["y0"] for w in wave], dtype=np.complex128), int(args.batch_prepass_epochs), float(args.tol), float(args.accept), int(args.line_search), parse_float_list(args.line_grid, []), geometry.seed + 0x404000 + 8191 * trial, int(args.hypercube_nodes), float(args.lm_damping), float(args.trust_radius))
    ...
    result = {
        ...
        "mode": "404-standalone-pandrosion-irp-batched-local-jet-gemm-matrix",
        "flow_formula": "any system as black-box geometric oracle -> lazily-sampled local-jet field (O(n)/step, no global precompute) -> universal Mobius atlas starts -> StartOpt -> batched GEMM direct prepass -> Pandrosion lazy IRP charts -> hypercube Torch GEMM/Newton-Schulz matrix-jet correction + matrix polish, ALL on the jet geometry -> faithful residual acceptance",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="404 standalone batched Pandrosion IRP local-jet Torch GEMM matrix engine.")
    ...
    p.add_argument("--matrix-backend", choices=["torch", "numpy", "auto"], default="torch", help="Matrix kernel backend. torch+gemm-ns is the 404 default; numpy preserves the SVD fallback behavior.")
    p.add_argument("--matrix-algorithm", choices=["gemm-ns", "svd", "auto"], default="gemm-ns", help="Matrix correction algorithm. gemm-ns keeps Pandrosion IRP but uses GEMM/Newton-Schulz inside each local chart.")
    p.add_argument("--ns-iters", type=int, default=10, help="Newton-Schulz iterations for --matrix-algorithm gemm-ns.")
    ...
    p.add_argument("--outdir", default="/private/tmp/404_standalone_batched_pandrosion_irp_gemm_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(args.self_test):
        ...
        args.out = args.out or "/private/tmp/404_standalone_batched_pandrosion_irp_gemm_out/self_test_404.json"
    ...
    out = Path(args.out) if args.out else Path(args.outdir) / f"404_batched_pandrosion_irp_gemm_{cases[0].replace(',', 'x') if cases else 'case'}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")
    print("=" * 120, flush=True)
    print("404 STANDALONE BATCHED PANDROSION IRP -- PYTORCH GEMM MATRIX-JET CALCULUS", flush=True)
    print("PyTorch + NumPy + stdlib only; no local flow imports. The geometry is 317's lazily-sampled local-jet field;", flush=True)
    print(f"matrix_backend={MATRIX_BACKEND}, algorithm={MATRIX_ALGORITHM}, torch={torch.__version__}, device={TORCH_RESOLVED_DEVICE}, complex_dtype={TORCH_RESOLVED_DTYPE}, batch_wave={int(args.batch_wave)}, irp=on", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
        g = r.get("geometry", {})
