"""Pandrosion 322: compact standalone NumPy root-harvesting engine.
...
expression polynomials and dense Kostlan systems are supported; large cases use
a stratified feature surrogate with guaranteed constant/axis features.  Final
acceptance includes a scale-invariant backward-error check.  No local project
imports are used.
"""
...
def make_oracle(args: argparse.Namespace, n: int, d: int) -> Oracle:
    feature_mode = args.system_source not in {"poly", "polynomial"} and math.comb(n+d, d) > args.dense_max_terms
    seed = stable_seed(n, d, args.seed_index, 0x314314 if feature_mode else 0)
    ...
    return KostlanOracle(n, d, seed, args.dense_max_terms, args.features, args.equation_normalize, args.eval_block)
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    return {"script": Path(__file__).name, "version": 322, "standalone": True, "case": f"{n},{d}", "n": n, "degree": d,
            "oracle": {"kind": oracle.kind, "seed": oracle.seed, "samples": oracle.eval_count, "seconds": oracle.seconds,
                       ...
                       "validation_mode": "raw-residual" if isinstance(oracle, ExpressionOracle) else "scale-invariant-backward-error",
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Pandrosion 322 compact standalone NumPy engine")
    ...
    p.add_argument("--out"); p.add_argument("--outdir", default="/tmp/322_pandrosion")
    return p
...
def self_test(args: argparse.Namespace) -> dict[str, Any]:
    ...
    a = argparse.Namespace(**vars(args)); a.system_source = "ks"; a.dense_max_terms = 1; a.features = 512
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    out = Path(args.out) if args.out else Path(args.outdir) / ("self_test.json" if args.self_test else f"322_{cases[0].replace(',', 'x')}.json")
    ...
    if args.self_test:
        print(f"322 self-test: {'PASS' if final['passed'] else 'FAIL'} ({sum(int(c['passed']) for c in final['checks'])}/{len(final['checks'])})")
        print(f"out={out}"); return 0 if final["passed"] else 1
    for result in (results if len(cases) > 1 else [final]):
        s = result["summary"]; print(f"322 case={result['case']} oracle={result['oracle']['kind']} roots={s['unique']}/{s['requested']} trials={s['trials']} samples={s['oracle_samples']} seconds={s['seconds']:.3f}")
