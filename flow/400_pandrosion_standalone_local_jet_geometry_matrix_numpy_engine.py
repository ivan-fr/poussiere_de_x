#!/usr/bin/env python3
"""
400_pandrosion_standalone_local_jet_geometry_matrix_numpy_engine.py

Standalone NumPy engine that reuses the 317 local-jet geometry but turns the local
root search into a MATRIX COMPUTATION.
...
resolution.  Dependencies: Python stdlib + NumPy only.  No local flow imports.
...
def make_system_400(args: argparse.Namespace, n: int, d: int) -> tuple[LocalJetGeometry, Any, str, str]:
...
def universal_atlas_start(target: TargetTrack, n: int, trial: int, seed: int, powers: Sequence[float], angles: Sequence[float], radii: Sequence[float], cap: float, roots_found: int, duplicates: int, failures: int, target_count: int, universal_cells: int = 16, universal_shells: int = 5, atlas_selection: str = "diverse-shell", **_: Any) -> tuple[Any, dict[str, Any]]:
    ...
    if mode in {"diverse", "diverse-shell", "shell", "raw-shell"}:
        ...
        meta = {
            "chart": "400-standalone-universal-diverse-shell-atlas",
            "atlas_mode": "400-diverse-shell",
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        if not trigger:
            if improved:
                ...
                status = "local-collapsed" if collapsed else "direct-400-matrixjet-running"
                continue
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    geometry, base, source, backend = make_system_400(args, n, d)
    ...
    for trial in range(int(args.pool)):
        ...
        if explicit:
            ...
            geom = {"chart": "400-explicit-start", "atlas_mode": "explicit-start", "homothety": 1.0, "theta_deg": None}
        else:
            y_raw = None
            geom: dict[str, Any] = {}
            if bool(args.origin_seed) and (trial == 0 if int(args.origin_seed_period) <= 0 else trial % max(1, int(args.origin_seed_period)) == 0):
                ...
                if y_origin is not None:
                    y_raw = y_origin
                    geom = {"chart": "400-origin-affine-matrix-seed", "atlas_mode": "origin-affine-matrix-seed", "homothety": 1.0, "theta_deg": None, **ometa}
            if y_raw is None:
    ...
    result = {
        ...
        "mode": "400-standalone-local-jet-geometry-matrix-svd",
        "flow_formula": "any system as black-box geometric oracle -> lazily-sampled local-jet field (O(n)/step, no global precompute) -> universal Mobius atlas starts -> StartOpt -> lazy IRP charts -> hypercube matrix-jet SVD correction + matrix polish, ALL on the jet geometry -> faithful residual acceptance",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="400 standalone NumPy local-jet-geometry matrix/SVD engine.")
    ...
    p.add_argument("--outdir", default="/private/tmp/400_standalone_matrix_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(args.self_test):
        ...
        args.out = args.out or "/private/tmp/400_standalone_matrix_out/self_test_400.json"
    ...
    out = Path(args.out) if args.out else Path(args.outdir) / f"400_matrix_{cases[0].replace(',', 'x') if cases else 'case'}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")
    print("=" * 120, flush=True)
    print("400 STANDALONE LOCAL-JET GEOMETRY -- MATRIX/SVD CALCULUS", flush=True)
    print("NumPy + stdlib only; no local flow imports. The geometry is 317's lazily-sampled local-jet field;", flush=True)
    print("corrections are SVD-filtered matrix computations on sampled jets; residual acceptance stays faithful.", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
        g = r.get("geometry", {})
