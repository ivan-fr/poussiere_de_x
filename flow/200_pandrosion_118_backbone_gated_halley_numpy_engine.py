#!/usr/bin/env python3
"""
200_pandrosion_118_backbone_gated_halley_numpy_engine.py
...
def _halley_gate_200(residual: float, cond: Optional[float], gate_residual: float, cond_weight: float, min_gate: float) -> float:
    ...
    g = max(0.0, min(1.0, rg * cg))
    return float(g if g >= float(min_gate) else 0.0)


def _gated_finite_slope_halley_delta_200(
    target: TargetTrack,
    y: "np.ndarray",
    f: "np.ndarray",
    Q: "np.ndarray",
    ...
    """
    ensure_numpy()
    gate = _halley_gate_200(residual, cond, gate_residual, cond_weight, min_gate)
    meta: dict[str, Any] = {
...
def pandrosion_corrector(
    ...
    """200 corrector: 118/120 finite-slope core plus gated cubic finite-slope Halley.
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        if bool(halley_enabled):
            delta, hmeta = _gated_finite_slope_halley_delta_200(
                target=target,
                y=y,
                f=f,
                Q=Q,
    ...
    return {
        ...
        "corrector": "200-118-backbone-equal-value-safe-gated-finite-slope-halley",
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    for trial in range(int(args.pool)):
        ...
        roots.append({
            "id": rid,
            "source": "200-118-backbone-gated-halley-pandrosion",
            "trial": int(trial),
    ...
    result = {
        "script": "200_pandrosion_118_backbone_gated_halley_numpy_engine.py",
        "autonomous": True,
        "dependencies": {"python_scripts": [], "numpy": bool(np is not None)},
        "mode": "200-118-backbone-projective-equal-value-safe-gated-finite-slope-halley",
        "flow_formula": "u -> Mobius_Riemann(theta,pole)(u) -> y=Lambda*Mobius(u) -> StartOpt(y) -> equal-value-safe probe b -> Q_G(a,b) delta=-G(a) -> gated finite-slope Halley correction -> batched line search; no analytic Newton/Jacobian/Hessian fallback",
        "case": f"{n},{d}",
        "family": "ks",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="200 standalone NumPy Pandrosion engine: 118 backbone with 120 equal-value safety and gated finite-slope Halley local correction.")
    ...
    p.add_argument("--halley", action="store_true", default=True, help="200: enable gated cubic finite-slope Halley correction")
    p.add_argument("--no-halley", dest="halley", action="store_false", help="200: disable gated Halley layer and behave close to 120")
    p.add_argument("--halley-gate-residual", type=float, default=0.25, help="200: residual scale for continuous Halley activation")
    p.add_argument("--halley-probe-fraction", type=float, default=0.50, help="200: symmetric second-slope probe fraction along raw Pandrosion step")
    p.add_argument("--halley-cond-weight", type=float, default=0.025, help="200: condition penalty in Halley activation gate")
    p.add_argument("--halley-min-gate", type=float, default=0.04, help="200: below this gate the Halley direction is skipped")
    p.add_argument("--halley-max-correction", type=float, default=1.25, help="200: cap ||delta2|| relative to ||delta_raw||")
    ...
    p.add_argument("--outdir", default="/mnt/data/200_gated_halley_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(getattr(args, "self_test", False)):
        ...
        args.out = args.out or "/mnt/data/200_gated_halley_out/self_test_200.json"
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "200_pandrosion_118_backbone_gated_halley_numpy_engine.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
    ...
    print("200 autonomous 118-BACKBONE + EQUAL-VALUE SAFE + GATED FINITE-SLOPE HALLEY NumPy Pandrosion", flush=True)
    print("No dependency on previous Python scripts; 118-style homothetic/Mobius starts + 120 equal-value-safe probes + gated finite-slope Halley correction; no Newton/Jacobian fallback path.", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
