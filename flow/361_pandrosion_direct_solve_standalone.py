"""Pandrosion 361: restart-hardened standalone root solver.
...
Version 361 adds a final deterministic atlas-restart layer.  It is activated
only after the complete 360 trajectory fails, so every former success keeps
the same numerical path.  Restart directions and radii are reproducible and
all extra oracle evaluations are exposed in the result audit.
"""
...
def _correct_once(target: Target, y0: Any, args: argparse.Namespace, known: Sequence[np.ndarray],
                  epochs: int, deadline: Optional[float], cached: Optional[CachedStart]=None) -> CorrectResult:
    """Run the complete 360 trajectory once, without atlas restarts."""
...
def correct(target: Target, y0: Any, args: argparse.Namespace, known: Sequence[np.ndarray],
            epochs: int, deadline: Optional[float], cached: Optional[CachedStart]=None) -> CorrectResult:
    """361 portfolio: the unchanged 360 path followed by deterministic restarts."""
    primary=_correct_once(target,y0,args,known,epochs,deadline,cached)
    long_call=epochs>=args.restart_min_epochs
    if primary.ok or not args.deterministic_restarts or not long_call:
        return primary

    radii=floats(args.restart_radii,[.125,.25,.5,1.,2.,4.,.0625,8.])
    radii=[float(r) for r in radii if math.isfinite(float(r)) and float(r)>0]
    if not radii:
        return primary
    seed=mix64(int(target.oracle.seed)+0x361A71A5)
    best=primary
    for attempt in range(max(0,int(args.restart_attempts))):
        if deadline is not None and clock()>=deadline:
            break
        radius=radii[attempt%len(radii)]
        candidate=radius*direction(target.oracle.n,attempt,seed)
        before=int(target.oracle.eval_count)
        loc=_correct_once(target,candidate,args,known,epochs,deadline,None)
        loc.restart_attempts=1
        loc.restart_evals=max(0,int(target.oracle.eval_count)-before)
        loc.status=f"restart-{attempt+1}:{loc.status}"
        winner=loc if loc.residual<best.residual else best
        best=add_results(best,loc,winner.y,winner.f,winner.residual,winner.status)
        if best.ok:
            return best
    if not best.ok and best.restart_attempts:
        best.status=f"restart-exhausted:{best.status}"
    return best
...
def add_results(base: CorrectResult, loc: CorrectResult, y: np.ndarray, f: np.ndarray,
    ...
    return CorrectResult(y, f, residual, base.ok or loc.ok, status,
        ...
        base.restart_attempts+loc.restart_attempts,
        base.restart_evals+loc.restart_evals)
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    for trial in range(args.pool):
        ...
        rec={"trial":trial,"status":loc.status,"accepted":accepted,"residual":residual,
            ...
            "deterministic_restart_attempts":loc.restart_attempts,"deterministic_restart_evals":loc.restart_evals,
    ...
    return {"script":Path(__file__).name,"version":361,"standalone":True,"case":f"{n},{d}","n":n,"degree":d,
      "method":{"family":"pandrosion-finite-transport-with-residual-chain-broyden-and-atlas-restarts","derivatives":False,"jacobian":False,"newton":False,
        "broyden":True,"deterministic_restarts":True,"levenberg_marquardt":False,"scipy":False,
        ...
        "restart_scope":"deterministic absolute atlas points, activated only after the unchanged 360 path fails"},
      ...
      "parameters":{"count":args.count,"pool":args.pool,"epochs":args.epochs,"accept":args.accept,
        ...
        "deterministic_restarts":args.deterministic_restarts,"restart_attempts":args.restart_attempts,
        "restart_radii":args.restart_radii,"restart_min_epochs":args.restart_min_epochs},
...
def run_monomial_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    return {"script":Path(__file__).name,"version":361,"standalone":True,"case":f"1,{p}","n":1,"degree":p,
...
def parser() -> argparse.ArgumentParser:
    p=argparse.ArgumentParser(description="Pandrosion 361 restart-hardened finite-slope engine")
    ...
    p.add_argument("--deterministic-restarts",action="store_true",default=True)
    p.add_argument("--no-deterministic-restarts",dest="deterministic_restarts",action="store_false")
    p.add_argument("--restart-attempts",type=int,default=8)
    p.add_argument("--restart-radii",default=".125,.25,.5,1,2,4,.0625,8")
    p.add_argument("--restart-min-epochs",type=int,default=12)
    ...
    p.add_argument("--out"); p.add_argument("--outdir",default="/tmp/361_pandrosion")
    return p
...
def validate(args: argparse.Namespace) -> None:
    ...
    if args.residual_escape_steps<=0 or args.broyden_rescue_epochs<=0 or args.broyden_rescue_min_epochs<=0 or args.coupled_finite_epochs<=0: raise ValueError("invalid 361 rescue controls")
    restart_radii=floats(args.restart_radii,[])
    if args.restart_attempts<0 or args.restart_min_epochs<=0 or not restart_radii or any((not math.isfinite(x)) or x<=0 for x in restart_radii): raise ValueError("invalid 361 deterministic restart controls")
...
def self_test(args: argparse.Namespace) -> dict[str,Any]:
    ...
    checks.append({"name":"361-method-audit","passed":rescued>0 and flags["broyden"] and not any(flags[k] for k in ("derivatives","jacobian","newton","levenberg_marquardt","scipy")),
        "transported_steps":transported,"broyden_rescue_steps":rescued,"method":flags})
    # Regression from the 1,200-system evaluation: this coupled quadratic is
    # deliberately stationary at the imposed origin.  The unchanged 360 path
    # fails, while the deterministic 361 atlas layer must recover a root.
    ...
    def literal(z: complex) -> str: return f"({z.real:.17g}{z.imag:+.17g}j)"
    ...
    raw=";".join(equations); origin=np.zeros(n,np.complex128)
    base_args=argparse.Namespace(**vars(args)); base_args.epochs=96; base_args.accept=1e-8
    primary_oracle=ExpressionOracle(n,d,stable_seed(n,d,0),raw,None)
    primary=_correct_once(Target(primary_oracle),origin,base_args,[],96,None)
    restart_oracle=ExpressionOracle(n,d,stable_seed(n,d,0),raw,None)
    hardened=correct(Target(restart_oracle),origin,base_args,[],96,None)
    checks.append({"name":"361-deterministic-restart-regression",
        "passed":not primary.ok and hardened.ok and 0<hardened.restart_attempts<=base_args.restart_attempts,
        "primary_status":primary.status,"primary_residual":primary.residual,
        "hardened_status":hardened.status,"hardened_residual":hardened.residual,
        "restart_attempts":hardened.restart_attempts,"restart_evals":hardened.restart_evals})
...
def main(argv:Optional[Sequence[str]]=None)->int:
    ...
    out=Path(args.out) if args.out else Path(args.outdir)/("self_test.json" if args.self_test else f"361_{cases[0].replace(',','x')}.json")
    ...
    if args.self_test:
        print(f"361 self-test: {'PASS' if final['passed'] else 'FAIL'} ({sum(int(c['passed']) for c in final['checks'])}/{len(final['checks'])})")
        print(f"out={out}"); return 0 if final["passed"] else 1
    for result in (results if len(cases)>1 else [final]):
        s=result["summary"]; print(f"361 case={result['case']} oracle={result['oracle']['kind']} roots={s['unique']}/{s['requested']} trials={s['trials']} samples={s['oracle_samples']} seconds={s['seconds']:.3f}")
