#!/usr/bin/env python3
"""
312_pandrosion_lazy_irp_hypercube_inversejet_numpy_engine.py
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    for trial in range(int(args.pool)):
        ...
        roots.append({
            "id": rid,
            "source": "312-lazy-irp-hypercube-inversejet",
            "trial": int(trial),
    ...
    result = {
        "script": "312_pandrosion_lazy_irp_hypercube_inversejet_numpy_engine.py",
        "autonomous": True,
        "dependencies": {"python_scripts": [], "numpy": bool(np is not None)},
        "mode": "312-lazy-irp-hypercube-inversejet",
        "flow_formula": "304 universal atlas -> StartOpt(y) -> 312 lazy policy: direct 306 hypercube inverse-jet first; activate IRP chart cascade only on stagnation/weak contraction/bad condition; return chart; no Halley, no anchored Q, no Newton fallback",
        "case": f"{n},{d}",
        "family": "ks",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="312 standalone NumPy Pandrosion engine: 306 hypercube inverse-jet core plus lazy/collapse-triggered IRP chart renormalization.")
    ...
    p.add_argument("--outdir", default="/mnt/data/312_lazy_irp_hypercube_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(getattr(args, "self_test", False)):
        ...
        args.out = args.out or "/mnt/data/304_universal_atlas_out/self_test_312.json"
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "312_pandrosion_lazy_irp_hypercube_inversejet_numpy_engine.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
        first = cases[0].replace(",", "x") if cases else "case"
        out = Path(args.outdir) / f"312_lazy_irp_hypercube_{first}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")

    print("=" * 120, flush=True)
    print("312 autonomous LAZY IRP + UNIVERSAL HYPERCUBE INVERSE-JET NumPy Pandrosion", flush=True)
    print("No dependency on previous Python scripts; 304/306 universal atlas + hypercube tensor inverse-jet core wrapped in 312 lazy/collapse-triggered IRP chart homothety/reciprocal renormalization; no Halley/anchored-Q/Newton fallback path.", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
