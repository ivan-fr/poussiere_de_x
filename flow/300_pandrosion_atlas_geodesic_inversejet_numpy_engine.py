#!/usr/bin/env python3
"""
300_pandrosion_atlas_geodesic_inversejet_numpy_engine.py

Autonomous ATLAS-GEODESIC-INVERSEJET vectorized PURE Pandrosion root extractor.

300 is the standalone NumPy successor of 201. It keeps the pure finite-slope
Thales/Pandrosion corrector, but folds in the later paper stack:
  - atlas/log-scale energy from the chart papers;
  - equal-value pole barriers from the pole-dynamics paper;
  - geodesic probe action: residual + distance + log-scale + curvature;
  - Łojasiewicz/Lyapunov-style line-search acceptance on an extended energy;
  - inverse-jet finite-probe clouds for the gated tensor-Halley layer.

It intentionally does NOT add analytic Jacobian fallback, Newton, Broyden,
SciPy, homotopy paths, or imports from previous flow scripts.
...
def _log_stability_energy_300(y: Any, eps: float = 1e-12) -> float:
...
def _log_stability_batch_300(Y: Any, eps: float = 1e-12) -> "np.ndarray":
    ensure_numpy()
    YY = np.asarray(Y, dtype=np.complex128)
    if YY.ndim == 1:
        YY = YY[None, :]
...
def _atlas_energy_300(residual: float, y: Any, log_weight: float = 0.02, norm_weight: float = 1e-6) -> float:
    """Extended-state Lyapunov score inspired by Papers 008/012/016.

    It is intentionally cheap and derivative-free: residual energy plus small
    chart/log-scale and norm terms.  It lets the line search accept steps that
    decrease the atlas energy, not merely the raw residual at machine noise.
    """
    try:
    ...
    if not math.isfinite(r):
        return float("inf")
    yn = stable_norm(y)
    return float(math.log1p(r) + float(log_weight) * _log_stability_energy_300(y) + float(norm_weight) * math.log1p(max(0.0, yn)))


def _inversejet_probe_cloud_300(n: int, delta: "np.ndarray", seed: int, count: int) -> list["np.ndarray"]:
    """Small symmetric total-degree-ish probe cloud for local tensor sensing.

    This is not a symbolic tensor reconstruction.  It operationalizes Papers
    014/017 by sampling several symmetric directions near the accepted finite
    slope direction and averaging their second finite differences.
    """
    ensure_numpy()
    out: list[np.ndarray] = []
    d = np.asarray(delta, dtype=np.complex128)
    dn = max(1e-300, float(np.linalg.norm(d)))
    if dn <= 1e-300 or not math.isfinite(dn):
        return out
    out.append(d.copy())
    out.append(1j * d)
    for j in range(n):
        ...
        if len(out) >= max(1, int(count)):
            break
        out.append((d + e) / math.sqrt(2.0))
    k = 0
    while len(out) < max(1, int(count)):
        q = raw_direction(n, int(seed) + 65537 * (k + 1), int(seed) ^ (0x300ABC + k), True)
        q = np.asarray(q, dtype=np.complex128)
        q = q / max(1e-300, float(np.linalg.norm(q))) * dn
        out.append(q)
        k += 1
...
def _probe_endpoint_candidates(
    ...
    geodesic_distance_weight: float = 0.002,
    geodesic_pole_power: float = 1.0,
) -> tuple[Any, dict[str, Any]]:
    ...
    # 300 logarithmic/projective stability: probes that violently change
    ...
    log_y = _log_stability_energy_300(y)
    log_B = _log_stability_batch_300(B)
    ...
    # 300 geodesic action term: distance in chart coordinates is cheap,
    # derivative-free, and discourages violent probes unless residual/log-scale
    # benefits compensate.
    if float(geodesic_distance_weight) > 0:
        scores = scores + float(geodesic_distance_weight) * np.log1p(D / max(1e-300, ynorm))
    ...
    if float(equal_value_weight) > 0:
        ...
        pole_power = max(0.25, float(geodesic_pole_power))
        pole_penalty = np.where(nonself, float(equal_value_weight) * np.log1p(1.0 / (safe_gap ** pole_power)), 0.0)
    ...
    return best_b, {
        ...
        "geodesic_distance_weight": float(geodesic_distance_weight),
        "geodesic_pole_power": float(geodesic_pole_power),
...
def _halley_gate_300(residual: float, cond: Optional[float], gate_residual: float, cond_weight: float, min_gate: float, log_energy: float = 0.0, log_weight: float = 0.0, log_scale: float = 1.0) -> float:
...
def _gated_tensor_halley_delta_300(
    target: TargetTrack,
    y: "np.ndarray",
    f: "np.ndarray",
    Q: "np.ndarray",
    ...
    """
    ensure_numpy()
    gate = _halley_gate_300(residual, cond, gate_residual, cond_weight, min_gate, log_energy=log_energy, log_weight=halley_log_weight, log_scale=halley_log_scale)
    meta: dict[str, Any] = {
    ...
    try:
        ...
        # 300 inverse-jet cloud: a small symmetric total-degree-ish probe cloud.
        # This is the numerical counterpart of Papers 014/017: no analytic
        # Hessian is built, but local tensor curvature is sensed from finite
        # symmetric probes before mixing the Halley layer.
        extra = max(0, int(tensor_extra_directions))
        if extra > 0:
            for qdir in _inversejet_probe_cloud_300(len(y), delta_raw, 0x300000 + 7919 * extra, extra + 2)[1:]:
                hvec2 = frac * np.asarray(qdir, dtype=np.complex128)
                try:
...
def pandrosion_corrector(
    ...
    halley_log_weight: float = 0.03,
    halley_log_scale: float = 1.0,
    tensor_extra_directions: int = 4,
    geodesic_distance_weight: float = 0.002,
    geodesic_pole_power: float = 1.25,
    lyapunov_log_weight: float = 0.02,
    lyapunov_norm_weight: float = 1e-6,
) -> dict[str, Any]:
    """300 corrector: log-stable optimal-probe tensor-Halley Pandrosion.

    Base step:
        Q_G(a,b_*) delta = -G(a)

    Optional local high-order layer:
        Q_G(a,b_*) delta_2 = -1/2 H_G(delta,delta)
        delta_200 = (1-gate) delta + gate (delta + delta_2)

    All matrices are Pandrosion finite-slope matrices.  No analytic Jacobian,
    Hessian, Newton fallback, Broyden update, or homotopy continuation is used.
    """
    ensure_numpy()
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        try:
            b, pmeta = _probe_endpoint_candidates(
                ...
                condition_top=int(probe_condition_top),
                condition_weight=float(probe_condition_weight),
                axis_probes=bool(probe_axis),
                equal_value_weight=float(probe_equal_value_weight),
                equal_value_eps=float(probe_equal_value_eps),
                curvature_top=int(probe_curvature_top),
                curvature_weight=float(probe_curvature_weight),
                curvature_mid=float(probe_curvature_mid),
                probe_log_weight=float(probe_log_weight),
                probe_log_delta_weight=float(probe_log_delta_weight),
                geodesic_distance_weight=float(geodesic_distance_weight),
                geodesic_pole_power=float(geodesic_pole_power),
        ...
        if dnorm > cap_factor * ynorm:
            delta_raw = delta_raw * ((cap_factor * ynorm) / max(dnorm, 1e-300))

        delta = delta_raw
        if bool(halley_enabled):
            delta, hmeta = _gated_tensor_halley_delta_300(
                target=target,
                y=y,
                f=f,
                Q=Q,
                ...
                residual=r,
                cond=last_cond,
                gate_residual=float(halley_gate_residual),
                probe_fraction=float(halley_probe_fraction),
                cond_weight=float(halley_cond_weight),
                min_gate=float(halley_min_gate),
                max_correction=float(halley_max_correction),
                log_energy=_log_stability_energy_300(y),
                halley_log_weight=float(halley_log_weight),
                halley_log_scale=float(halley_log_scale),
                tensor_extra_directions=int(tensor_extra_directions),
            )
            last_halley_meta = hmeta
            if hmeta.get("halley_used"):
                total_halley_evals += 2 * int(hmeta.get("halley_tensor_terms", 1))
                halley_used_count += 1
        else:
        ...
        if bool(halley_enabled) and last_halley_meta.get("halley_used"):
        ...
        # 300 Lyapunov/KL-style acceptance: prefer residual decrease, but score
        # candidates by an extended atlas energy that includes log-scale and
        # tiny norm regularization.  This implements the Papers 012/016 idea
        # without adding derivatives or external dependencies.
        e_cur = _atlas_energy_300(r, y, float(lyapunov_log_weight), float(lyapunov_norm_weight))
        Etry = np.asarray([_atlas_energy_300(float(Rtry[i]), Ytry[i], float(lyapunov_log_weight), float(lyapunov_norm_weight)) if bool(finite[i]) else float("inf") for i in range(len(Ytry))], dtype=float)
        better = finite & ((Rtry < r) | (Rtry < best_r) | (Etry < e_cur))
        if not np.any(better):
            status = "no-lyapunov-decrease"
            break

        scores = np.where(better, Etry * (1.0 + 1e-15 / np.maximum(lambda_tags, 1e-15)), np.inf)
    ...
    return {
        ...
        "corrector": "300-atlas-geodesic-inversejet-finite-slope-halley",
        ...
        "halley_enabled": bool(halley_enabled),
        "halley_total_evals": int(total_halley_evals),
        "halley_used_count": int(halley_used_count),
        "halley_gate_residual": float(halley_gate_residual),
        "halley_probe_fraction": float(halley_probe_fraction),
        "halley_cond_weight": float(halley_cond_weight),
        "halley_min_gate": float(halley_min_gate),
        ...
        "geodesic_distance_weight": float(geodesic_distance_weight),
        "geodesic_pole_power": float(geodesic_pole_power),
        "lyapunov_log_weight": float(lyapunov_log_weight),
        "lyapunov_norm_weight": float(lyapunov_norm_weight),
        **last_probe_meta,
        **last_halley_meta,
    }
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    for trial in range(int(args.pool)):
        ...
        roots.append({
            "id": rid,
            "source": "300-atlas-geodesic-inversejet-pandrosion",
            "trial": int(trial),
    ...
    result = {
        "script": "300_pandrosion_atlas_geodesic_inversejet_numpy_engine.py",
        "autonomous": True,
        "dependencies": {"python_scripts": [], "numpy": bool(np is not None)},
        "mode": "300-atlas-geodesic-inversejet-finite-slope-halley",
        "flow_formula": "u -> Mobius_Riemann(theta,pole)(u) -> y=Lambda*Mobius(u) -> StartOpt(y) -> log-stable optimal probe b -> Q_G(a,b) delta=-G(a) -> tensorial gated finite-slope Halley -> batched line search; no analytic Newton/Jacobian/Hessian fallback",
        "case": f"{n},{d}",
        "family": "ks",
        ...
        "parameters": {
            ...
            "geodesic_distance_weight": float(args.geodesic_distance_weight),
            "geodesic_pole_power": float(args.geodesic_pole_power),
            "lyapunov_log_weight": float(args.lyapunov_log_weight),
            "lyapunov_norm_weight": float(args.lyapunov_norm_weight),
            "powers": powers,
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="300 standalone NumPy Pandrosion engine: atlas/geodesic/inverse-jet successor of 201.")
    ...
    p.add_argument("--halley", action="store_true", default=True, help="300: enable gated cubic finite-slope Halley correction")
    p.add_argument("--no-halley", dest="halley", action="store_false", help="300: disable gated Halley layer and behave close to 120")
    p.add_argument("--halley-gate-residual", type=float, default=0.25, help="300: residual scale for continuous Halley activation")
    p.add_argument("--halley-probe-fraction", type=float, default=0.50, help="300: symmetric second-slope probe fraction along raw Pandrosion step")
    p.add_argument("--halley-cond-weight", type=float, default=0.025, help="300: condition penalty in Halley activation gate")
    p.add_argument("--halley-min-gate", type=float, default=0.04, help="300: below this gate the Halley direction is skipped")
    p.add_argument("--halley-max-correction", type=float, default=1.25, help="300: cap ||delta2|| relative to ||delta_raw||")
    p.add_argument("--probe-log-weight", type=float, default=0.0005, help="300: penalty for absolute logarithmic/projective scale energy of probes")
    p.add_argument("--probe-log-delta-weight", type=float, default=0.0015, help="300: penalty for probes that distort the current logarithmic scale")
    p.add_argument("--halley-log-weight", type=float, default=0.03, help="300: logarithmic stability penalty in the Halley gate")
    p.add_argument("--halley-log-scale", type=float, default=1.0, help="300: scale for logarithmic stability gate")
    p.add_argument("--tensor-extra-directions", type=int, default=4, help="300: extra symmetric inverse-jet finite-slope tensor directions near a root")
    p.add_argument("--geodesic-distance-weight", type=float, default=0.002, help="300: probe geodesic action penalty for long chart steps")
    p.add_argument("--geodesic-pole-power", type=float, default=1.25, help="300: exponent in equal-value pole barrier")
    p.add_argument("--lyapunov-log-weight", type=float, default=0.02, help="300: line-search atlas Lyapunov log-scale weight")
    p.add_argument("--lyapunov-norm-weight", type=float, default=1e-6, help="300: line-search tiny norm regularization weight")
    ...
    p.add_argument("--outdir", default="/mnt/data/300_atlas_geodesic_inversejet_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(getattr(args, "self_test", False)):
        ...
        args.out = args.out or "/mnt/data/300_atlas_geodesic_inversejet_out/self_test_300.json"
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "300_pandrosion_atlas_geodesic_inversejet_numpy_engine.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
        first = cases[0].replace(",", "x") if cases else "case"
        out = Path(args.outdir) / f"300_atlas_geodesic_inversejet_{first}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")

    print("=" * 120, flush=True)
    print("300 autonomous ATLAS-GEODESIC INVERSE-JET NumPy Pandrosion", flush=True)
    print("No dependency on previous Python scripts; 118/120/200 backbone + logarithmic stability + optimal-probe energy + tensorial gated finite-slope Halley; no Newton/Jacobian fallback path.", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
