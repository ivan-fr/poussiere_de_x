#!/usr/bin/env python3
"""
119_pandrosion_probe_aware_pure_thales_numpy_engine.py

Autonomous PROBE-AWARE vectorized PURE Pandrosion Thales root extractor.

119 is the direct NumPy-only successor of the robust 118 branch.  It keeps the
pure finite-slope Thales/Pandrosion corrector, but adds:
  - batched NumPy evaluation of start/probe/line-search candidates;
  - residual-gated finite-slope conditioning for top b-probes;
  - optional coordinate-axis Thales probes;
  - vectorized best-line-search instead of first-decrease line-search.

It intentionally does NOT add inverse-jet tensors or Newton fallback paths.
...
def startopt(target: TargetTrack, y0: Any, trial: int, seed: int, steps: int, candidates: int, gains: Sequence[float], micro_epochs: int) -> tuple[Any, dict[str, Any]]:
    ...
    118 scored start candidates one-by-one.  119 keeps the same geometric pool
    ...
    for step in range(max(0, steps)):
        ...
        if micro_epochs > 0:
            ...
            # on the best batched start candidate to keep 119 lightweight.
...
def _probe_endpoint_candidates(
    ...
    119 keeps the 118 theorem-guided principle b_* = argmin ||G(b)||, but uses
    a NumPy batch to score all probes at once.  Among the top residual probes it
    optionally adds a small penalty for the condition number of the exact
    finite-slope matrix Q_G(a,b).  This is not a Newton fallback: Q_G is the
    same telescopic Pandrosion slope used for the accepted step.
    """
    ensure_numpy()
    n = len(y)
    ...
    while len(candidates) < budget:
        ...
        qdir = raw_direction(n, direction_seed + 104729 * (ep + 1) + 7919 * (k + 1), direction_seed ^ (0x119119 + 17 * k), True)
    ...
    return best_b, {
        "probe_mode": "119-batched-residual-min-with-finite-slope-conditioning",
...
def pandrosion_corrector(
    ...
    """119 probe-aware pure derivative-free Pandrosion corrector.
    ...
    return {
        ...
        "corrector": "119-batch-probe-aware-pure-pandrosion-conditioned-exact-slope",
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    # Fully autonomous 119 keeps A simple and robust; the projective/homothetic flow provides the main geometry.
    ...
    for trial in range(int(args.pool)):
        ...
        loc = pandrosion_corrector(target, y0, max_epochs=int(args.epochs), tol=float(args.tol), accept=float(args.accept), trial_timeout=float(args.trial_timeout), line_search=int(args.line_search), probe_scale=float(getattr(args, "probe_scale", 0.035)), direction_seed=system.seed + 7919 * trial, probe_candidates=int(args.probe_candidates), probe_radii=probe_radii, include_self_probe=bool(args.probe_self), probe_condition_top=int(args.probe_condition_top), probe_condition_weight=float(args.probe_condition_weight), probe_axis=bool(args.probe_axis), line_grid=parse_float_list(args.line_grid, []))
        ...
        roots.append({
            "id": rid,
            "source": "119-autonomous-batch-probe-aware-pure-thales-pandrosion",
            "trial": int(trial),
    ...
    result = {
        "script": "119_pandrosion_probe_aware_pure_thales_numpy_engine.py",
        "autonomous": True,
        "dependencies": {"python_scripts": [], "numpy": bool(np is not None)},
        "mode": "119-autonomous-batched-probe-aware-pure-pandrosion/thales-power-riemann-startopt-exact-slope-finite-slope-conditioning",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="119 standalone NumPy pure-Thales Pandrosion engine: robust 118 successor with batched probes and finite-slope conditioning.")
    ...
    p.add_argument("--outdir", default="/mnt/data/119_probe_aware_pure_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(getattr(args, "self_test", False)):
        ...
        args.out = args.out or "/mnt/data/119_probe_aware_pure_out/self_test_119.json"
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "119_pandrosion_probe_aware_pure_thales_numpy_engine.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
        first = cases[0].replace(",", "x") if cases else "case"
        out = Path(args.outdir) / f"119_probe_aware_pure_{first}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")

    print("=" * 120, flush=True)
    print("119 autonomous BATCH-PROBE-AWARE NumPy PURE Thales Pandrosion - direct successor of 118", flush=True)
    print("No dependency on previous Python scripts; batched theorem-guided b-probes + exact finite-slope conditioning + batched line search; no Newton/Jacobian fallback path.", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
