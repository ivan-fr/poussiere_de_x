#!/usr/bin/env python3
"""
402_pandrosion_standalone_local_jet_geometry_torch_gemm_engine.py

Standalone PyTorch/NumPy engine that reuses the 317 local-jet geometry but turns
the local root search into a GEMM-first Torch MATRIX COMPUTATION.
...
resolution.  The 402 difference from 401 is a GEMM-first pseudo-inverse:
...
def matrix_svd_step(A: Any, rhs: Any, rcond: float = 1e-12, condition_cap: float = 1e12, damping: float = 0.0) -> tuple[Any, dict[str, Any]]:
    ...
    In 402 the default path is GEMM-first Newton-Schulz on regularised normal
    equations, while ``--matrix-algorithm svd`` preserves the 401 SVD filter.
    """
...
def make_system_402(args: argparse.Namespace, n: int, d: int) -> tuple[LocalJetGeometry, Any, str, str]:
...
def universal_atlas_start(target: TargetTrack, n: int, trial: int, seed: int, powers: Sequence[float], angles: Sequence[float], radii: Sequence[float], cap: float, roots_found: int, duplicates: int, failures: int, target_count: int, universal_cells: int = 16, universal_shells: int = 5, atlas_selection: str = "diverse-shell", **_: Any) -> tuple[Any, dict[str, Any]]:
    ...
    if mode in {"diverse", "diverse-shell", "shell", "raw-shell"}:
        ...
        meta = {
            "chart": "402-standalone-universal-diverse-shell-atlas",
            "atlas_mode": "402-diverse-shell",
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        if not trigger:
            if improved:
                ...
                status = "local-collapsed" if collapsed else "direct-402-pandrosion-irp-gemm-running"
                continue
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    geometry, base, source, backend = make_system_402(args, n, d)
    ...
    for trial in range(int(args.pool)):
        ...
        if explicit:
            ...
            geom = {"chart": "402-explicit-start", "atlas_mode": "explicit-start", "homothety": 1.0, "theta_deg": None}
        else:
            y_raw = None
            geom: dict[str, Any] = {}
            if bool(args.origin_seed) and (trial == 0 if int(args.origin_seed_period) <= 0 else trial % max(1, int(args.origin_seed_period)) == 0):
                ...
                if y_origin is not None:
                    y_raw = y_origin
                    geom = {"chart": "402-origin-affine-gemm-matrix-seed", "atlas_mode": "origin-affine-gemm-matrix-seed", "homothety": 1.0, "theta_deg": None, **ometa}
            if y_raw is None:
    ...
    result = {
        ...
        "mode": "402-standalone-pandrosion-irp-local-jet-gemm-matrix",
        "flow_formula": "any system as black-box geometric oracle -> lazily-sampled local-jet field (O(n)/step, no global precompute) -> universal Mobius atlas starts -> StartOpt -> Pandrosion lazy IRP charts -> hypercube Torch GEMM/Newton-Schulz matrix-jet correction + matrix polish, ALL on the jet geometry -> faithful residual acceptance",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="402 standalone Pandrosion IRP local-jet Torch GEMM matrix engine.")
    ...
    p.add_argument("--matrix-backend", choices=["torch", "numpy", "auto"], default="torch", help="Matrix kernel backend. torch+gemm-ns is the 402 default; numpy preserves the SVD fallback behavior.")
    ...
    p.add_argument("--outdir", default="/private/tmp/402_standalone_pandrosion_irp_gemm_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(args.self_test):
        ...
        args.out = args.out or "/private/tmp/402_standalone_pandrosion_irp_gemm_out/self_test_402.json"
    ...
    out = Path(args.out) if args.out else Path(args.outdir) / f"402_pandrosion_irp_gemm_{cases[0].replace(',', 'x') if cases else 'case'}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")
    print("=" * 120, flush=True)
    print("402 STANDALONE PANDROSION IRP -- PYTORCH GEMM MATRIX-JET CALCULUS", flush=True)
    print("PyTorch + NumPy + stdlib only; no local flow imports. The geometry is 317's lazily-sampled local-jet field;", flush=True)
    print(f"matrix_backend={MATRIX_BACKEND}, algorithm={MATRIX_ALGORITHM}, torch={torch.__version__}, device={TORCH_RESOLVED_DEVICE}, complex_dtype={TORCH_RESOLVED_DTYPE}, irp=on", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
        g = r.get("geometry", {})
