...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    for trial in range(int(args.pool)):
        ...
        roots.append({
            "id": rid,
            "source": "306-universal-atlas-hypercube-inversejet",
            "trial": int(trial),
    ...
    result = {
        "script": "306_pandrosion_universal_atlas_hypercube_inversejet_numpy_engine.py",
        "autonomous": True,
        "dependencies": {"python_scripts": [], "numpy": bool(np is not None)},
        "mode": "306-universal-atlas-hypercube-inversejet",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="306 standalone NumPy Pandrosion engine: 304 fixed universal atlas plus 305 hypercube inverse-jet corrector.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "306_pandrosion_universal_atlas_hypercube_inversejet_numpy_engine.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
    ...
    print("306 autonomous UNIVERSAL FIXED ATLAS + HYPERCUBE INVERSE-JET NumPy Pandrosion", flush=True)
