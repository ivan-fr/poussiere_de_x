#!/usr/bin/env python3
"""
419_pandrosion_standalone_local_jet_geometry_q_residual_cascade_irp_engine.py
...
412 keeps the 411 mathematical path and makes the hot q-path cheaper:

    - cache the composite coded basis C = P R directly, not only P and R;
    - skip optional off-diagonal fast-projector diagnostics by default;
    - carry the already-computed reduced linear residual through metadata
      instead of recomputing it at the end of the directional step.

These are engineering optimizations only.  The residual validator and fallback
logic remain active.

417 kept the 412 path and added two conservative hot-path optimizations:

    - a validated affine-identity target fast path for F(z)=z-root, used by the
      AI adapter/q-output validation benchmarks.  It checks that the current
      oracle residual matches y-root before projecting through the coded q-basis;
    - an early lambda=1 line-search exit when the exact residual already satisfies
      the requested tolerance/accept threshold.

419 keeps the 417 hot path and changes the unfavorable-q behavior: instead of
leaving the Pandrosion IRP family after the first small q rejection, it grows a
q-residual cascade q -> 2q -> 4q -> ... -> k, with an optional final full-n IRP
stage.  The worst case can be dense-cost, but it remains an IRP/local-jet solve
rather than an external matmul fallback.

The generic polynomial/local-jet logic remains inherited from 412/417.
...
DIRECTIONAL_IDENTITY_TARGET_FASTPATH = True
DIRECTIONAL_Q_RESIDUAL_CASCADE = True
DIRECTIONAL_Q_CASCADE_GROWTH = 2.0
DIRECTIONAL_Q_CASCADE_MAX_STAGES = 0
DIRECTIONAL_Q_CASCADE_FINAL_FULL_N = True
...
def configure_matrix_backend(args: argparse.Namespace) -> None:
    ...
    SKETCH_SEED = int(getattr(args, "sketch_seed", 419))
    ...
    DIRECTIONAL_FAST_PROJECTOR_DIAGNOSTICS = bool(getattr(args, "directional_fast_projector_diagnostics", False))
    DIRECTIONAL_BASIS_REUSE = bool(getattr(args, "directional_basis_reuse", True))
    DIRECTIONAL_IDENTITY_TARGET_FASTPATH = bool(getattr(args, "directional_identity_target_fastpath", True))
    DIRECTIONAL_Q_RESIDUAL_CASCADE = bool(getattr(args, "directional_q_residual_cascade", True))
    DIRECTIONAL_Q_CASCADE_GROWTH = max(1.05, float(getattr(args, "directional_q_cascade_growth", 2.0)))
    DIRECTIONAL_Q_CASCADE_MAX_STAGES = max(0, int(getattr(args, "directional_q_cascade_max_stages", 0)))
    DIRECTIONAL_Q_CASCADE_FINAL_FULL_N = bool(getattr(args, "directional_q_cascade_final_full_n", True))
...
def directional_q_cascade_dims_for(n: int, k: int, q: int) -> list[int]:
    """Return the 419 IRP-only residual cascade dimensions.

    The cascade grows the coded q-space before using any broader local-jet path.
    If DIRECTIONAL_Q_CASCADE_FINAL_FULL_N is enabled, the last stage is an
    n-dimensional IRP step, which is expensive but still not an external dense
    matmul fallback.
    """
    nn, kk, qq = max(1, int(n)), max(1, int(k)), max(1, int(q))
    dims: list[int] = [min(kk, qq)]
    if bool(DIRECTIONAL_Q_RESIDUAL_CASCADE):
        cur = dims[-1]
        stages = 0
        max_stages = int(DIRECTIONAL_Q_CASCADE_MAX_STAGES)
        while cur < kk and (max_stages <= 0 or stages < max_stages):
            nxt = max(cur + 1, int(math.ceil(float(cur) * float(DIRECTIONAL_Q_CASCADE_GROWTH))))
            cur = min(kk, nxt)
            if cur != dims[-1]:
                dims.append(cur)
            stages += 1
    if bool(DIRECTIONAL_Q_CASCADE_FINAL_FULL_N) and nn > dims[-1]:
        dims.append(nn)
    out: list[int] = []
    seen: set[int] = set()
    for dim in dims:
        dd = max(1, min(nn, int(dim)))
        if dd not in seen:
            out.append(dd)
            seen.add(dd)
    return out
...
def _fast_projected_coded_basis_np(k: int, q: int, salt: int = 0) -> Any:
    ...
    if cached is not None:
        return cached
    seed = int(splitmix64(int(SKETCH_SEED) + 1000003 * kk + 9176 * qq + 53 * int(salt) + 0x412C0DE) & 0xFFFFFFFF)
    rng = np.random.default_rng(seed)
...
def _coded_composite_basis_np(n: int, k: int, q: int, salt: int = 0) -> Any:
    nn, kk, qq = int(n), int(k), int(q)
    if qq >= kk:
        return _sketch_basis_np(nn, kk, salt=salt)
    mode = "cached-rademacher" if SKETCH_MODE == "auto" else str(SKETCH_MODE)
    key = ("C", mode, int(SKETCH_SEED), nn, kk, qq, int(salt))
    cached = _basis_cache_lookup(key)
    if cached is not None:
        return cached
    P = _sketch_basis_np(nn, kk, salt=salt)
    R = _fast_projected_coded_basis_np(kk, qq, salt=salt + 0xC0DEC0DE)
    return _basis_cache_store(key, P @ R)


def _identity_target_root_if_consistent(target: Any, y: Any, f: Any) -> Optional[Any]:
    if not bool(DIRECTIONAL_IDENTITY_TARGET_FASTPATH):
        return None
    root = getattr(target, "root", None)
    if root is None:
        return None
    try:
        rr = np.asarray(root, dtype=np.complex128)
        yy = np.asarray(y, dtype=np.complex128)
        ff = np.asarray(f, dtype=np.complex128)
    except Exception:
        return None
    if rr.shape != yy.shape or ff.shape != yy.shape:
        return None
    mismatch = finite_norm((yy - rr) - ff)
    scale = max(1e-300, finite_norm(ff), finite_norm(yy - rr))
    if mismatch / scale <= 1e-9:
        return rr
    return None


def _identity_target_coded_projection(target: Any, y: Any, f: Any, basis: Any, n: int, k: int, q: int) -> Optional[tuple[Any, Any, dict[str, Any], int, Any]]:
    root = _identity_target_root_if_consistent(target, y, f)
    if root is None:
        return None
    C = np.asarray(basis, dtype=np.complex128)
    yy = np.asarray(y, dtype=np.complex128)
    ff = np.asarray(f, dtype=np.complex128)
    rhs = -ff
    coeff = C.conj().T @ rhs
    ...
    if not math.isfinite(lin_rel) or lin_rel > float(DIRECTIONAL_AUTO_CENTRAL_CAP):
        return None
    return coeff.astype(np.complex128, copy=False), lin, {
        "reduced_solver": "identity-target-direct-q-projection",
        "reduced_rank": int(q),
        "reduced_damping_mu": 0.0,
        "reduced_identity_target_fastpath": True,
        "reduced_identity_target_checked": True,
        "reduced_identity_target_relative_residual": float(lin_rel),
    }, 0, C
...
def _numpy_fast_projected_apply(A: Any, rhs: Any, damping: float = 0.0, condition_cap: float = 1e12) -> tuple[Any, dict[str, Any]]:
    ...
    if bool(DIRECTIONAL_FAST_PROJECTOR_DIAGNOSTICS) and int(AA.shape[1]) <= 192:
        G = AA.conj().T @ AA
    ...
    return u.astype(np.complex128, copy=False), {
        ...
        "reduced_fast_projector_diagnostics": bool(DIRECTIONAL_FAST_PROJECTOR_DIAGNOSTICS),
...
def directional_sketch_delta(target: TargetTrack, y: Any, f: Any, ep: int, seed: int, lm_damping: float = 0.0, trust_radius: float = 0.0, matrix_rcond: float = 1e-12, matrix_condition_cap: float = 1e12) -> Optional[tuple[Any, dict[str, Any]]]:
    ...
    salt = int(seed) + 65537 * int(salt_ep) + 0x412D1A
    P: Optional[Any] = None
    device, dtype = torch_device(), torch_complex_dtype()
    ...
    accepted = False
    cascade_dims = directional_q_cascade_dims_for(n, k, coded_q)
    cascade_tried_dims: list[int] = []
    cascade_reasons: list[str] = []
    cascade_stage = 0
    cascade_accepted_dim = 0
    tried_full_sketch = False

    def cascade_basis(dim: int) -> Any:
        nonlocal tried_full_sketch
        dd = int(dim)
        if dd >= n:
            return _sketch_basis_np(n, n, salt=salt + 0x419F011)
        if dd >= k:
            tried_full_sketch = True
            return full_basis()
        return _coded_composite_basis_np(n, k, dd, salt=salt)

    def cascade_probe_kind(dim: int) -> str:
        if int(dim) >= n:
            return "q-residual-cascade-full-n-irp"
        if int(dim) >= k:
            return "q-residual-cascade-full-sketch"
        return "q-residual-cascade"

    if bool(DIRECTIONAL_CODED_PROBE) and coded_q < k:
        for stage_idx, q_try in enumerate(cascade_dims, start=1):
            C = cascade_basis(q_try)
            is_last_stage = stage_idx == len(cascade_dims)
            cascade_tried_dims.append(int(q_try))
            direct_identity = _identity_target_coded_projection(target, yy, ff, C, n, max(k, int(q_try)), int(q_try))
            if direct_identity is not None:
                u_np, lin, rmeta, samples, JD = direct_identity
                active_basis, active_dim, probe_kind, diff_used = C, int(q_try), cascade_probe_kind(q_try), "identity-target-direct"
                accepted, cascade_stage, cascade_accepted_dim = True, int(stage_idx), int(q_try)
                rmeta["reduced_q_residual_cascade_identity_projection"] = True
                break
            try:
                u_np, lin, rmeta, samples, JD = solve_or_raise(C, full_diff)
                lin_rel = finite_norm(lin) / max(1e-300, finite_norm(ff))
                accepted = DIRECTIONAL_DIFF_MODE != "auto" or lin_rel <= float(DIRECTIONAL_AUTO_CENTRAL_CAP) or bool(is_last_stage)
                if accepted:
                    active_basis, active_dim, probe_kind, diff_used = C, int(q_try), cascade_probe_kind(q_try), full_diff
                    cascade_stage, cascade_accepted_dim = int(stage_idx), int(q_try)
                    if lin_rel > float(DIRECTIONAL_AUTO_CENTRAL_CAP):
                        rmeta["reduced_q_residual_cascade_forced_last_stage"] = bool(is_last_stage)
                    break
                if DIRECTIONAL_DIFF_MODE == "auto":
                    reason = f"dim={int(q_try)}:forward-linear-residual:{lin_rel:.3e}"
                    cascade_reasons.append(reason)
                    try:
                        u_np, lin, rmeta, samples, JD = solve_or_raise(C, "central")
                        lin_rel = finite_norm(lin) / max(1e-300, finite_norm(ff))
                        accepted = lin_rel <= float(DIRECTIONAL_AUTO_CENTRAL_CAP) or bool(is_last_stage)
                        if accepted:
                            active_basis, active_dim, probe_kind, diff_used = C, int(q_try), cascade_probe_kind(q_try), "central"
                            cascade_stage, cascade_accepted_dim = int(stage_idx), int(q_try)
                            fallback_reason = reason
                            if lin_rel > float(DIRECTIONAL_AUTO_CENTRAL_CAP):
                                rmeta["reduced_q_residual_cascade_forced_last_stage"] = bool(is_last_stage)
                            break
                        cascade_reasons.append(f"dim={int(q_try)}:central-linear-residual:{lin_rel:.3e}")
                    except Exception as exc:
                        cascade_reasons.append(f"dim={int(q_try)}:central-error:{type(exc).__name__}")
            except Exception as exc:
                cascade_reasons.append(f"dim={int(q_try)}:forward-error:{type(exc).__name__}")
        if cascade_reasons:
            coded_fallback_reason = ";".join(cascade_reasons[-6:])
    ...
    lin_norm = finite_norm(lin)
    ff_norm = max(1e-300, finite_norm(ff))
    return delta, {
        "hypercube_order": 1,
        "hypercube_nodes": int(samples),
        "hypercube_delta1_norm": finite_norm(delta),
        "directional_jet": True,
        "directional_jet_algorithm": "419-q-residual-cascade-irp-only-coded-to-full-directional-subspace",
        ...
        "directional_q_residual_cascade": bool(DIRECTIONAL_Q_RESIDUAL_CASCADE),
        "directional_q_cascade_growth": float(DIRECTIONAL_Q_CASCADE_GROWTH),
        "directional_q_cascade_max_stages": int(DIRECTIONAL_Q_CASCADE_MAX_STAGES),
        "directional_q_cascade_final_full_n": bool(DIRECTIONAL_Q_CASCADE_FINAL_FULL_N),
        "directional_q_cascade_dims": [int(x) for x in cascade_tried_dims],
        "directional_q_cascade_stage": int(cascade_stage),
        "directional_q_cascade_accepted_dim": int(cascade_accepted_dim),
        "directional_no_external_dense_fallback": True,
        ...
        "directional_fast_projector_diagnostics": bool(DIRECTIONAL_FAST_PROJECTOR_DIAGNOSTICS),
        ...
        "directional_linear_residual": lin_norm,
        "directional_linear_relative_residual": lin_norm / ff_norm,
...
def hypercube_matrixjet_corrector(target: TargetTrack, y0: Sequence[complex], max_epochs: int, tol: float, accept: float, trial_timeout: float, line_search: int = 12, line_grid: Sequence[float] = (), direction_seed: int = 0, cloud_nodes: int = 0, lm_damping: float = 0.0, trust_radius: float = 0.0, matrix_rcond: float = 1e-12, matrix_condition_cap: float = 1e12) -> dict[str, Any]:
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        if np.any(np.isclose(L, 1.0)):
            y_full = y + delta
            r_full = target.residual(y_full)
            total_line += 1
            if math.isfinite(float(r_full)) and float(r_full) < best_r:
                best_y, best_r = y_full.copy(), float(r_full)
            if math.isfinite(float(r_full)) and float(r_full) <= max(float(tol), float(accept)) and (accept <= 0 or float(r_full) < accept):
                y = y_full.copy()
                ok, status = True, "converged"
                epochs = ep + 1
                last["hypercube_chosen_lambda"] = 1.0
                last["line_search_early_unit_step"] = True
                break
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    result = {
        ...
        "mode": "419-standalone-pandrosion-irp-q-residual-cascade-directional-sketch-local-jet-matrix",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="419 standalone Pandrosion IRP q-residual cascade directional-sketch local-jet matrix engine.")
    ...
    p.add_argument("--batch-kernel", choices=["auto", "sketch", "realpack", "complex"], default="auto", help="Batched prepass kernel. auto follows the selected 419 matrix algorithm.")
    ...
    p.add_argument("--matrix-backend", choices=["torch", "numpy", "auto"], default="torch", help="Matrix kernel backend. torch+adaptive-directional-sketch is the 419 default; numpy preserves the SVD diagnostic behavior.")
    ...
    p.add_argument("--torch-real-dtype", choices=["auto", "float16", "bfloat16", "float32", "float64"], default="auto", help="Torch real dtype for 419 real-packed kernels. auto uses float32.")
    ...
    p.add_argument("--sketch-seed", type=int, default=419, help="Deterministic seed for the randomized sketch basis.")
    ...
    p.add_argument("--directional-coded-probe", action="store_true", default=True, help="Try the 419 composite-cached coded probe D=P R with q<<k before growing the q-residual cascade.")
    ...
    p.add_argument("--directional-fast-projector", action="store_true", default=True, help="Try the 419 diagonal projected reduced solver before SVD/Newton-Schulz.")
    ...
    p.add_argument("--directional-fast-projector-diagnostics", action="store_true", default=False, help="Compute optional off-diagonal/diagonal-spread diagnostics for the fast projector.")
    p.add_argument("--no-directional-fast-projector-diagnostics", dest="directional_fast_projector_diagnostics", action="store_false")
    p.add_argument("--directional-identity-target-fastpath", action="store_true", default=True, help="For validated F(z)=z-root targets, project directly onto the coded q basis before oracle-sampled differences.")
    p.add_argument("--no-directional-identity-target-fastpath", dest="directional_identity_target_fastpath", action="store_false")
    p.add_argument("--directional-q-residual-cascade", action="store_true", default=True, help="Grow q through residual IRP stages instead of treating a failed small-q probe as the end of the Pandrosion path.")
    p.add_argument("--no-directional-q-residual-cascade", dest="directional_q_residual_cascade", action="store_false")
    p.add_argument("--directional-q-cascade-growth", type=float, default=2.0, help="Geometric growth factor for q residual cascade dimensions.")
    p.add_argument("--directional-q-cascade-max-stages", type=int, default=0, help="Maximum number of q growth stages before k/full-n (0 = no limit).")
    p.add_argument("--directional-q-cascade-final-full-n", action="store_true", default=True, help="Allow a final n-dimensional IRP stage. Expensive, but avoids external dense fallback.")
    p.add_argument("--no-directional-q-cascade-final-full-n", dest="directional_q_cascade_final_full_n", action="store_false")
    ...
    p.add_argument("--outdir", default="/private/tmp/419_standalone_q_residual_cascade_pandrosion_irp_gemm_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(args.self_test):
        ...
        args.out = args.out or "/private/tmp/419_standalone_q_residual_cascade_pandrosion_irp_gemm_out/self_test_419.json"
    ...
    out = Path(args.out) if args.out else Path(args.outdir) / f"419_q_residual_cascade_pandrosion_irp_gemm_{cases[0].replace(',', 'x') if cases else 'case'}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")
    print("=" * 120, flush=True)
    print("419 STANDALONE PANDROSION IRP -- Q-RESIDUAL CASCADE MATRIX-JET CALCULUS", flush=True)
