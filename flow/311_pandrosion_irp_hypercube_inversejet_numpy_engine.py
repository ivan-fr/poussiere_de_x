#!/usr/bin/env python3
"""
311_pandrosion_irp_hypercube_inversejet_numpy_engine.py
...
This Python engine does not use Halley as the active corrector.  The 311 path
samples fixed universal atlas cells, applies StartOpt-style geometric
improvement, then runs a 310-style IRP chart loop whose local engine is the
306 hypercube least-squares inverse-jet corrector:
...
No imports from previous Pandrosion scripts.  This file contains its own:
  ...
  - 311 IRP scalar homothety / coordinatewise reciprocal chart wrapper
...
Core flow per trial:
    ...
    u
      ...
      -> y
      -> z = A y
      -> repeated IRP chart selection: direct homothety or reciprocal chart
      -> 306 hypercube tensor inverse-jet burst inside the selected chart
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    for trial in range(int(args.pool)):
        ...
        loc = irp_hypercube_inversejet_corrector_311(
            target, y0,
        ...
        roots.append({
            "id": rid,
            "source": "311-irp-hypercube-inversejet",
            "trial": int(trial),
    ...
    result = {
        "script": "311_pandrosion_irp_hypercube_inversejet_numpy_engine.py",
        "autonomous": True,
        "dependencies": {"python_scripts": [], "numpy": bool(np is not None)},
        "mode": "311-irp-hypercube-inversejet",
        "flow_formula": "304 universal atlas -> StartOpt(y) -> 311 IRP chart cascade (homothety/optional reciprocal) -> 306 hypercube least-squares inverse-jet delta1+delta2+delta3 in the selected chart -> return chart; no Halley, no anchored Q, no Newton fallback",
        "case": f"{n},{d}",
        "family": "ks",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="311 standalone NumPy Pandrosion engine: 306 hypercube inverse-jet core plus IRP chart renormalization.")
    ...
    p.add_argument("--irp-layers", type=int, default=2, help="311: number of IRP chart layers per capture epoch")
    p.add_argument("--irp-inner-epochs", type=int, default=2, help="311: 306 hypercube epochs inside each IRP chart layer")
    p.add_argument("--local-inner-epochs", type=int, default=3, help="311: inner 306 epochs after locality collapse")
    p.add_argument("--irp-chart-top", type=int, default=2, help="311: number of ranked charts tested per IRP layer")
    p.add_argument("--irp-gains", default="1,0.5,2,0.25,4,0.125,8", help="311: homothety gains for the IRP chart palette")
    p.add_argument("--irp-phases", default="0,0.08,-0.08,0.19,-0.19", help="311: phase offsets in radians for complex homothety charts")
    p.add_argument("--irp-top", type=int, default=14, help="311: maximum number of complex chart scales")
    p.add_argument("--irp-inversion", action="store_true", default=True, help="311: include coordinatewise reciprocal charts y=1/(c u)")
    p.add_argument("--no-irp-inversion", dest="irp_inversion", action="store_false")
    p.add_argument("--collapse", action="store_true", default=True, help="311: freeze to local direct chart after observed localization")
    ...
    p.add_argument("--outdir", default="/mnt/data/304_universal_atlas_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(getattr(args, "self_test", False)):
        ...
        args.out = args.out or "/mnt/data/304_universal_atlas_out/self_test_304.json"
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "311_pandrosion_irp_hypercube_inversejet_numpy_engine.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
        first = cases[0].replace(",", "x") if cases else "case"
        out = Path(args.outdir) / f"304_universal_atlas_{first}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")

    print("=" * 120, flush=True)
    print("311 autonomous IRP + UNIVERSAL HYPERCUBE INVERSE-JET NumPy Pandrosion", flush=True)
    print("No dependency on previous Python scripts; 304/306 universal atlas + hypercube tensor inverse-jet core wrapped in 311 IRP chart homothety/reciprocal renormalization; no Halley/anchored-Q/Newton fallback path.", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
