#!/usr/bin/env python3
"""
201_pandrosion_logstable_optprobe_tensor_halley_numpy_engine.py

Autonomous PROBE-AWARE vectorized PURE Pandrosion Thales root extractor.

120 is the direct NumPy-only successor of 120.  It keeps the
pure finite-slope Thales/Pandrosion corrector, but adds theorem-guided
geometric safeguards from the newer papers:
  - equal-value pole avoidance: penalize probes with F(b) close to F(a);
  - finite-slope curvature scoring: prefer probes whose averaged slopes vary less;
  - condition-aware trust region for ill-conditioned finite slopes;
  - all scoring remains residual-gated and NumPy-batched.

It intentionally does NOT add inverse-jet tensors, analytic Jacobian fallback, Newton,
Halley, Broyden, or homotopy paths.
...
def _probe_endpoint_candidates(
    ...
    # 201 logarithmic/projective stability: probes that violently change
...
def _gated_tensor_halley_delta_201(
    ...
    try:
        ...
        # 201 tensorial enrichment: add a small number of deterministic
...
def pandrosion_corrector(
    ...
    """201 corrector: log-stable optimal-probe tensor-Halley Pandrosion.
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        if bool(halley_enabled):
            delta, hmeta = _gated_tensor_halley_delta_201(
                target=target,
                y=y,
                f=f,
                Q=Q,
                ...
                log_energy=_log_stability_energy_201(y),
    ...
    return {
        ...
        "corrector": "201-logstable-optprobe-tensor-finite-slope-halley",
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    for trial in range(int(args.pool)):
        ...
        roots.append({
            "id": rid,
            "source": "201-logstable-optprobe-tensor-halley-pandrosion",
            "trial": int(trial),
    ...
    result = {
        "script": "201_pandrosion_logstable_optprobe_tensor_halley_numpy_engine.py",
        "autonomous": True,
        "dependencies": {"python_scripts": [], "numpy": bool(np is not None)},
        "mode": "201-logstable-optimal-probe-tensor-finite-slope-halley",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="201 standalone NumPy Pandrosion engine: log-stable optimal-probe tensor-Halley successor of 200.")
    ...
    p.add_argument("--halley", action="store_true", default=True, help="201: enable gated cubic finite-slope Halley correction")
    p.add_argument("--no-halley", dest="halley", action="store_false", help="201: disable gated Halley layer and behave close to 120")
    p.add_argument("--halley-gate-residual", type=float, default=0.25, help="201: residual scale for continuous Halley activation")
    p.add_argument("--halley-probe-fraction", type=float, default=0.50, help="201: symmetric second-slope probe fraction along raw Pandrosion step")
    p.add_argument("--halley-cond-weight", type=float, default=0.025, help="201: condition penalty in Halley activation gate")
    p.add_argument("--halley-min-gate", type=float, default=0.04, help="201: below this gate the Halley direction is skipped")
    p.add_argument("--halley-max-correction", type=float, default=1.25, help="201: cap ||delta2|| relative to ||delta_raw||")
    p.add_argument("--probe-log-weight", type=float, default=0.0005, help="201: penalty for absolute logarithmic/projective scale energy of probes")
    p.add_argument("--probe-log-delta-weight", type=float, default=0.0015, help="201: penalty for probes that distort the current logarithmic scale")
    p.add_argument("--halley-log-weight", type=float, default=0.03, help="201: logarithmic stability penalty in the Halley gate")
    p.add_argument("--halley-log-scale", type=float, default=1.0, help="201: scale for logarithmic stability gate")
    p.add_argument("--tensor-extra-directions", type=int, default=0, help="201: extra symmetric finite-slope tensor directions near a root")
    ...
    p.add_argument("--outdir", default="/mnt/data/201_logstable_tensor_halley_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(getattr(args, "self_test", False)):
        ...
        args.out = args.out or "/mnt/data/201_logstable_tensor_halley_out/self_test_201.json"
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "201_pandrosion_logstable_optprobe_tensor_halley_numpy_engine.py", "autonomous": True, "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
        first = cases[0].replace(",", "x") if cases else "case"
        out = Path(args.outdir) / f"120_probe_aware_pure_{first}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")

    print("=" * 120, flush=True)
    print("201 autonomous LOG-STABLE OPTIMAL-PROBE TENSOR-HALLEY NumPy Pandrosion", flush=True)
