#!/usr/bin/env python3
"""
318_pandrosion_standalone_local_jet_geometry_fully_geometric_irp_hypercube_inversejet_numpy_engine.py

Standalone NumPy engine that realises pandrosion as a FULLY (jet-)GEOMETRIC method.

NEW IN 318 (over 317) -- same architecture, six structural upgrades:

  [318-A] PAIRED CENTRAL-DIFFERENCE JETS.  The hypercube corrector now builds its
      local Jacobian from n paired probes z +/- h_j e_j with a PER-COORDINATE radius
      h_j ~ jet_radius * max(1,|z_j|).  Same oracle budget as the old random-sign
      cloud (2n+1 samples) but the truncation error drops from O(h) to O(h^2), the
      sampling is deterministic, and the diagonal curvature comes for free.  The old
      random cloud is kept behind --jet-sampling cloud.  Near-singular inversions
      fall back to a truncated-SVD pseudo-inverse before giving up.

  [318-B] VECTORISED SWARM PREFILTER.  Instead of correcting one atlas start at a
      time, 318 first runs a few damped-Newton iterations on a whole BATCH of atlas
      starts simultaneously (one stacked eval_batch for all Jacobians, one for all
      line searches; batched solve/pinv).  Survivors are culled by residual descent
      and a diversity separation, and only then enter the full 312/306 pipeline as
      priority starts.  This amortises the oracle cost across starts and typically
      cuts trials and wall time sharply on all-root extraction.

  [318-C] GHOST-TRAP ESCAPES.  The parametric dynamics of the pandrosion iteration
      provably admits attracting cycles that are NOT roots (period-doubling below
      the explicit threshold x* = (1+sqrt(3))^-3 in the scalar p=3 chart; attracting
      9/27-cycles in the complex parameter plane).  A trial that stalls in such a
      ghost attractor used to be abandoned; 318 detects the stall and spends a small
      escape budget (--ghost-escapes) on phase/reciprocal chart kicks of the best
      iterate before giving up.

  [318-D] GUARANTEED CHART-KIND DIVERSITY.  The same threshold theorem shows the
      direct chart has a hard instability region; the IRP candidate shortlist now
      always contains at least one reciprocal AND one direct chart when available,
      rather than letting the static score pick two of the same kind.

  [318-E] EARLY DUPLICATE ABORT.  During correction, an iterate that has entered a
      small neighbourhood of an already-accepted root aborts immediately with status
      'duplicate-early' instead of spending its full epoch budget to rediscover it.

  [318-F] SINGULAR-DIRECTION SEEDS + BOUNDED JET CACHE.  The origin affine seed now
      also spawns a few starts along the smallest right singular directions of the
      origin Jacobian (the locally flattest -- root-richest -- directions), and the
      jet cache is a bounded LRU instead of an unbounded dict.
...
def universal_atlas_start(target: TargetTrack, n: int, trial: int, seed: int, powers: Sequence[float], angles: Sequence[float], radii: Sequence[float], cap: float, roots_found: int, duplicates: int, failures: int, target_count: int, universal_cells: int = 16, universal_shells: int = 5, atlas_selection: str = "diverse-shell", **_: Any) -> tuple[Any, dict[str, Any]]:
    ...
    if mode in {"diverse", "diverse-shell", "shell", "raw-shell"}:
        ...
        meta = {
            "chart": "318-standalone-universal-diverse-shell-atlas",
            "atlas_mode": "318-diverse-shell",
...
def batched_paired_jacobians(target: TargetTrack, Y: Any, h_rel: float = 1e-5) -> tuple[Any, Any]:
...
def swarm_prefilter(target: TargetTrack, n: int, seed: int, radii: Sequence[float], size: int, iters: int, keep: int, line_lams: Sequence[float], sep: float) -> tuple[list[Any], dict[str, Any]]:
    ...
    for it in range(max(0, int(iters))):
        ...
        if idx.size == 0:
            break
        F0, J = batched_paired_jacobians(target, Y[idx])
        ...
        Rc = target.residuals_batch(cand.reshape(-1, n)).reshape(idx.size, L.size)
        bestl = np.argmin(np.where(np.isfinite(Rc), Rc, np.inf), axis=1)
        Rbest = Rc[np.arange(idx.size), bestl]
        improved = np.isfinite(Rbest) & (Rbest < R[idx])
        Y[idx[improved]] = cand[np.arange(idx.size), bestl][improved]
        R[idx[improved]] = Rbest[improved]
        if it >= 1:
...
def hypercube_delta(target: TargetTrack, y: Any, f: Any, ep: int, seed: int, cloud_nodes: int = 0, lm_damping: float = 0.0, trust_radius: float = 0.0, sampling: str = "paired") -> tuple[Any, dict[str, Any]]:
    n = len(y)
    ...
    # Inverse-jet inversion of A.  Order of defences: optional LM damping ->
    ...
    if lm or tr:
        extra.update({"hypercube_lm_damping": float(lm_damping), "hypercube_trust_radius": float(trust_radius)})
    try:
        d1 = cap(solve_step(-f))
    except Exception:
...
def hypercube_inversejet_corrector(target: TargetTrack, y0: Sequence[complex], max_epochs: int, tol: float, accept: float, trial_timeout: float, line_search: int = 12, line_grid: Sequence[float] = (), direction_seed: int = 0, cloud_nodes: int = 0, lm_damping: float = 0.0, trust_radius: float = 0.0, sampling: str = "paired") -> dict[str, Any]:
    t0 = now()
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        if r <= max(float(tol), float(accept)) and (accept <= 0 or r < accept):
            ...
            break
        delta, meta = hypercube_delta(target, y, f, ep, int(direction_seed), int(cloud_nodes), float(lm_damping), float(trust_radius), str(sampling))
        last = dict(meta)
...
def lazy_irp_hypercube_inversejet_corrector_312(base: TargetTrack, y0: Sequence[complex], max_epochs: int, tol: float, accept: float, trial_timeout: float, line_search: int = 12, line_grid: Sequence[float] = (), direction_seed: int = 0, cloud_nodes: int = 0, irp_layers: int = 2, irp_inner_epochs: int = 2, irp_scales: Optional[Sequence[complex]] = None, irp_chart_top: int = 2, irp_inversion: bool = True, collapse: bool = True, collapse_residual: float = 1e-4, collapse_drop: float = 0.42, collapse_rel_step: float = 0.35, collapse_after: int = 2, local_inner_epochs: int = 3, lazy_direct_epochs: int = 1, lazy_trigger_drop: float = 0.82, lazy_trigger_after: int = 1, lazy_bad_cond: float = 1e10, lazy_log_energy: float = 8.0, eager_irp: bool = False, rescue_collapsed: bool = False, lm_damping: float = 0.0, trust_radius: float = 0.0, sampling: str = "paired", known_y: Optional[Sequence[Any]] = None, dup_sep: float = 0.0, ghost_escapes: int = 2, ghost_kick: float = 0.45, ghost_gate: float = 1e4) -> dict[str, Any]:
    ...
    for ep in range(max(1, int(max_epochs))):
        ...
        if r0 <= max(float(tol), float(accept)) and (accept <= 0 or r0 < accept):
            ok, status = True, "converged"
            break
        loc = hypercube_inversejet_corrector(base, y, max(1, int(local_inner_epochs if collapsed else lazy_direct_epochs)), tol, accept, 0.0 if deadline is None else max(0.0, deadline - now()), line_search, line_grid, int(direction_seed) + 1000003 * ep + 17, cloud_nodes, lm_damping, trust_radius, sampling)
        absorb(loc)
        last = dict(loc)
        ...
        for layer in range(max(1, int(irp_layers))):
            ...
            for _, ct, u0, cm in shortlist:
                loc2 = hypercube_inversejet_corrector(ct, u0, max(1, int(irp_inner_epochs)), tol, accept, 0.0, line_search, line_grid, int(direction_seed) + 1000003 * ep + 65537 * layer, cloud_nodes, lm_damping, trust_radius, sampling)
                absorb(loc2)
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    if bool(args.swarm) and int(args.count) > 0 and not user_starts:
        ...
        swarm_starts_list, swarm_meta = swarm_prefilter(
            target, n, geometry.seed + 0x318000, parse_float_list(args.rays, DEFAULT_RADII, positive=True),
    ...
    priority_starts: list[tuple[str, Any]] = (
        [("318-explicit-start", s) for s in user_starts]
        + [("318-swarm-start", s) for s in swarm_starts_list]
        + [("318-singular-direction-seed", s) for s in eigen_starts]
    )
    starts = user_starts
    ...
    for trial in range(int(args.pool)):
        ...
        if explicit:
            ...
            if tag == "318-swarm-start" and roots:
                # [318-B/E] swarm survivors have already descended several Newton
            ...
            if tag in {"318-swarm-start", "318-singular-direction-seed"}:
            ...
            if bool(args.origin_seed) and (trial == 0 if int(args.origin_seed_period) <= 0 else trial % max(1, int(args.origin_seed_period)) == 0):
                ...
                if y_origin is not None:
                    y_raw = y_origin
                    geom = {"chart": "318-origin-affine-finite-difference-seed", "atlas_mode": "origin-affine-seed", "homothety": 1.0, "theta_deg": None, **ometa}
            if y_raw is None:
    ...
    result = {
        ...
        "mode": "318-standalone-local-jet-geometry-fully-geometric-swarm-lazy-irp-hypercube-inversejet",
        "flow_formula": "any system as black-box geometric oracle -> lazily-sampled local-jet field (paired central differences, O(n)/step, no global precompute) -> [318-B] batched swarm prefilter -> universal Mobius atlas + singular-direction starts -> StartOpt -> 312 lazy IRP (kind-diverse charts, ghost escapes) / 306 hypercube inverse-jet (SVD fallback) -> early duplicate abort -> jet-Newton polish, ALL on the jet geometry -> faithful (jet) residual acceptance",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="318 standalone NumPy local-jet-geometry fully-geometric swarm + lazy IRP / hypercube inverse-jet engine.")
    ...
    p.add_argument("--outdir", default="/tmp/318_standalone_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if bool(args.self_test):
        ...
        args.out = args.out or "/tmp/318_standalone_out/self_test_318.json"
    ...
    out = Path(args.out) if args.out else Path(args.outdir) / f"318_standalone_{cases[0].replace(',', 'x') if cases else 'case'}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")
    print("=" * 120, flush=True)
    print("318 STANDALONE LOCAL-JET GEOMETRY -- FULLY GEOMETRIC SWARM + LAZY IRP / HYPERCUBE INVERSE-JET", flush=True)
    print("NumPy + stdlib only; no local flow imports. Paired central-difference jets, batched swarm prefilter,", flush=True)
    print("ghost-trap escapes, kind-diverse IRP charts, early duplicate abort; residual is faithful (true oracle).", flush=True)
    print("=" * 120, flush=True)
    for r in outputs:
        s = r["summary"]
        g = r.get("geometry", {})
        ...
        if sw.get("swarm_enabled"):
            print(f"swarm: size={sw.get('swarm_size')} iters={sw.get('swarm_iters')} survivors={sw.get('swarm_survivors')} kept={sw.get('swarm_kept')} best_r={sw.get('swarm_best_residual'):.3e} ({sw.get('swarm_seconds', 0.0):.2f}s)", flush=True)
