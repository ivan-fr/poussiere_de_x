"""
123_pandrosion_standalone_riemann_aware_auto_probe_engine.py

Standalone FULL RIEMANN-CHART + RIEMANN-AWARE AUTO-PROBE Pandrosion engine.

This is the clean 123 release.  It is standalone like 122, but its auto probe
heuristic is updated from the 118-vs-122 geometry benchmark: once the projective
Riemann/Mobius chart layer is active, the best local frame is often not the old
(n,d)-only choice.  123 therefore chooses among star/adaptive/simplex-cube/balanced
with a Riemann-aware rule, while keeping the same exact Pandrosion correction.
...
def projective_chart_homothety_start(
    ...
    meta = {
        ...
        "chart": "123-coordinate-projective-mobius-thales-return-map",
...
def pandrosion_corrector(
    ...
    is unknown, 119 chooses b by minimizing ||G(b)|| over a structured nD family
    of geometric probes.
...
def run_case(args: argparse.Namespace, case_raw: str) -> dict[str, Any]:
    ...
    for trial in range(int(args.pool)):
        ...
        roots.append({
            "id": rid,
            "source": "123-standalone-riemann-aware-auto-probe-pure-pandrosion",
            "trial": int(trial),
    ...
    result = {
        "script": "123_pandrosion_standalone_riemann_aware_auto_probe_engine.py",
        "autonomous": True,
        "dependencies": standalone_dependency_audit(),
        "mode": "standalone-full-riemann-chart-riemann-aware-auto-probe-pure-pandrosion",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--outdir", default="/mnt/data/123_riemann_aware_auto_probe_out")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if len(outputs) == 1:
        ...
        final = {"script": "123_pandrosion_standalone_riemann_aware_auto_probe_engine.py", "standalone": True, "dependencies": standalone_dependency_audit(), "cases": outputs}
    if args.out:
        out = Path(args.out)
    else:
        first = cases[0].replace(",", "x") if cases else "case"
        out = Path(args.outdir) / f"123_riemann_aware_auto_probe_{first}.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(final, indent=2), encoding="utf-8")

    if not bool(getattr(args, "quiet", False)):
        print("=" * 120, flush=True)
        print("123 standalone FULL RIEMANN-CHART + RIEMANN-AWARE AUTO-PROBE PURE Pandrosion - no fallback", flush=True)
        print("No dependency on previous Python scripts; explicit chart Phi(y), homothety/inversion/return map + Riemann-aware automatic nD b-probes; no Newton/Jacobian fallback path.", flush=True)
        print("=" * 120, flush=True)
        for r in outputs:
            s = r["summary"]
