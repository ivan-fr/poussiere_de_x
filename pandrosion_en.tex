...
\bottomrule
\end{tabular}
\caption{Contraction ratios for $x < 1$. The negative sign indicates oscillatory convergence. Note the symmetry: $|\lambda_{p,x}| = |\lambda_{p,1/x}|$.}
\end{table}

\begin{remark}[Residual symmetry]
An attractive consequence is the \emph{residual symmetry}: $|\lambda_{p,x}| = |\lambda_{p,1/x}|$. Computing the $p$-th root of $x$ and that of $1/x$ by Pandrosion's method have exactly the same speed of convergence.
\end{remark}
...
\begin{itemize}
  ...
  \item \textbf{Extension to $x < 1$} (Proposition~\ref{prop:x_small}): the method converges with \emph{oscillatory} contraction ratio $\lambda < 0$, and $|\lambda_{p,x}| = |\lambda_{p,1/x}|$.
